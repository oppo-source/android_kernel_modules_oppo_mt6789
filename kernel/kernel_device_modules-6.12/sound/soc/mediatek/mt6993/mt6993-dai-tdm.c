...
{
	if (strstr(name, "DPTX"))
		return MT6993_DAI_TDM_DPTX;
	else
		return MT6993_DAI_TDM;
}
				...
				int event)
{
	...
	int apll_rate = mt6993_get_apll_rate(afe, tdm_priv->mclk_apll);
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6993_mck_enable(afe, tdm_priv->bck_id, tdm_priv->bck_rate);
		break;
	case SND_SOC_DAPM_POST_PMD:
		mt6993_mck_disable(afe, tdm_priv->bck_id, tdm_priv->bck_rate);
		break;
	default:
		break;
	}
				...
				int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt6993_mck_enable(afe, tdm_priv->mclk_id, tdm_priv->mclk_rate);
		break;
	case SND_SOC_DAPM_POST_PMD:
		tdm_priv->mclk_rate = 0;
		mt6993_mck_disable(afe, tdm_priv->mclk_id, tdm_priv->mclk_rate);
		break;
	default:
		break;
	}
				 ...
				 struct snd_soc_dai *dai)
{
	...
	if (tdm_id == MT6993_DAI_TDM_DPTX) {
		/* for mt6993 use 0 = 2ch for 4 pin */
		...
		int id)
{
	...
	tdm_priv->bck_id = MT6993_TDMOUT_BCK;
	tdm_priv->mclk_id = MT6993_TDMOUT_MCK;
...
int mt6993_dai_tdm_register(struct mtk_base_afe *afe)
{
	...
	tdm_priv = init_tdm_priv_data(afe, MT6993_DAI_TDM);
	if (!tdm_priv)
		return -ENOMEM;

	tdm_dptx_priv = init_tdm_priv_data(afe, MT6993_DAI_TDM_DPTX);
	if (!tdm_dptx_priv)
		return -ENOMEM;

	afe_priv->dai_priv[MT6993_DAI_TDM] = tdm_priv;
	afe_priv->dai_priv[MT6993_DAI_TDM_DPTX] = tdm_dptx_priv;
