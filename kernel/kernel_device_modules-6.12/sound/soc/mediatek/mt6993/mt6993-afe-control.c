...
unsigned int mt6993_swpm_rate_transform(struct device *dev,
		unsigned int rate)
{
	switch (rate) {
	case 32000:
				   ...
				   unsigned int rate, int aud_blk)
{
	...
	default:
		return mt6993_general_rate_transform(dev, rate);
	}
}
...
EXPORT_SYMBOL(mt6993_enable_dc_compensation);
...
EXPORT_SYMBOL(mt6993_set_lch_dc_compensation);
...
EXPORT_SYMBOL(mt6993_set_rch_dc_compensation);
...
EXPORT_SYMBOL(mt6993_adda_dl_gain_control);

int mt6993_get_output_device(struct mtk_base_afe *afe)
{
...
int mt6993_get_input_device(struct mtk_base_afe *afe)
{
...
void mt6993_aud_update_power_scenario(void)
{
	...
	adda_priv = afe_priv->dai_priv[MT6993_DAI_ADDA];
	...
	audio_data.output_device = mt6993_get_output_device(local_afe);
	audio_data.input_device = mt6993_get_input_device(local_afe);
	if (audio_data.input_device != AUDIO_INPUT_INVALID &&
		...
		audio_data.sample_rate = mt6993_swpm_rate_transform(local_afe->dev, value);
...
EXPORT_SYMBOL(mt6993_aud_get_power_scenario);
