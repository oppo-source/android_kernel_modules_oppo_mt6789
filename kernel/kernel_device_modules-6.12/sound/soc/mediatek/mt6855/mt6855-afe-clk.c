				    ...
				    int clk_id)
{
	...
	if (clk_id < 0 || clk_id >= CLK_NUM) {
		dev_info(afe->dev, "%s(), clk_id=%d invalid!", __func__, clk_id);
		return -EINVAL;
	}
...
int mt6855_mck_enable(struct mtk_base_afe *afe, int mck_id, int rate)
{
	...
	if (mck_id >= 0 && mck_id < MT6855_MCK_NUM) {
		...
		dev_info(afe->dev, "%s, mck_id=%d invalid!\n",
			__func__, mck_id);
		return -EINVAL;
	}
	...
	if (div_clk_id >= 0 && div_clk_id < CLK_NUM) {
		...
		dev_info(afe->dev, "%s, div_clk_id=%d invalid!\n",
			__func__, div_clk_id);
		return -EINVAL;
	}
...
int mt6855_init_clock(struct mtk_base_afe *afe)
{
	...
	if (ret) {
		dev_info(afe->dev, "%s(), clk_set_rate %s, rate 180633601, fail %d\n",
			__func__, aud_clks[CLK_APMIXED_APLL1], ret);
		return ret;
	}
	ret = clk_set_rate(afe_priv->clk[CLK_APMIXED_APLL1], 180633600);
	if (ret) {
		dev_info(afe->dev, "%s(), clk_set_rate %s, rate 180633600, fail %d\n",
			__func__, aud_clks[CLK_APMIXED_APLL1], ret);
		return ret;
	}
	...
	if (IS_ERR(afe_priv->pericfg)) {
		dev_info(afe->dev, "%s() Cannot find pericfg: %ld\n",
