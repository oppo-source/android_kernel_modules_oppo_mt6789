...
int mtk_scp_audio_init_mem(void)
{
	...
	if (!scp_audio_rsv_mem->phy_addr) {
		pr_info("%s(), scp audio rsv mem phy_addr error\n", __func__);
		return -1;
	}

	if (!scp_audio_rsv_mem->vir_addr) {
		pr_info("%s(), scp audio rsv mem phy_addr vir_addr error\n", __func__);
		return -1;
	}

	if (!scp_audio_rsv_mem->size) {
		pr_info("%s(), scp audio rsv mem phy_addr size error\n", __func__);
		return -1;
	}
	...
	if (!vaddr) {
		pr_info("%s(), alloc ATOD mem vaddr Fail!!!\n", __func__);
		return -1;
	}

	paddr = gen_pool_virt_to_phys(scp_audio->genpool, vaddr);
	if (paddr < 0) {
		pr_info("%s(), alloc ATOD mem paddr Fail!!!\n", __func__);
		return -1;
	}
	...
	if (!vaddr) {
		pr_info("%s(), alloc DTOA mem vaddr Fail!!!\n", __func__);
		return -1;
	}

	paddr = gen_pool_virt_to_phys(scp_audio->genpool, vaddr);
	if (paddr < 0) {
		pr_info("%s(), alloc DTOA mem paddr Fail!!!\n", __func__);
		return -1;
	}
	...
	dev_info(scp_audio->dev,
		 "%s(), scp reserve mem pa=0x%llx, va=0x%p, size=0x%llx\n",
		 __func__,
		 scp_audio_rsv_mem->phy_addr,
		 scp_audio_rsv_mem->vir_addr,
		 scp_audio_rsv_mem->size);
				     ...
				     struct gen_pool *genpool)
{
	...
	if (!vaddr) {
		pr_info("%s(), alloc pcm mem vaddr Fail!!!\n", __func__);
		return -1;
	}

	paddr = gen_pool_virt_to_phys(genpool, vaddr);
	if (paddr < 0) {
		pr_info("%s(), alloc pcm mem paddr Fail!!!\n", __func__);
		return -1;
	}
...
int mtk_scp_allocate_mem(struct snd_pcm_substream *substream, unsigned int size)
{
	...
	if (id != scp_audio->dl_memif &&
		...
		pr_info("%s(), not match scp audio memif\n", __func__);
		return -1;
	}
	...
	//    !scp_audio_rsv_mem->vir_addr ||
	//    !scp_audio_rsv_mem->size) {
	//	pr_info("%s(), scp audio rsv mem error\n");
	//	return -1;
	//}

	vaddr = gen_pool_alloc(scp_audio->genpool, size);
	if (!vaddr) {
		pr_info("%s(), alloc afe mem vaddr Fail!!!\n", __func__);
		return -1;
	}

	paddr = gen_pool_virt_to_phys(scp_audio->genpool, vaddr);
	if (paddr < 0) {
		pr_info("%s(), alloc afe mem paddr Fail!!!\n", __func__);
		return -1;
	}
