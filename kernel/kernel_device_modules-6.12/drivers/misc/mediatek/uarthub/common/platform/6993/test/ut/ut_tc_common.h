...
{
	uarthub_set_host_trx_request_mt6993(0, TRX);
	usleep_range(3000, 3010);
#if !(SSPM_DRIVER_EN) || (UARTHUB_SUPPORT_FPGA)
	UARTHUB_REG_WRITE(IRQ_CLR_ADDR, 0xFFFFFFFF);
	usleep_range(1000, 1010);
#endif
	if (uarthub_is_host_uarthub_ready_state_mt6993(0) != 1)
		return -1;
	uarthub_set_host_trx_request_mt6993(1, TRX);
	uarthub_set_host_trx_request_mt6993(2, TRX);
	return 0;
}
static inline int __uh_ut_clear_hosts_trx_request(void)
{
	uarthub_clear_host_trx_request_mt6993(0, TRX);
	uarthub_clear_host_trx_request_mt6993(1, TRX);
	uarthub_clear_host_trx_request_mt6993(2, TRX);
	usleep_range(3000, 3010);
#if !(SSPM_DRIVER_EN) || (UARTHUB_SUPPORT_FPGA)
	UARTHUB_REG_WRITE(IRQ_CLR_ADDR, 0xFFFFFFFF);
	usleep_range(1000, 1010);
...
{
	uarthub_set_host_loopback_ctrl_mt6993(0, 1, 1);
	uarthub_set_host_loopback_ctrl_mt6993(1, 1, 1);
	uarthub_set_host_loopback_ctrl_mt6993(2, 1, 1);
	uarthub_set_cmm_loopback_ctrl_mt6993(1, 1);
	return 0;
}
static inline int __uh_ut_disable_loopback_ctrl(void)
{
	uarthub_set_host_loopback_ctrl_mt6993(0, 1, 0);
	uarthub_set_host_loopback_ctrl_mt6993(1, 1, 0);
	uarthub_set_host_loopback_ctrl_mt6993(2, 1, 0);
	uarthub_set_cmm_loopback_ctrl_mt6993(1, 0);
	return 0;
}
