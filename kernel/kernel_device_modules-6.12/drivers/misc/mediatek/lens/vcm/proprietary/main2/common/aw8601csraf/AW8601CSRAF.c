...
#define AF_DRVNAME "AW8601CSRAF_DRV"
...
static int s4AW8601CSRAF_ReadReg(unsigned short *a_pu2Result)
{
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		initdrv();
		ret = s4AW8601CSRAF_ReadReg(&InitPos);
...
/* ////////////////////////////////////////////////////////////// */
long AW8601CSRAF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
...
int AW8601CSRAF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	LOG_INF("Start\n");
	spin_lock(g_pAF_SpinLock);
	if (*g_pAF_Opened == 2)
...
int AW8601CSRAF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int AW8601CSRAF_GetFileName(unsigned char *pFileName)
{
	pFileName[0] = '\0';
	return 1;
}
