...
static struct scp_res_mbrain_dbg_ops _scp_res_mbrain_tmon_ops;
...
{
	return &_scp_res_mbrain_tmon_ops;
}
EXPORT_SYMBOL(get_scp_mbrain_tmon_ops);
...
int register_scp_mbrain_tmon_ops(struct scp_res_mbrain_dbg_ops *ops)
{
	...
	_scp_res_mbrain_tmon_ops.get_length = ops->get_length;
	_scp_res_mbrain_tmon_ops.get_data = ops->get_data;

	return 0;
}
EXPORT_SYMBOL(register_scp_mbrain_tmon_ops);

void unregister_scp_mbrain_tmon_ops(void)
{
	_scp_res_mbrain_tmon_ops.get_length = NULL;
	_scp_res_mbrain_tmon_ops.get_data = NULL;
}
EXPORT_SYMBOL(unregister_scp_mbrain_tmon_ops);
