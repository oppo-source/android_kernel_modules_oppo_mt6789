...
#include "s5khp3mipiraw_Sensor.h"
...
#define LOG_TAG "s5khp3"
#define S5KHP3_LOG_INF(format, args...) pr_info(LOG_TAG "[%s] " format, __func__, ##args)
#define S5KHP3_LOG_DBG(format, args...) pr_debug(LOG_TAG "[%s] " format, __func__, ##args)
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KHP3_SENSOR_ID,		// record sensor id defined in Kd_imgsensor.h
	...
	.margin = 34,                //sensor framelength & shutter margin
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {
	...
	0,    6,  4080, 3060,    0,    0, 4080, 3060}, // Preview 12.5M 30fps
	{ 16320,12288, 4080, 3084,   8160, 6120,  8160, 6120,
	0,    0,  8160, 6120,    0,    0, 8160, 6120}, // capture 12.5M 30fps
	{ 16320,12288,    0, 1536,  16320, 9216,  4080, 2304,
	0,    4,  4080, 2296,    0,    0, 4080, 2296}, // normal video 4080x2296 30fps
	{ 16320,12288,  480, 1792,  15360, 8704,  1920, 1088,
	0,    4,  1920, 1080,    0,    0, 1920, 1080}, // high speed video 60fps
	{ 16320,12288,    0, 1536,  16320, 9216,  4080, 2304,
	0,    4,  4080, 2296,    0,    0, 4080, 2296}, // slim video same as preview
	{ 16320,12288,    0,   24,  16320,12240, 16320,12240,
	0,    0, 16320,12240,    0,    0,16320,12240}, // 200,
};
...
static struct SENSOR_VC_INFO2_STRUCT SENSOR_VC_INFO2[] = {
	...
	{
		0x01, 0x0a, 0x00, 0x08, 0x40, 0x00, // 200
		{
			{VC_STAGGER_NE, 0x00, 0x2b, 0x3fc0, 0x2fd0}, // 16320*12288
		},
		1
	},
};
...
static kal_uint32 return_sensor_id(void)
{
	...
	sensor_id = ((read_cmos_sensor(SENSOR_ID_ADDR) << 8) |
		read_cmos_sensor(SENSOR_ID_ADDR + 1));
	sensor_version = ((read_cmos_sensor(0x000f)<<8) | read_cmos_sensor(0x0010));

	vendor_id = get_vendor_id();
	if(vendor_id != VENDOR_ID) {
		S5KHP3_LOG_INF("not match vendor id:0x%X\n",vendor_id);
		//sensor_id = 0xFFFF;
	}

	S5KHP3_LOG_INF("vendor_id: 0x%X, sensor_id: 0x%X, sensor_version:0x%X\n",
		vendor_id, sensor_id, sensor_version);
	/* N6 code for HQ-306077 by HQ camera at 20230713 end */
	return sensor_id;
}
...
{
	S5KHP3_LOG_DBG("dummyline = %d, dummypixels = %d\n",
...
static void set_max_framerate(UINT16 framerate, kal_bool min_framelength_en)
{
	...
	S5KHP3_LOG_DBG("framerate = %d, min framelength = %d, should enable?\n",
...
static void write_shutter(kal_uint32 shutter)
{
	...
	if(shutter >= 68900) {
		...
		kal_uint16 new_framelength = 0;
		kal_uint16 long_shutter = 0;

		bIsLongExposure = KAL_TRUE;
		exposure_time = shutter * imgsensor.line_length /
				(imgsensor_info.pre.pclk / 1000 - 1000);
		long_shutter = shutter / 128;
		LOG_INF("enter lvy long exposure mode long_shutter = %d", long_shutter);
		LOG_INF("long_exposure_time = %u ms\n",  exposure_time);
	...
	S5KHP3_LOG_INF("Exit! shutter = %d, framelength = %d, Reg(0x0101) = 0x%X\n",
		shutter, imgsensor.frame_length, read_cmos_sensor(0x0101));
	spin_unlock(&imgsensor_drv_lock);
}
			...
			kal_uint16 frame_length, kal_bool auto_extend_en)
{
	...
	if (imgsensor.autoflicker_en) {
		...
		if (realtime_fps >= 297 && realtime_fps <= 305)
			...
			write_cmos_sensor(FRAME_LENGTH_LINES_ADDR,
	...
	write_cmos_sensor(FRAME_LENGTH_LINES_ADDR,
		(imgsensor.frame_length >> Rshift) & 0xFFFF);
	...
	S5KHP3_LOG_DBG("set shutter = %d, framelength = %d\n",
...
static void set_awb_gain(struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
	...
	S5KHP3_LOG_INF("write awb gain(r,g,b): %3d, %3d,% 3d\n", r_Gain, g_Gain, b_Gain);
}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint32 reg_dig_gain = 0;
	kal_uint16 max_gain = imgsensor_info.max_gain = 0;

	S5KHP3_LOG_INF("E: gain: %d\n", gain);
	if (gain < imgsensor_info.min_gain || gain > imgsensor_info.max_gain) {
		S5KHP3_LOG_INF("Error gain setting");
	...
	S5KHP3_LOG_DBG("gain = %d, reg_gain = 0x%X\n", gain, reg_gain);
...
{
	S5KHP3_LOG_DBG("le:0x%X, se:0x%X, gain:0x%X\n", le, se, gain);
	spin_lock(&imgsensor_drv_lock);
	if (imgsensor.ihdr_en) {
		...
		write_cmos_sensor(0x3500, (le >> 12) & 0xFF);
...
static kal_uint32 streaming_control(struct imgsensor_struct imgsensor,kal_bool enable)
{
	...
	S5KHP3_LOG_INF("streaming_enable(0 = sw standby,1 = streaming): %d\n",enable);
	if (enable) {
		...
		for(i = 0; i < timeout; i++) {
			...
			if(framecnt == 0xFF) {
				S5KHP3_LOG_INF("stream off at i = %d\n",i);
				return ERROR_NONE;
			}
		}
		S5KHP3_LOG_INF("stream off fail! framecnt = %d\n",framecnt);
	}
	return ERROR_NONE;
}
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	S5KHP3_LOG_INF("image_mirror = %d, itemp =0x%X\n", image_mirror, itemp);
	itemp &= ~0x03;
...
static void sensor_init(void)
{
	...
	// Global
	S5KHP3_LOG_INF("loading global setting+\n");
	table_write_cmos_sensor(s5khp3_init_setting_global,
			sizeof(s5khp3_init_setting_global) / sizeof(kal_uint16));
	S5KHP3_LOG_INF("loading global setting-\n");
	S5KHP3_LOG_INF("lcy test\n");
	set_mirror_flip(imgsensor.mirror); // mirror flip
} /* sensor_init */

static void preview_setting(void)
{
	S5KHP3_LOG_INF("+\n");
	table_write_cmos_sensor(s5khp3_preview_setting,
			sizeof(s5khp3_preview_setting) / sizeof(kal_uint16));
	S5KHP3_LOG_INF("-\n");
	set_mirror_flip(imgsensor.mirror);  //mirror flip
} /* preview_setting */
...
{
	S5KHP3_LOG_INF("capture setting E\n");
	table_write_cmos_sensor(s5khp3_capture_setting,
			sizeof(s5khp3_capture_setting) / sizeof(kal_uint16));
	set_mirror_flip(imgsensor.mirror); // mirror flip
}   /* capture_setting */
...
{
	S5KHP3_LOG_INF("E!\n");
	table_write_cmos_sensor(s5khp3_normal_video_setting,
			sizeof(s5khp3_normal_video_setting) / sizeof(kal_uint16));
	set_mirror_flip(imgsensor.mirror);  //mirror flip
}
...
{
	S5KHP3_LOG_INF("E\n");
	table_write_cmos_sensor(s5khp3_hs_video_setting,
		sizeof(s5khp3_hs_video_setting) / sizeof(kal_uint16));
	set_mirror_flip(imgsensor.mirror);  //mirror flip
}
...
{
	S5KHP3_LOG_INF("E\n");
	table_write_cmos_sensor(s5khp3_slim_video_setting,
		sizeof(s5khp3_slim_video_setting) / sizeof(kal_uint16));
	set_mirror_flip(imgsensor.mirror);  //mirror flip
}
...
{
	S5KHP3_LOG_INF("E\n");
	table_write_cmos_sensor(s5khp3_custom1_setting,
		sizeof(s5khp3_custom1_setting) / sizeof(kal_uint16));
	set_mirror_flip(imgsensor.mirror);  //mirror flip
}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				S5KHP3_LOG_INF("Read sensor id success\n");
				S5KHP3_LOG_INF("i2c write id: 0x%X, sensor id: 0x%X\n",
						imgsensor.i2c_write_id, *sensor_id);
				//read_QSC_data();
				return ERROR_NONE;
			}
			S5KHP3_LOG_INF("Read sensor id fail\n");
			S5KHP3_LOG_INF("i2c write id: 0x%X, id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	S5KHP3_LOG_INF("PLATFORM:MT6877, MIPI 3LANE\n");
	while (imgsensor_info.i2c_addr_table[i] != 0xFF) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		spin_unlock(&imgsensor_drv_lock);
		do {
			...
			S5KHP3_LOG_INF("Read sensor id fail, i2c write id: 0x%X, id: 0x%X\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
{
	S5KHP3_LOG_INF("loading preview_setting\n");
...
static kal_uint32 capture(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3_LOG_INF("loading capture_setting\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 normal_video(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3_LOG_INF("loading normal_video_setting\n");
...
static kal_uint32 hs_video(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3_LOG_INF("loading hs_video_setting\n");
...
static kal_uint32 custom1(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3_LOG_INF("loading custom1_setting\n");
				...
				MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case MSDK_SCENARIO_ID_CUSTOM1:
		...
		break;
	default:
		S5KHP3_LOG_INF("Error ScenarioId setting");
		preview(image_window, sensor_config_data);
				...
				kal_uint32 shutter_2ndframe, kal_uint32 gain_2ndframe)
{
	...
	S5KHP3_LOG_INF("zbd 3333333333333333333333 seamless switch to %d!\n",scenario_id);
	switch (scenario_id) {
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
	{
		S5KHP3_LOG_INF("zbd 44444444444444444 seamless switch to preview!\n");
		spin_lock(&imgsensor_drv_lock);
		...
		//imgsensor.extend_frame_length_en = KAL_FALSE;
		spin_unlock(&imgsensor_drv_lock);
		S5KHP3_LOG_INF("zbd 55555555555555555555 seamless switch 1-exp!\n");
		preview_setting();
		// JD1 is in AE LOCK stage for development.
		if (gain != 0)
			write_cmos_sensor(AGAIN_ADDR, gain);
		if (shutter != 0)
			write_cmos_sensor(0X0202, shutter);
	}
		break;
	default:
		S5KHP3_LOG_INF("error! wrong setting in set_seamless_switch = %d\n",
				scenario_id);
		return 0xff;
	}
	imgsensor.fast_mode_on = KAL_TRUE;
	S5KHP3_LOG_INF("%s success, scenario is switched to %d\n", __func__, scenario_id);
	return 0;
}

static kal_uint32 set_video_mode(UINT16 framerate)
{
	S5KHP3_LOG_DBG("framerate = %d\n", framerate);
...
{
	S5KHP3_LOG_DBG("enable = %d, framerate = %d\n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
				...
				enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	S5KHP3_LOG_DBG("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	switch (scenario_id) {
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		spin_lock(&imgsensor_drv_lock);
		...
		if (imgsensor.frame_length > imgsensor.shutter)
			...
			/*No need to set*/
			S5KHP3_LOG_DBG("frame_length %d < shutter %d",
	...
	default: // coding with  preview scenario by default
		...
		if (imgsensor.frame_length > imgsensor.shutter)
			...
			/*No need to set*/
			S5KHP3_LOG_INF("frame_length %d < shutter %d",
				imgsensor.frame_length, imgsensor.shutter);
		S5KHP3_LOG_INF("error scenario_id = %d, we use preview scenario\n",
				scenario_id);
		spin_unlock(&imgsensor_drv_lock);
		break;
	}
		...
		UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		S5KHP3_LOG_INF("current fps :%d\n", (UINT32)*feature_data);
		spin_lock(&imgsensor_drv_lock);
		imgsensor.current_fps = *feature_data;
		spin_unlock(&imgsensor_drv_lock);
		break;
	case SENSOR_FEATURE_SET_HDR:
		S5KHP3_LOG_INF("ihdr enable :%d\n", (BOOL)*feature_data);
		spin_lock(&imgsensor_drv_lock);
		imgsensor.ihdr_en = (BOOL)*feature_data;
		spin_unlock(&imgsensor_drv_lock);
		break;
	case SENSOR_FEATURE_GET_CROP_INFO:
		S5KHP3_LOG_INF("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%d\n",
				(UINT32)*feature_data);
		spin_lock(&imgsensor_drv_lock);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		spin_lock(&imgsensor_drv_lock);
		S5KHP3_LOG_INF("SENSOR_SET_SENSOR_IHDR LE = %d, SE = %d, Gain = %d\n",
				(UINT16)*feature_data,
	...
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		S5KHP3_LOG_INF("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		spin_lock(&imgsensor_drv_lock);
		streaming_control(imgsensor,KAL_FALSE);
		spin_unlock(&imgsensor_drv_lock);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		S5KHP3_LOG_INF("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter: %llu\n",
			*feature_data);
		spin_lock(&imgsensor_drv_lock);
		if (*feature_data != 0)
			set_shutter(*feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		S5KHP3_LOG_INF("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
	...
	case SENSOR_FEATURE_GET_AE_EFFECTIVE_FRAME_FOR_LE:
		...
		*feature_return_para_32 = imgsensor.current_ae_effective_frame;
		S5KHP3_LOG_INF("GET AE EFFECTIVE %d\n", *feature_return_para_32);
		spin_unlock(&imgsensor_drv_lock);
		break;
	case SENSOR_FEATURE_GET_AE_FRAME_MODE_FOR_LE:
		...
		S5KHP3_LOG_INF("GET_AE_FRAME_MODE");
		spin_unlock(&imgsensor_drv_lock);
		break;
	...
	case SENSOR_FEATURE_GET_VC_INFO2:
		spin_lock(&imgsensor_drv_lock);
		S5KHP3_LOG_INF("SENSOR_FEATURE_GET_VC_INFO %d\n",
			(UINT16) *feature_data);
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		S5KHP3_LOG_INF("odin GET_PDAF_DATA EEPROM\n");
		break;
	case SENSOR_FEATURE_GET_PDAF_INFO:
		spin_lock(&imgsensor_drv_lock);
		S5KHP3_LOG_INF("SENSOR_FEATURE_GET_PDAF_INFO scenarioId:%d\n",
			(UINT16) *feature_data);
...
/* N6 code for HQHW-4840 by huabinchen at 20230810 end */
	...
	case SENSOR_FEATURE_SET_PDAF:
		...
		S5KHP3_LOG_INF("[odin] pdaf mode : %d\n", imgsensor.pdaf_mode);
		spin_unlock(&imgsensor_drv_lock);
		break;
	...
	case SENSOR_FEATURE_SEAMLESS_SWITCH:
		spin_lock(&imgsensor_drv_lock);
		S5KHP3_LOG_INF("zbd 888888888888888888888 SENSOR_FEATURE_SEAMLESS_SWITCH");
		fpsOV = imgsensor_info.custom5.pclk /
			imgsensor.frame_length /
		...
		S5KHP3_LOG_INF("zbd 9999999999999999999999 call seamless_switch");
		if (pAeCtrls != NULL) {
			S5KHP3_LOG_INF("zbd 1212121212121212121212\n");
			S5KHP3_LOG_INF("aectrlcall seamless_switch_arctrl\n");
			seamless_switch((*feature_data),*pAeCtrls, *(pAeCtrls + 1),
					*(pAeCtrls + 4), *(pAeCtrls + 5));
		} else {
			S5KHP3_LOG_INF("zbd 10101010101010101010 call seamless_switch_null");
	...
	case SENSOR_FEATURE_GET_SEAMLESS_SCENARIOS:
		...
		if ((feature_data + 1) != NULL) {
			...
			S5KHP3_LOG_INF("zbd 1616161616161616161616");
			S5KHP3_LOG_INF("feature_data(%llu)", *feature_data);
			S5KHP3_LOG_INF("*pScenarios(%d)", *pScenarios);
		} else {
			S5KHP3_LOG_INF("input pScenarios vector is NULL!\n");
			return ERROR_INVALID_SCENARIO_ID;
		}
		switch (*feature_data) {
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
			...
			*(pScenarios + 1) = MSDK_SCENARIO_ID_CUSTOM3; // 7
			S5KHP3_LOG_INF("zbd 131313131313131313\n");
			S5KHP3_LOG_INF("MSDK_SCENARIO_ID_CAMERA_PREVIEW to\n");
			S5KHP3_LOG_INF("MSDK_SCENARIO_ID_CUSTOM5 + MSDK_SCENARIO_ID_CUSTOM3");
			break;
		default:
			*pScenarios = 0xff;
			break;
		}
		S5KHP3_LOG_INF("zbd 151515151515151515\n");
		S5KHP3_LOG_INF("SENSOR_FEATURE_GET_SEAMLESS_SCENARIOS %llu %d\n",
