...
#include "ov08f10mipiraw_cruiserl4_Sensor.h"

#define PFX "OV08F10_camera_sensor"
...
#define OV08F10_SENSOR_ID 0x5608471c
#define MULTI_WRITE 0

static kal_uint8 otp_data[LEFT_SIZE] = {0};
static BYTE ov08f10_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV08F10_SENSOR_ID_CRUISERL4,
			 ...
			 kal_uint16 frame_length, kal_bool auto_extend_en)
{
	...
	printk("OV08F10_set_shutter_frame_length() shutter =%d, frame_length = %d, dummy_line = %d, auto_extend_en = %d, imgsensor.frame_length =%d\n",
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if((gain >= 0x40) && (gain <= (15.5*0x40))) //base gain = 0x40
	{
		...
		if(iReg<=0x10)
		{
			...
			write_cmos_sensor(0xfd, 0x01);	//page1
			write_cmos_sensor(0x01, 0x01);	//fresh
			LOG_INF("OV08F10MIPI_SetGain = 16");
		}
		else if(iReg>= 0xf8)//gpw
		{
			...
			write_cmos_sensor(0xfd, 0x01);	//page1
			write_cmos_sensor(0x01, 0x01);	//fresh
			LOG_INF("OV08F10MIPI_SetGain = 160");
		}
		else
		{
			write_cmos_sensor(0xfd, 0x01);
			...
			write_cmos_sensor(0xfd, 0x01);	//page1
			write_cmos_sensor(0x01, 0x01);	//fresh
			LOG_INF("OV08F10MIPI_SetGain = %d",iReg);
		}
	}
	else
...
static kal_uint16 ov08f10_init_setting[] = {
	0xfd, 0x00,
};

static kal_uint16 ov08f10_preview_setting[] = {
	0x20, 0x0b,
	0xfd, 0x00,
	0xc2, 0x32,
	0x21, 0x07,
...
static kal_uint16 ov08f10_capture_setting[] = {
	0x20, 0x0b,
	0xfd, 0x00,
	0xc2, 0x32,
	0x21, 0x07,
...
static kal_uint16 ov08f10_normal_video_setting[] = {
	0x20, 0x0b,
	0xfd, 0x00,
	0xc2, 0x32,
	0x21, 0x07,
...
static kal_uint16 ov08f10_slim_video_setting[] = {
	0x20, 0x0b,
	0xfd, 0x00,
	0xc2, 0x32,
	0x21, 0x07,
...
static kal_uint16 ov08f10_hs_video_setting[] = {
	0x20, 0x0b,
	0xfd, 0x00,
	0xc2, 0x32,
	0x21, 0x07,
...
static kal_uint16 ov08f10_custom1_setting[] = {
	0x20, 0x0b,
	0xfd, 0x00,
	0xc2, 0x32,
	0x21, 0x07,
...
static kal_uint16 ov08f10_custom2_setting[] = {
	0x20, 0x0b,
	0xfd, 0x00,
	0xc2, 0x32,
	0x21, 0x07,
...
{
	table_write_cmos_sensor(ov08f10_init_setting,
		sizeof(ov08f10_init_setting) / sizeof(kal_uint16));
}
...
static void preview_setting(void)
{
	...
	mdelay(3);
	table_write_cmos_sensor(ov08f10_preview_setting,
		sizeof(ov08f10_preview_setting) / sizeof(kal_uint16));
}
...
static void capture_setting(void)
{
	...
	write_cmos_sensor(0x20, 0x0e);
	mdelay(3);
	table_write_cmos_sensor(ov08f10_capture_setting,
		sizeof(ov08f10_capture_setting) / sizeof(kal_uint16));
}
...
static void normal_video_setting(void)
{
	...
	write_cmos_sensor(0x20, 0x0e);
	mdelay(3);
	table_write_cmos_sensor(ov08f10_normal_video_setting,
		sizeof(ov08f10_normal_video_setting) / sizeof(kal_uint16));
}
...
static void slim_video_setting(void)
{
	...
	write_cmos_sensor(0x20, 0x0e);
	mdelay(3);
	table_write_cmos_sensor(ov08f10_slim_video_setting,
		sizeof(ov08f10_slim_video_setting) / sizeof(kal_uint16));
}
...
{	write_cmos_sensor(0xfd, 0x00);
	write_cmos_sensor(0x20, 0x0e);
	mdelay(3);
	table_write_cmos_sensor(ov08f10_hs_video_setting,
		sizeof(ov08f10_hs_video_setting) / sizeof(kal_uint16));
}
...
static void custom1_setting(void)
{
	...
	write_cmos_sensor(0x20, 0x0e);
	mdelay(3);
	table_write_cmos_sensor(ov08f10_custom1_setting,
		sizeof(ov08f10_custom1_setting) / sizeof(kal_uint16));
}
static void custom2_setting(void)
{
	...
	write_cmos_sensor(0x20, 0x0e);
	mdelay(3);
	table_write_cmos_sensor(ov08f10_custom2_setting,
		sizeof(ov08f10_custom2_setting) / sizeof(kal_uint16));
}
...
static kal_uint16 read_ov08f10_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, OV08F10_EEPROM_I2C_ADDR);

	return get_byte;
}
static void read_ov08f10_module_data(void)
{
// ov08f10_common_data
// length   : 64
// QR       : [8 ~ 26]
...
// sensorid : [30 ~ 33]
	...
	memset(ov08f10_common_data, 0,sizeof(ov08f10_common_data));
	// QR
	for(idx = 0; idx < sn_length; idx++)
	{
		ov08f10_common_data[8 + idx] = read_ov08f10_eeprom_module(sn_starAddr + idx);
	}
	//vcm
	ov08f10_common_data[40] = read_ov08f10_eeprom_module(vcmAddr);
	ov08f10_common_data[41] = read_ov08f10_eeprom_module(vcmAddr + 1);
	//lensid
	ov08f10_common_data[44] = read_ov08f10_eeprom_module(lensAddr);
	ov08f10_common_data[45] = read_ov08f10_eeprom_module(lensAddr + 1);

	for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
		LOG_INF("ov08f10 common cam data: %02x %02x %02x %02x\n",
				ov08f10_common_data[idx],
				ov08f10_common_data[idx + 1],
				ov08f10_common_data[idx + 2],
				ov08f10_common_data[idx + 3]);
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		LOG_INF("cruiserl4_WIDE_OV08F10 get_imgsensor_id : 0x%x\n", *sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_info("[get_imgsensor_id] ov08f10 Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				if (deviceInfo_register_value == 0x00) {
					read_ov08f10_module_data();
			...
			pr_info("[get_imgsensor_id] ov08f10 Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		LOG_ERR("cruiserl4_OV08F10 get_imgsensor_id failed: 0x%x\n", *sensor_id);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_info("[Open] ov08f10 Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_info("[Open] ov08f10 Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
...
{
	pr_info("[ov08f10] preview mode start\n");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[ov08f10] capture mode start\n");
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[ov08f10] normal_video mode start\n");
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[ov08f10] hs_video mode start\n");
...
static kal_uint32 slim_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[ov08f10] slim_video mode start\n");
...
static kal_uint32 custom1(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[ov08f10] custom1 mode start\n");
...
static kal_uint32 custom2(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[ov08f10] custom2 mode start\n");
		...
		UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	/*case SENSOR_FEATURE_GET_SERIANO_IC:
		read_ov08f10_QRSerialdata(feature_data_8, feature_para_len);
	...
	case SENSOR_FEATURE_GET_EEPROM_COMDATA:
		memcpy(feature_return_para_32, ov08f10_common_data,
...
UINT32 OV08F10_MIPI_RAW_CRUISERL4_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
