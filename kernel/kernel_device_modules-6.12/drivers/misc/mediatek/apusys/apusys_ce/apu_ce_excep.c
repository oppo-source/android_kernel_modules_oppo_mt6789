...
uint32_t apu_ce_reg_dump(struct device *dev)
{
	...
	else if (g_apu_ce_ver == APU_CE_VER_2)
		apu_ce_reg_dump_v2(dev);
...
int apu_ce_excep_init(struct platform_device *pdev, struct mtk_apu *apu)
{
	...
	} else if (apu_ce_excep_is_compatible_v2(pdev)) {
		g_apu_ce_ver = APU_CE_VER_2;
		return apu_ce_excep_init_v2(pdev, apu);
	}
...
int is_apu_ce_excep_init(void)
{
	if (g_apu_ce_ver == APU_CE_VER_1)
		...
		return is_apu_ce_excep_init_v2();
...
void apu_ce_mrdump_register(struct mtk_apu *apu)
{
	if (g_apu_ce_ver == APU_CE_VER_1)
		...
		return apu_ce_mrdump_register_v2(apu);
}
	...
	struct proc_dir_entry *procfs_root)
{
	if (g_apu_ce_ver == APU_CE_VER_1)
		...
		return apu_ce_procfs_init_v2(pdev, procfs_root);
}
	...
	struct proc_dir_entry *procfs_root)
{
	if (g_apu_ce_ver == APU_CE_VER_1)
		...
		return apu_ce_procfs_remove_v2(pdev, procfs_root);
}
