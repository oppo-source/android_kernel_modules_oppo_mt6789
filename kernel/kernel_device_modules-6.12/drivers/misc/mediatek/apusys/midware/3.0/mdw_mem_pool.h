...
int mdw_mem_pool_create(struct mdw_fpriv *mpriv, struct mdw_mem_pool *pool,
	enum mdw_mem_type type, enum mdw_buf_type buf_type, uint32_t size, uint32_t align, uint64_t flags);
...
struct mdw_mem_map *mdw_mem_pool_alloc(struct mdw_mem_pool *pool, uint64_t size,
	uint32_t align);
void mdw_mem_pool_free(struct mdw_mem_map *map);

int mdw_mem_pool_flush(struct mdw_mem_map *map);
int mdw_mem_pool_invalidate(struct mdw_mem_map *map);
