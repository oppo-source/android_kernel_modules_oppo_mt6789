...
static int mdw_rsc_req_add_dev(struct mdw_dev_info *d, struct mdw_rsc_req *req)
{
	...
	mdw_flw_debug("put dev(%s%d) to req(%pK), ref(%d)\n",
...
int mdw_rsc_get_dev(struct mdw_rsc_req *req)
{
	...
	mdw_flw_debug("req(%pK) bmp(0x%llx) total(%d) mode(%d) policy(%d)\n",
	...
	while (1) {
		...
		for (i = 0; i < n; i++) {
			...
			if (mdw_rsc_req_add_dev(d, req))
				mdw_drv_err("req(%pK) add dev(%s%d) fail\n",
					req, d->name, d->idx);
		}
		mutex_unlock(&tab->mtx);
