...
static inline _Bool list_in_list(struct list_node *item)
{
...
static inline void list_add_tail_new(struct list_node *list, struct list_node *item)
{
	item->prev = list->prev;
	item->next = list;
	list->prev->next = item;
...
#define list_add_before(entry, new_entry) list_add_tail_new(entry, new_entry)
...
static inline _Bool list_is_empty(struct list_node *list)
{
