...
{
	int ret = 0;
	int i, j, render_num = 0, dep_num = 0;
	...
	if (!tmp_render_arr || !tmp_dep_arr)
		goto out;

	fpsgo_other2comp_get_render_fw_info(0, FPSGO_MAX_RENDER_INFO_SIZE, &render_num, tmp_render_arr);
	for (i = 0; i < render_num; i++) {
		dep_num = fpsgo_other2xgf_get_critical_tasks(tmp_render_arr[i].producer_pid, MAX_DEP_NUM,
				tmp_dep_arr, 0, tmp_render_arr[i].buffer_id);
		for (j = 0; j < dep_num; j++) {
			if (tmp_dep_arr[j].pid == mr->tid) {
				ret = 1;
				goto out;
			}
		}
