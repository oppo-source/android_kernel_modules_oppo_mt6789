...
#define SBE_SYSFS_DIR_NAME "sbe"

static struct kobject *sbe_kobj;
		...
		const char *name, struct kobject **ppsKobj)
{
	...
	parent = (parent != NULL) ? parent : sbe_kobj;
...
{
	sbe_kobj = kobject_create_and_add(SBE_SYSFS_DIR_NAME,
			kernel_kobj);
	if (!sbe_kobj) {
