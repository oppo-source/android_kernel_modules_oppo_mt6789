...
#include <usb_offload/usb_offload.h>
...
void usb_uo_info(struct uo_mbrain data)
{
	unsigned int vid = data.vid;
	unsigned int pid = data.pid;
...
int mbraink_usb_init(void)
{
	...
	ret = register_uo_mbrain_cb(usb_uo_info);
	return ret;
}
