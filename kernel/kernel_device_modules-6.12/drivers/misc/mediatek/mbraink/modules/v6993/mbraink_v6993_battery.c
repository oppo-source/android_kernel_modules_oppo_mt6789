...
static void mbraink_v6993_get_battery_info(struct mbraink_battery_data *battery_buffer,
			      long long timestamp)
{
...
static struct mbraink_battery_ops mbraink_v6993_battery_ops = {
	.getBatteryInfo = mbraink_v6993_get_battery_info,
};

int mbraink_v6993_battery_init(struct device *dev)
{
	int ret = 0;
	...
	ret = register_mbraink_battery_ops(&mbraink_v6993_battery_ops);
	return ret;
}

int mbraink_v6993_battery_deinit(void)
{
	int ret = 0;
