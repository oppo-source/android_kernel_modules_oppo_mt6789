...
static int mbraink_v6993_set_camera_hw_info(struct ht_mbrain ht_data_to_mbrain)
{
...
static struct bridge2platform_ops mbraink_bridge_v6993_camera_ops = {
	.set_data = mbraink_v6993_set_camera_hw_info,
};

int mbraink_v6993_camera_init(void)
{
	int ret = 0;

	ret = register_mbraink_bridge_platform_camera_ops(&mbraink_bridge_v6993_camera_ops);
	if (ret != 0) {
...
int mbraink_v6993_camera_deinit(void)
{
	int ret = 0;
