...
static const char *const eusb2_params_name[] = {
	"usb20-fs-sr",
	"eusb20-fsrx-hys-sel",
	"vrt-sel",
	"discth",
...
static int proc_oplus_eusb2_params_show(struct seq_file *s, void *unused)
{
	...
	for (index = 0; index < params_update_num; index++) {
		...
		if (index == params_update_num - 1)
			seq_printf(s, "%s:0x%02x", eusb2_params_name[params_id_seq[index]], params_val[params_id_seq[index]]);
		else
			seq_printf(s, "%s:0x%02x,", eusb2_params_name[params_id_seq[index]], params_val[params_id_seq[index]]);
	}
...
static int oplus_eusb2_params_parse(u32 *params_seq, int* params_val, u32 cnt, struct eusb2_repeater *rptr)
{
	...
	for (index = 0, seq_i = 0; index < cnt && seq_i < params_update_num; index = index + 2, ++seq_i) {
		...
		dev_info(rptr->dev, "update eusb2 params:%s=0x%x\n", eusb2_params_name[params_seq[index]], params_seq[index + 1]);
	}
...
static void oplus_check_params(int* params_val, struct eusb2_repeater *rptr)
{
	...
	for (id = 0; id < EUSB2_MAX_PARAMS_NUM; ++id) {
		/*
		 * Only treat -EINVAL as "unconfigured" for non-offset params.
		 * For intr_ofs/term_ofs, the unconfigured sentinel is -(MASK + 1).
		 */
		if ((id != OPLUS_INTR_OFS && id != OPLUS_TERM_OFS && def_params_val[id] == -EINVAL)
			|| (id == OPLUS_INTR_OFS && def_params_val[id] == -(RG_USB20_INTR_CAL_MASK + 1))
			|| (id == OPLUS_TERM_OFS && def_params_val[id] == -(RG_USB20_TERM_CAL_MASK + 1))) {
			dev_info(rptr->dev, "param: %s is not configured in dtsi, reset default\n", eusb2_params_name[id]);
			params_val[id] = def_params_val[id];
		}

		/*
		 * Validate ranges for intr_ofs and term_ofs against current cal values.
		 * Allow negative offsets, but require (cal + ofs) within [0, MASK].
		 * If out of legal range, reset to default and log for visibility.
		 */
		if (id == OPLUS_INTR_OFS && def_params_val[id] != -(RG_USB20_INTR_CAL_MASK + 1)) {
			if (rptr->intr_cal != -EINVAL) {
				new_val = rptr->intr_cal + params_val[id];
				if (new_val < 0 || new_val > RG_USB20_INTR_CAL_MASK) {
					dev_info(rptr->dev, "invalid %s=%d with intr_cal=%d, reset to default\n",
					eusb2_params_name[id], params_val[id], rptr->intr_cal);
					params_val[id] = def_params_val[id];
				}
			}
		}

		if (id == OPLUS_TERM_OFS && def_params_val[id] != -(RG_USB20_TERM_CAL_MASK + 1)) {
			if (rptr->term_cal != -EINVAL) {
				new_val = rptr->term_cal + params_val[id];
				if (new_val < 0 || new_val > RG_USB20_TERM_CAL_MASK) {
					dev_info(rptr->dev, "invalid %s=%d with term_cal=%d, reset to default\n",
					eusb2_params_name[id], params_val[id], rptr->term_cal);
					params_val[id] = def_params_val[id];
				}
			}
		}
...
static int oplus_eusb2_params_init(struct eusb2_repeater *rptr, int *params_val)
{
	...
	for (index = 0; index < EUSB2_MAX_PARAMS_NUM; index++) {
		/*
		 * Allow negative values for intr_ofs/term_ofs so that sign-aware
		 * offsets can be applied later with existing mask constraints.
		 * For other parameters, a negative value still means "not configured".
		 */
		if (params_val[index] < 0 && index != OPLUS_INTR_OFS && index != OPLUS_TERM_OFS)
			continue;
	...
	const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	while (*str != '\0' && *str != '\n') {
		...
		for (id = 0; id < EUSB2_MAX_PARAMS_NUM; id++) {
			if (!strncmp(eusb2_params_name[id], val_str, strlen(eusb2_params_name[id]))) {
