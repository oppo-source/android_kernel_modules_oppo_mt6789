...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* oplus add for pd become usb port */
	if (!ddata->bc12_dn[idx] || attach == ATTACH_TYPE_PD_DCP)
#else
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* oplus add for pd become usb port */
	if ((attach > ATTACH_TYPE_PD && ddata->bc12_dn[idx] && !ddata->bc12_retry) ||
...
out:
#ifdef CONFIG_OPLUS_HVDCP_SUPPORT
	if (!atomic_read(&ddata->attach[active_idx])) {
		cancel_delayed_work_sync(&ddata->hvdcp_work);
		cancel_delayed_work_sync(&ddata->hvdcp_result_check_work);
		ddata->hvdcp_type = POWER_SUPPLY_TYPE_UNKNOWN;
...
/*#ifdef OPLUS_FEATURE_CHG_BASIC*/
	case POWER_SUPPLY_PROP_TYPE:
		...
		return 0;
	case POWER_SUPPLY_PROP_USB_TYPE:
		ddata->psy_usb_type[active_idx] = val->intval;
		return 0;
	case POWER_SUPPLY_PROP_POWER_NOW:
		atomic_set(&ddata->attach[active_idx], val->intval);
		return 0;
	case POWER_SUPPLY_PROP_ONLINE:
