...
int mt_leds_parse_dt(struct mt_led_data *mdev, struct fwnode_handle *fwnode)
{
	...
	if (!ret) {
		if (!strncmp(state, "enable", strlen("enable")))
			mBrightness2Enable = true;
		else
			mBrightness2Enable = false;
	} else {
		mBrightness2Enable = false;
	}
	pr_info("parse mBrightness2Enable: %d\n", mBrightness2Enable);
