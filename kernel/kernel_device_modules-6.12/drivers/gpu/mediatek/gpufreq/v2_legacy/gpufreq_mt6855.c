...
//static void gpu_set_rgx_bus_secure(void);
...
/* dvfs function */
static int __gpufreq_generic_scale_gpu(unsigned int freq_old, unsigned int freq_new,
...
static unsigned int __gpufreq_get_aging_table_idx(u32 a_t0_efuse1, u32 a_t0_efuse2,
						  u32 a_tn_sensor1, u32 a_tn_sensor2,
						  u32 efuse_error,
...
int __gpufreq_power_control(enum gpufreq_power_state power)
{
	...
	if (power == GPU_PWR_ON && g_gpu.power_count == 1) {
		...
		__gpufreq_aoc_control(GPU_PWR_ON);
		...
		__gpufreq_aoc_control(GPU_PWR_OFF);
...
int __gpufreq_generic_commit_gpu(int target_oppidx, enum gpufreq_dvfs_state key)
{
	...
	GPUFREQ_LOGD("begin to commit GPU OPP index: (%d->%d)", cur_oppidx_gpu, target_oppidx_gpu);

	ret = __gpufreq_generic_scale_gpu(cur_fgpu, target_fgpu,
					  cur_vgpu, target_vgpu, cur_vsram_gpu, target_vsram_gpu);
	if (unlikely(ret)) {
		GPUFREQ_LOGE("fail to scale Idx(%d->%d), Freq(%d->%d), Volt(%d->%d), Vsram(%d->%d)",
...
int __gpufreq_fix_custom_freq_volt_gpu(unsigned int freq, unsigned int volt)
{
	...
	if (freq == 0 && volt == 0) {
		...
		if (unlikely(ret)) {
			GPUFREQ_LOGE("fail to commit GPU Freq: %d, Volt: %d (%d)", freq, volt, ret);
			__gpufreq_set_dvfs_state(false, DVFS_FIX_OPP);
		}
	}
				    ...
				    unsigned int vsram_old, unsigned int vsram_new)
{
	...
	if (unlikely(g_gpu.cur_volt != volt_new))
		__gpufreq_abort("inconsistent scaled Vgpu, cur_volt: %d, target_volt: %d, volt_old: %d",
				g_gpu.cur_volt, volt_new, volt_old);
...
static void __gpufreq_aoc_control(enum gpufreq_power_state power)
{
	...
	if (readl(g_sleep + 0x6AC) & 0x1)
		__gpufreq_abort("fail to release SPM_SEMA_M4: 0x%08x", readl(g_sleep + 0x6AC));
...
hw_semaphore_timeout:
	...
	__gpufreq_abort("acquire SPM_SEMA_M4 timeout");
}
...
static int __gpufreq_mtcmos_control(enum gpufreq_power_state power)
{
	...
	if (power == GPU_PWR_ON) {
		...
		if (unlikely(val != MFG_0_2_PWR_MASK)) {
			__gpufreq_abort("incorrect MFG0-2 power on status: 0x%08x", val);
			ret = GPUFREQ_EINVAL;
			goto done;
		}
...
static void __gpufreq_apply_adjust(struct gpufreq_adj_info *adj_table, int adj_num)
{
	...
	for (i = 0; i < adj_num; i++) {
		...
		GPUFREQ_LOGD("[%02d*] Freq: %d, Volt: %d, Vsram: %d, Vaging: %d",
			     ...
			     signed_table[oppidx].vsram, signed_table[oppidx].margin);
	}
...
static void __gpufreq_aging_adjustment(void)
{
	...
	if (__gpufreq_dvfs_enable()) {
		...
		is_efuse_read_success =
		    __gpufreq_asensor_read_efuse(&a_t0_efuse1, &a_t0_efuse2, &efuse_error);
		...
		aging_table_idx =
		    __gpufreq_get_aging_table_idx(a_t0_efuse1, a_t0_efuse2, a_tn_sensor1,
						  a_tn_sensor2, efuse_error, is_efuse_read_success);
	}

	g_asensor_info.aging_table_idx_agrresive = GPUFREQ_AGING_MOST_AGRRESIVE;
	g_asensor_info.aging_table_idx = aging_table_idx;

	GPUFREQ_LOGD("Aging Sensor table id: %d", aging_table_idx);
#endif				/* GPUFREQ_ASENSOR_ENABLE */
...
static int __gpufreq_init_opp_table(struct platform_device *pdev)
{
	...
	GPUFREQ_LOGD("preloader init [GPU] Freq: %d, Volt: %d, Vsram: %d",
...
static int __gpufreq_init_segment_id(struct platform_device *pdev)
{
	...
	GPUFREQ_LOGD("efuse_id: 0x%x, segment_id: %d", efuse_id, segment_id);
