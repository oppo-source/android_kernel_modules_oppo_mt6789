...
#include "../mediatek/mediatek_v2/v1/mtk_corner_pattern/ac242_p_1_data_hw_roundcornerpattern.h"
//#include "oplus_bl.h"
		...
		unsigned int id)
{
	...
	OPLUS_DSI_INFO("lcm mode:%d,mode_id:%d\n", id, mode_id);
	...
	if (*ext_param)
		OPLUS_DSI_DEBUG("lcm data_rate:%d\n", (*ext_param)->data_rate);
	else
		OPLUS_DSI_ERR("lcm ext_param is NULL;\n");
			 ...
			 struct drm_connector *connector, unsigned int mode)
{
	...
	if (!m) {
		OPLUS_DSI_ERR("lcm invalid drm_display_mode\n");
		return 1;
	}
	mode_id = get_mode_enum(m);
	OPLUS_DSI_INFO("lcm mode:%d,mode_id:%d\n", mode, mode_id);
				 ...
				 unsigned int level)
{
	...
	if (level == 0) {
		OPLUS_DSI_INFO("lcm [INFO][%s:%d]backlight lvl:%u, flag_hbm = %d\n", __func__, __LINE__, level, flag_hbm);
	...
	bl_tb0[2] = level & 0xFF;
	if (level == 1) {
		OPLUS_DSI_INFO("lcm [INFO][%s:%d]filter backlight %d setting\n", __func__, __LINE__, level);
		return 0;
	} else if (level <= 2047) {
		OPLUS_DSI_INFO("lcm [%s:%d]backlight =	%d setting, flag_hbm = %d\n", __func__, __LINE__, level, flag_hbm);
		if (flag_hbm == 1) {
			bl_tb1[1] = 0x20;
			cb(dsi, handle, bl_tb1, ARRAY_SIZE(bl_tb1));
			flag_hbm = 0;
...
static int oplus_esd_backlight_recovery(void *dsi, dcs_write_gce cb, void *handle)
{
	...
	if (esd_brightness == 1) {
		OPLUS_DSI_INFO("[INFO][%s:%d]filter backlight %d setting\n", __func__, __LINE__, esd_brightness);
		return 0;
	}  else if (esd_brightness <= 2047) {
		OPLUS_DSI_INFO("[%s:%d]esd_brightness =  %d setting\n", __func__, __LINE__, esd_brightness);
		if (flag_hbm == 1) {
			bl_tb1[1] = 0x20;
			cb(dsi, handle, bl_tb1, ARRAY_SIZE(bl_tb1));
			flag_hbm = 0;
	...
	OPLUS_DSI_INFO("[INFO][%s:%d]esd_brightness=%d\n", __func__, __LINE__, esd_brightness);
	return 0;
}
...
static int lcm_panel_poweroff(struct drm_panel *panel)
{
	...
	OPLUS_DSI_INFO("lcm lcm_panel_poweroff begin+\n");
...
static int lcm_get_modes(struct drm_panel *panel,
	...
	if (!mode[0]) {
		OPLUS_DSI_ERR("lcm failed to add mode %ux%ux@%u\n",
	...
	OPLUS_DSI_DEBUG("clock=%d,htotal=%d,vtotal=%d,hskew=%d,vrefresh=%d\n",mode[0]->clock, mode[0]->htotal,
...
static int panel_set_seed(void *dsi, dcs_write_gce cb, void *handle, unsigned int seed_mode)
{
	...
	OPLUS_DSI_INFO("debug for lcm %s, seed_mode=%d\n", __func__, seed_mode);
...
static struct mtk_panel_funcs ext_funcs = {
	...
	.set_seed_legacy = panel_set_seed,
...
/* #endif  *//* OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
	OPLUS_DSI_INFO("ac242_p_1_a0017_fhd_vdo lcm_probe\n");
...
/* #endif  *//* OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
	...
	OPLUS_DSI_INFO("lcm ac242_p_1_a0017_fhd_vdo-\n");
	return ret;
}
