...
#include "../mediatek/mediatek_v2/v1/oplus_drm_v1/oplus_display_temp_compensation.h"
...
//#include "oplus_drm_disp_panel.h"
//#include "../../../misc/mediatek/lpm/inc/lpm_module.h"
#include "k6895_pwr_ctrl.h"
//#include "../../../misc/mediatek/wl2868c/wl2868c.h"
#include "AC160_P_7_A0001_cmd_panel.h"

#include "../mediatek/mediatek_v2/v1/mtk_corner_pattern/ac160_p_7_a0001_data_hw_roundedpattern_l_tm.h"
#include "../mediatek/mediatek_v2/v1/mtk_corner_pattern/ac160_p_7_a0001_data_hw_roundedpattern_r_tm.h"
...
static int lcm_panel_1p2_ldo_regulator_init(struct device *dev)
{
        ...
        if (IS_ERR_OR_NULL(vddr_ldo)) { /* handle return value */
                ...
                pr_err("get vddr_ldo fail, error: %d\n", ret);
        }
...
static int lcm_panel_1p2_ldo_enable(struct device *dev)
{
	...
	if (!IS_ERR_OR_NULL(vddr_ldo)) {
		...
		if (ret < 0)
			pr_err("set voltage vddr_ldo fail, ret = %d\n", ret);
		retval |= ret;
	}
        /* enable regulator */
	if (!IS_ERR_OR_NULL(vddr_ldo)) {
		...
		if (ret < 0)
			pr_err("enable regulator vddr_ldo fail, ret = %d\n", ret);
		retval |= ret;
	}
...
static int lcm_panel_1p2_ldo_disable(struct device *dev)
{
	...
	if (!IS_ERR_OR_NULL(vddr_ldo)) {
		...
		if (ret < 0)
			pr_err("disable regulator vddr_ldo fail, ret = %d\n", ret);
		retval |= ret;
	}
	return ret;
}
...
static int lcm_panel_vufs_ldo_regulator_init(struct device *dev)
{
        ...
        if (regulator_vufs_inited)
                return ret;
		pr_info("get lcm_panel_vufs_ldo_regulator_init\n");
...
static int lcm_panel_vufs_ldo_enable(struct device *dev)
{
	...
	pr_info("get lcm_panel_vufs_ldo_enable\n");
...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	pr_info(" call lpm_smc_spm_dbg keep vcore off for display off!\n");
...
//#ifdef OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT
	...
	.oplus_ofp_hbm_on_need_wait_extra_te = true,
...
static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle, unsigned int level)
{
	...
	if (panel_power_on == true || level == 0) {
		...
		pr_debug("[INFO][%s:%d]backlight lvl:%u\n", __func__, __LINE__, level);
	}
		...
		unsigned int dst_mode, enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	...
	if (ext->params->data_rate != last_data_rate) {
		ret = 1;
		pr_info("lcm need to change mipi clk, data_rate=%d,last_data_rate=%d\n",ext->params->data_rate,last_data_rate);
...
void lcm_read_info(struct drm_panel *panel, int read_ic)
{
	...
	if (!IS_ERR_OR_NULL(vufs_ldo)) {
		...
		pr_info("get the Regulator vufs_ldo =%d.\n",volt);
	}
	...
	if (!IS_ERR_OR_NULL(vddr_ldo)) {
		if (((pcb_version > 0) && (pcb_version < 7)) || (pcb_version == 9)) {
			...
			pr_info("get the Regulator vddr_ldo =%d.\n",volt);
		}
	}
	...
	if (!IS_ERR_OR_NULL(wl2868c_ldo)) {
		...
		pr_info("get the Regulator wl2868c_ldo =%d.\n",volt);
	}

	gpio_dump_regs_range(120, 120);
	gpio_dump_regs_range(149, 149);

	pr_info("read_ic = %d. pcb_version=%d\n", read_ic, pcb_version);
...
static int lcm_get_modes(struct drm_panel *panel, struct drm_connector *connector)
{
	...
	pr_debug("clock=%d,htotal=%d,vtotal=%d,hskew=%d,vrefresh=%d\n", mode[0]->clock, mode[0]->htotal,
...
/* #endif */ /* OPLUS_FEATURE_ONSCREENFINGERPRINT */
	...
	pr_info("[LCM] %s+ p_7 AC160_A0001 Start\n", __func__);
...
/* #endif */ /* OPLUS_FEATURE_ONSCREENFINGERPRINT */
	...
	pr_info("AC160_A0001, oplus_lcm_id2:%s lcm_id2:%d End\n", oplus_lcm_id2, lcm_id2);
