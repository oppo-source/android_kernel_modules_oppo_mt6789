...
#include "../mediatek/mediatek_v2/v1/mtk_corner_pattern/ac124_p_b_a0012_data_hw_roundedpattern_l_boe.h"
#include "../mediatek/mediatek_v2/v1/mtk_corner_pattern/ac124_p_b_a0012_data_hw_roundedpattern_r_boe.h"
...
static void lcm_panel_get_data(struct dsi_panel_lcm *ctx)
{
	...
	if (ret == 0) {
		...
		pr_err("0x%08x\n",buffer[0] | (buffer[1] << 8));
		pr_err("return %d data(0x%08x) to dsi engine\n",
...
static int lcm_panel_wl2868c_ldo_regulator_init(struct device *dev)
{
        ...
        if (regulator_wl2868c_inited)
                return ret;
		pr_err("get lcm_panel_wl2868c_ldo_regulator_init\n");
...
static int lcm_panel_wl2868c_ldo_enable(struct device *dev)
{
	...
	pr_err("get lcm_panel_wl2868c_ldo_enable\n");
...
static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle, unsigned int level)
{
	...
	if (panel_power_on == true || level == 0) {
		pr_err("[INFO][%s:%d]backlight lvl:%u\n", __func__, __LINE__, level);
...
static int oplus_esd_backlight_recovery(void *dsi, dcs_write_gce cb, void *handle)
{
	...
	pr_debug("esd_bl_level[1]=%x, esd_bl_level[2]=%x\n", esd_bl_level[1], esd_bl_level[2]);
...
static int lcm_set_hbm(void *dsi, dcs_write_gce cb, void *handle, unsigned int hbm_mode)
{
	...
	if(hbm_mode == 1) {
	    ...
	    pr_debug("HBM_off_setting[1]=%x, HBM_off_setting[2]=%x\n", HBM_off_setting[1], HBM_off_setting[2]);
	}
	return 0;
}
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	pr_err("%s: p_b_a0012 lcm ctx->prepared %d\n", __func__, ctx->prepared);
...
static int lcm_get_modes(struct drm_panel *panel, struct drm_connector *connector)
{
	...
	//pr_err("en=%u, clock=%d,htotal=%d,vtotal=%d,hskew=%d,vrefresh=%d\n",mode[0], mode[0]->clock, mode[0]->htotal,
...
/* #endif */ /* OPLUS_FEATURE_ONSCREENFINGERPRINT */
	...
	oplus_max_normal_brightness = MAX_NORMAL_BRIGHTNESS - 1;
/* #ifdef OPLUS_FEATURE_ONSCREENFINGERPRINT */
	oplus_ofp_set_fp_type(&fp_type);
	oplus_ofp_init(dev->of_node);
/* #endif */ /* OPLUS_FEATURE_ONSCREENFINGERPRINT */

	pr_err("[LCM] %s- p_b_a0012, End\n", __func__);
