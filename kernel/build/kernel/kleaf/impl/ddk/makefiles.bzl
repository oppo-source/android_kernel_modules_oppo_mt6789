...
def _handle_opts(ctx, file_name, opts, pre_opts_json = None):
    ...
    expand_targets += ctx.attr.module_user_srcs
...
#add by  charger  start
def _handle_user_srcs(ctx):
    srcs_json_list = []
    gen_srcs_depsets = []
    for target in ctx.attr.module_user_srcs:
    ...
    srcs_json = ctx.actions.declare_file("{}/user_srcs.json".format(ctx.attr.name))
    ctx.actions.write(
        output = srcs_json,
        content = json.encode_indent(srcs_json_list, indent = "  "),
    )
...
def _makefiles_impl(ctx):
    ...
    user_srcs_files = depset(transitive = [target.files for target in ctx.attr.module_user_srcs])
    ...
    direct_include_infos = [DdkIncludeInfo(
        ...
        direct_files = depset(transitive = [srcs_files, user_srcs_files, crate_root_files]),
    ...
    user_srcs_ret = _handle_user_srcs(ctx)
    ...
    args.add("--kernel-user-srcs-json", user_srcs_ret.srcs_json)
    ...
    for user_def in ctx.attr.module_user_makefile_defs:
        args.add("--add-defs", user_def)
    ...
    ctx.actions.run(
        ...
        inputs = depset([
            ...
            user_srcs_ret.srcs_json,
        ], transitive = [submodule_makefiles, module_srcs_ret.gen_srcs_depset, user_srcs_ret.gen_srcs_depset]),
    ...
    srcs_depset_transitive = [srcs_files, user_srcs_files, crate_root_files]
...
makefiles = rule(
    ...
    attrs = {
        ...
        "module_user_makefile_defs": attr.string_list(),
        "module_user_srcs": attr.label_list(allow_files = True),
