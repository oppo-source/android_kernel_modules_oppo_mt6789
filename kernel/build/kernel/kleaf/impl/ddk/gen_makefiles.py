...
def _generate_kbuild_and_extra(
    ...
    kernel_user_srcs_json_content = json.load(kernel_user_srcs_json)
    ...
    for kernel_user_srcs_json_item in kernel_user_srcs_json_content:
        rel_item = dict(kernel_user_srcs_json_item)
        rel_item["files"] = [pathlib.Path(src).relative_to(package)
                             for src in rel_item.get("files", [])
                             if pathlib.Path(src).is_relative_to(package)]
        ...
        for short_path, path in rel_item.get("gen", {}).items():
            ...
            dest.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy(path, dest)
            print("wkcs: cp {} to {}".format(path, dest))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--add-defs", nargs="*", default=[])
    parser.add_argument("--kernel-user-srcs-json",
                        type=argparse.FileType("r"), required=True)
