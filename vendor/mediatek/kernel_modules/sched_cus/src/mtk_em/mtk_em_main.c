...
static int __init mtk_em_init(void)
{
	...
	if (!mtk_em_base_info)
		...
		if (mtk_static_power_int_init(mtk_em_base_info) != 0) {
			pr_info("%s module init failed.\n", __func__);
			return -EINVAL;
		}
	}

	if (mtk_em_base_info) {
		long *em_api_data = get_mtk_em_api_data();

		if (em_api_data == NULL) {
			pr_info("%s invalid em_api_data, module init failed\n", __func__);
			return EINVAL;
		}

		pr_info("%s %d: init_em_api_1st_stage(%ld, %ld, %ld)", __func__, __LINE__, em_api_data[0], em_api_data[1], em_api_data[2]);
		init_em_api_1st_stage(em_api_data);
