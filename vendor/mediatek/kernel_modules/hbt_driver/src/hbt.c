...
static long hbt_compat_ioctl(struct hbt_compat_ioctl __user *argp)
{
	...
	struct file *f_file = fd_file(f);
#else
	struct file *f_file = f.file;
#endif
	if (!f_file)
		return -EBADF;
	...
	retval = security_file_ioctl(f_file, args.cmd, args.arg);
	if (retval)
		goto out;

	retval = -ENOIOCTLCMD;
	if (f_file->f_op->compat_ioctl)
		retval = f_file->f_op->compat_ioctl(f_file, args.cmd, args.arg);
...
#if (KERNEL_VERSION(6, 12, 0) <= LINUX_VERSION_CODE)
	if (fd_file(f) && (fd_file(f)->f_mode & FMODE_ATOMIC_POS)) {
		if (file_count(fd_file(f)) > 1) {
			f.word |= FDPUT_POS_UNLOCK;
			mutex_lock(&fd_file(f)->f_pos_lock);
		}
	}
#else
...
static void my_fdput_pos(struct fd fd)
{
	...
	if (fd.word & FDPUT_POS_UNLOCK)
		mutex_unlock(&fd_file(fd)->f_pos_lock);
	fdput(fd);
#else
...
hbt_compat_getdents64(struct hbt_compat_getdents64 __user *argp)
{
	...
	error = iterate_dir(f_file, &buf.ctx);
	if (error >= 0)
		error = buf.error;
	if (buf.prev_reclen) {
...
static long hbt_compat_lseek(struct hbt_compat_lseek __user *argp)
{
	...
	set_32bit(true);
	offset = vfs_llseek(f_file, args.offset, args.whence);
	set_32bit(false);
