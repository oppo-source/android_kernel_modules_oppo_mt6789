...
#ifdef OV64B_ISF_DBG
struct subdrv_static_ctx ov64b_legacy_s_ctx = {
#else
...
#ifdef OV64B_ISF_DBG
const struct subdrv_entry ov64b_mipi_raw_entry_legacy = {
#else
...
static int ov64b_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	set_table_to_buffer(ctx, addr_data_pair_seamless_switch_step2_ov64b,
		ARRAY_SIZE(addr_data_pair_seamless_switch_step2_ov64b));
	if (scen1_hdr == HDR_RAW_STAGGER) {
		set_table_to_buffer(ctx, addr_data_pair_seamless_switch_step3_HDR_ov64b,
			ARRAY_SIZE(addr_data_pair_seamless_switch_step3_HDR_ov64b));
			DRV_LOG(ctx, "do hdr to linear mode\n");
	} else {
		set_table_to_buffer(ctx, addr_data_pair_seamless_switch_step3_ov64b,
...
{
#ifdef OV64B_ISF_DBG
	memcpy(&(ctx->s_ctx), &ov64b_legacy_s_ctx, sizeof(struct subdrv_static_ctx));
#else
