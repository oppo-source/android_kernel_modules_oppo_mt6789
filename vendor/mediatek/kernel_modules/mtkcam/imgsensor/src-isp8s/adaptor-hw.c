...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
void adaptor_vsvoter_set(struct adaptor_ctx *ctx)
{
	u32 reg, msk, val;

	if (IS_ERR_OR_NULL(ctx->vs_voter.vsv))
		return;

	/* write 1 to set and clr, update reg address */
	reg = ctx->vs_voter.vsv_reg + VS_VOTER_EN_LO_SET;
	msk = ctx->vs_voter.vsv_mask;
	val = ctx->vs_voter.vsv_mask;

	regmap_update_bits(ctx->vs_voter.vsv, reg, msk, val);
	adaptor_logi(ctx, "set voter for vs\n");

	return;
}

void adaptor_vsvoter_clr(struct adaptor_ctx *ctx)
{
	u32 reg, msk, val;
	...
	/* write 1 to set and clr, update reg address */
	reg = ctx->vs_voter.vsv_reg + VS_VOTER_EN_LO_CLR;
	msk = ctx->vs_voter.vsv_mask;
	val = ctx->vs_voter.vsv_mask;

	regmap_update_bits(ctx->vs_voter.vsv, reg, msk, val);
	adaptor_logi(ctx, "clr voter for vs\n");
