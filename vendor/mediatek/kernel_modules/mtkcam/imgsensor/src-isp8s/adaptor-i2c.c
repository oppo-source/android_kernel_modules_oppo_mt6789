...
static unsigned int ixc_err_flag = 0;
module_param(ixc_err_flag, uint, 0644);
inline unsigned char* payLoadConvert(struct adaptor_ctx *ctx, unsigned char* payload, const char* op, u16 addr, u16 reg, u16 val)
{
	if (ctx) {
		...
		if (strcmp(ctx->subdrv->name, "cjfront_mipi_raw") == 0) {
			ixc_err_flag = 1;
		}
	}
	return payload;
		...
		u16 addr, u16 *list, u32 len)
{
	...
	while (sent < total) {
		...
		if (ret < 0){
			...
			#if defined(OPLUS_FEATURE_CAMERA_COMMON) && defined(CONFIG_OPLUS_CAM_EVENT_REPORT_MODULE)
			if (ctx)
				cam_olc_raise_exception(EXCEP_I2C, payLoadConvert(ctx, payload, "ixc_wr_regs_u8_u8", addr, list[0], list[1]));
			#endif /* OPLUS_FEATURE_CAMERA_COMMON */
			kfree(pmem);
			return -EIO;
		}
