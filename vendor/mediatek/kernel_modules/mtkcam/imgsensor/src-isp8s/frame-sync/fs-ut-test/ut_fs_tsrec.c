...
static void ut_fs_tsrec_user_intr_en_dynamic_extra_ctrl(
	const unsigned int idx, const unsigned int tsrec_no)
{
	unsigned int exp0_en, exp1_en, exp2_en;
	...
	exp2_en = 1;
	UT_INF(
		"UT USER dynamic ctrl extra INTR => seninf_idx:%u, tsrec_no:%u => set exp(%u/%u/%u)\n",
		idx, tsrec_no, exp0_en, exp1_en, exp2_en);

	mtk_cam_seninf_tsrec_user_intr_en_dynamic_extra_ctrl(
		idx, tsrec_no, exp0_en, exp1_en, exp2_en, __func__);
	...
	UT_INF(
		"UT USER dynamic ctrl extra INTR => seninf_idx:%u, tsrec_no:%u => clr exp(0/0/0)\n",
		idx, tsrec_no);

	mtk_cam_seninf_tsrec_user_intr_en_dynamic_extra_ctrl(
		idx, tsrec_no, 0, 0, 0, __func__);
...
static void ut_fs_tsrec_settings_start(void)
{
	...
	UT_INF("\n");
	ut_fs_tsrec_user_intr_en_dynamic_extra_ctrl(idx, tsrec_no);
