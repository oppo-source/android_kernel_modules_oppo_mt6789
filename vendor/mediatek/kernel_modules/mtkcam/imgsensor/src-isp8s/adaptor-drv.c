...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
static int vsvoter_of_property_parse(struct adaptor_ctx *ctx)
{
	...
	ctx->vs_voter.vsv = NULL;
	/* vs vote function is optional */
	if (!of_property_read_bool(dev->of_node, "mediatek,vs-voter")) {
		adaptor_loge(ctx, "of_property_read_bool error");
		return 0;
	}

	ret = of_parse_phandle_with_fixed_args(dev->of_node, "mediatek,vs-voter", 3, 0, &args);
	if (ret) {
		adaptor_loge(ctx, "of_parse_phandle_with_fixed_args error");
		return ret;
	}

	pdev = of_find_device_by_node(args.np->child);
	if (!pdev) {
		adaptor_loge(ctx, "of_find_device_by_node error");
		return -ENODEV;
	}

	ctx->vs_voter.vsv = dev_get_regmap(pdev->dev.parent, NULL);
	if (!ctx->vs_voter.vsv) {
		adaptor_loge(ctx, "dev_get_regmap error");
		return -ENODEV;
	}

	ctx->vs_voter.vsv_reg = args.args[0];
	ctx->vs_voter.vsv_mask = args.args[1];
	ctx->vs_voter.vsv_vers = args.args[2];
	adaptor_logi(ctx, "vsv - reg:0x%x, mask:0x%x, version:%d\n", ctx->vs_voter.vsv_reg, ctx->vs_voter.vsv_mask, ctx->vs_voter.vsv_vers);

	return PTR_ERR_OR_ZERO(ctx->vs_voter.vsv);
}
#endif /*OPLUS_FEATURE_CAMERA_COMMON*/
