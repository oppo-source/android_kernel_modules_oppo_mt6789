...
static void mtk_aie_hw_disconnect(struct mtk_aie_dev *fd)
{
	...
	if (fd->fd_stream_count == 0) { /* have hw_connect */
		...
		if (fd->map_count == 1) { /* have qbuf + map memory */
			...
			dma_buf_vunmap_unlocked(fd->dmabuf, &fd->para_map);
#else
			dma_buf_vunmap(fd->dmabuf, &fd->para_map);
#endif
...
static signed int fdvt_dump_reg(struct mtk_aie_dev *fd)
{
	...
	if (fd->aie_cfg->sel_mode == 3) {
...
#if CHECK_SERVICE_1
		dev_info(fd->dev,
			"fdmode_fdvt_yuv2rgb_config:	0x%p, fdmode_fdvt_yuv2rgb_config_size:	%d",
		...
		dev_info(fd->dev,
			"fdmode_fdvt_rs_config:	  0x%p, fdmode_fdvt_rs_config_size:	 %d",
		...
		dev_info(fd->dev,
			"fdmode_fdvt_fd_config:	0x%p, fdmode_fdvt_fd_config_size:	%d",
		...
		for (i = 0; i <= 0x27; i++) {
			...
			dev_info(fd->dev, "[FDVT_DEBUG_SEL - %x]: 0x%08X %08X\n", i,
				(unsigned int)(FDVT_BASE_HW + DMA_DEBUG_SEL_REG),
				  ...
				  struct v4l2_buffer *buf)
{
	...
	if (buf->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) { /*IMG & data*/
		if (!fd->map_count) {
...
#if KERNEL_VERSION(6, 6, 0) <= LINUX_VERSION_CODE
			ret = (u64)dma_buf_vmap_unlocked(fd->dmabuf, &fd->para_map);
#else
			ret = (u64)dma_buf_vmap(fd->dmabuf, &fd->para_map);
#endif
			if (ret) {
			...
			fd->kva = (unsigned long long)fd->para_map.vaddr;
...
static int mtk_aie_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "cmdq fd->fdvt_clt:%p\n", fd->fdvt_clt);
	if (!fd->fdvt_clt)
...
static const struct of_device_id mtk_aie_71_of_ids[] = {
	{
		.compatible = "mediatek,aie-hw3.0",
	},
	{} };
MODULE_DEVICE_TABLE(of, mtk_aie_71_of_ids);
...
static struct platform_driver mtk_aie_driver = {
	...
	.driver = {
		.name = "mtk-aie-5.3",
		.of_match_table = mtk_aie_71_of_ids,
		.pm = &mtk_aie_pm_ops,
	} };
