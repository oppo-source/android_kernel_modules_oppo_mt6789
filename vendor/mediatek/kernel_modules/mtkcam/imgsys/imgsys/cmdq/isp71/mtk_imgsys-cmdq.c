...
void imgsys_cmdq_init_isp71(struct mtk_imgsys_dev *imgsys_dev, const int nr_imgsys_dev)
{
...
EXPORT_SYMBOL(imgsys_cmdq_init_isp71);

void imgsys_cmdq_release_isp71(struct mtk_imgsys_dev *imgsys_dev)
{
	struct mtk_imgcmdq_dev *cmdq_dev = platform_get_drvdata(imgsys_dev->imgcmdq_pdev);

	cmdq_dev->cust_data->cmdq_release(imgsys_dev);
}
EXPORT_SYMBOL(imgsys_cmdq_release_isp71);

void imgsys_cmdq_streamon_isp71(struct mtk_imgsys_dev *imgsys_dev)
{
	struct mtk_imgcmdq_dev *cmdq_dev = platform_get_drvdata(imgsys_dev->imgcmdq_pdev);

	cmdq_dev->cust_data->cmdq_streamon(imgsys_dev);
}
EXPORT_SYMBOL(imgsys_cmdq_streamon_isp71);


void imgsys_cmdq_streamoff_isp71(struct mtk_imgsys_dev *imgsys_dev)
{
	struct mtk_imgcmdq_dev *cmdq_dev = platform_get_drvdata(imgsys_dev->imgcmdq_pdev);

	cmdq_dev->cust_data->cmdq_streamoff(imgsys_dev);
}
EXPORT_SYMBOL(imgsys_cmdq_streamoff_isp71);

int imgsys_cmdq_sendtask_isp71(struct mtk_imgsys_dev *imgsys_dev,
...
EXPORT_SYMBOL(imgsys_cmdq_sendtask_isp71);

void imgsys_cmdq_clearevent_isp71(struct mtk_imgsys_dev *imgsys_dev, int event_id)
{
	struct mtk_imgcmdq_dev *cmdq_dev = platform_get_drvdata(imgsys_dev->imgcmdq_pdev);

	cmdq_dev->cust_data->cmdq_clearevent(event_id);
}
EXPORT_SYMBOL(imgsys_cmdq_clearevent_isp71);

#if DVFS_QOS_READY
void mtk_imgsys_mmdvfs_init_isp71(struct mtk_imgsys_dev *imgsys_dev)
{
	struct mtk_imgcmdq_dev *cmdq_dev = platform_get_drvdata(imgsys_dev->imgcmdq_pdev);

	cmdq_dev->cust_data->mmdvfs_init(imgsys_dev);
}
EXPORT_SYMBOL(mtk_imgsys_mmdvfs_init_isp71);

void mtk_imgsys_mmdvfs_uninit_isp71(struct mtk_imgsys_dev *imgsys_dev)
{
	struct mtk_imgcmdq_dev *cmdq_dev = platform_get_drvdata(imgsys_dev->imgcmdq_pdev);

	cmdq_dev->cust_data->mmdvfs_uninit(imgsys_dev);
}
EXPORT_SYMBOL(mtk_imgsys_mmdvfs_uninit_isp71);

void mtk_imgsys_mmqos_init_isp71(struct mtk_imgsys_dev *imgsys_dev)
{
	struct mtk_imgcmdq_dev *cmdq_dev = platform_get_drvdata(imgsys_dev->imgcmdq_pdev);

	cmdq_dev->cust_data->mmqos_init(imgsys_dev);
}
EXPORT_SYMBOL(mtk_imgsys_mmqos_init_isp71);

void mtk_imgsys_mmqos_uninit_isp71(struct mtk_imgsys_dev *imgsys_dev)
{
	struct mtk_imgcmdq_dev *cmdq_dev = platform_get_drvdata(imgsys_dev->imgcmdq_pdev);

	cmdq_dev->cust_data->mmqos_init(imgsys_dev);
}
EXPORT_SYMBOL(mtk_imgsys_mmqos_uninit_isp71);

void mtk_imgsys_power_ctrl_isp71(struct mtk_imgsys_dev *imgsys_dev, bool isPowerOn)
{
	struct mtk_imgcmdq_dev *cmdq_dev = platform_get_drvdata(imgsys_dev->imgcmdq_pdev);

	cmdq_dev->cust_data->power_ctrl(imgsys_dev, isPowerOn);
}
EXPORT_SYMBOL(mtk_imgsys_power_ctrl_isp71);

void mtk_imgsys_main_power_ctrl_isp71(struct mtk_imgsys_dev *imgsys_dev, bool isPowerOn)
{
	struct mtk_imgcmdq_dev *cmdq_dev = platform_get_drvdata(imgsys_dev->imgcmdq_pdev);

	cmdq_dev->cust_data->main_power_ctrl(imgsys_dev, isPowerOn);
}
EXPORT_SYMBOL(mtk_imgsys_main_power_ctrl_isp71);
...
static int mtk_imgsys_cmdq_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "- X. imgsys cmdq driver probe success====\n");
	return 0;
}
...
static const struct of_device_id mtk_imgsys_cmdq_of_match[] = {
	{.compatible = "mediatek,imgsys-cmdq", .data = (void *)&imgsys_cmdq_data_71},
	{}
};
