				   ...
				   int enable)
{
	...
	if (enable) {
		...
		if (imgsys_clear_dbgtop_enable()) {
			mtk_dbgtop_write_non_reset_reg(MTK_DBGTOP_NONRST2, MTK_IMGSYS_VMM_CVFS_WA_0);
		}
	} else {
...
bypass_larbs:
	...
	/* Add imgsys power hint read from hvs */
	vmm_cvfs_va = ioremap(0x31AC4000, 0x1000);
	imgsys_dev->pwr_hint = ioread32((void *)(vmm_cvfs_va + 0x490));
	iounmap(vmm_cvfs_va);
	dev_info(imgsys_dev->dev, "imgsys pwr_hint (0x%x) non-reset(0x%x)\n",
		imgsys_dev->pwr_hint, mtk_dbgtop_read_non_reset_reg(MTK_DBGTOP_NONRST2));
