...
#define IMGSYS_DMA_NUM_PER_FRAME_MAX (32)
#define IMGSYS_NORMAL_FRAME_NUM_MAX  TMAX
#define IMGSYS_SMVR_FRAME_NUM_MAX    TIME_MAX
#define IMGSYS_VSDOF_FRAME_NUM_MAX   (24)

#define IMGSYS_INVALID_DMA_IDX       (255)
#endif /* defined (IMGSYS_SINGLE_NODE_DESC_V2) */
...
#if defined(IMGSYS_SINGLE_NODE_DESC_V2)
struct framepack {
	struct frameparams ctrl_meta;
	struct frameparams ctrl_meta_from_user;
	struct frameparams ctrl_meta_from_user_mae;
	struct frameparams tuning_meta;
	struct frameparams dmas[IMGSYS_DMA_NUM_PER_FRAME_MAX];
	__u32              dmas_num;
	__u8               dmas_enable[IMGSYS_HW_DMAS_NUM_MAX];
};

struct singlenode_desc_normal {
	__u64 req_stat;
	__u32 reserved;
	__u32 frames_num;
	struct framepack frames[IMGSYS_NORMAL_FRAME_NUM_MAX];
};

struct singlenode_desc_smvr {
	__u64 req_stat;
	__u32 reserved;
	__u32 frames_num;
	struct framepack frames[IMGSYS_SMVR_FRAME_NUM_MAX];
};

struct singlenode_desc_vsdof {
	__u64 req_stat;
	__u32 reserved;
	__u32 frames_num;
	struct framepack frames[IMGSYS_VSDOF_FRAME_NUM_MAX];
};
#else
/* Following part will be deprecated */
