...
mtk_cam_resource_update_work_buf(struct mtk_cam_resource_v2 *user_ctrl)
{
	...
	/*u64 frame_interval_ns = INTERVAL_NS(s->interval.denominator);*/

	switch (scen->id) {
	case MTK_CAM_SCEN_NORMAL:
		exp_num = scen_exp_num(scen);
		buf_require = res_raw_is_dc_mode(r) ? exp_num : exp_num - 1;
		buf_require = !!(scen->scen.normal.w_chn_supported) ?
					buf_require * 2 : buf_require;
		/* double wbuf */
		/* 1. ois */
		/* 2. for small size sensor, reserve at least 2x wbuf for */
		/* camsv to write rotationally, avoid ufbc len table overwrited */
		/* 3. dc 60fps */
		if (res_raw_ois_compensation(r) ||
		    ((s->width <= SEN_4K_W_HALF) && (s->height <= SEN_4K_H_HALF)) ||
		    (res_raw_is_dc_mode(r)/* && (frame_interval_ns < INTERVAL_NS(350))*/))
			buf_require *= 2;
				     ...
				     struct mtk_cam_resource_driver *drv_data)
{
	...
	c.from_rawi = (res_raw_is_dc_mode(r) || scen_is_m2m(&r->scen));
...
EXIT:
	if (drv_data || ret == -EBUSY || CAM_DEBUG_ENABLED(V4L2_TRY)) {
		...
		dev_info(cam->dev,
			 "calc_resource:%d: sensor fps %u/%u %dx%d blank %u/%u prate %llu linet %ld num %d f_pixmode %d from_rawi %d %s\n",
			 ret,
			 s->interval.denominator, s->interval.numerator,
			 s->width, s->height, s->hblank, s->vblank,
			 s->pixel_rate,
			 c.line_time, c.raw_num, c.frontal_pixel_mode, c.from_rawi,
			 str);
		#endif /* OPLUS_FEATURE_CAMERA_COMMON */
	}
	return ret;
