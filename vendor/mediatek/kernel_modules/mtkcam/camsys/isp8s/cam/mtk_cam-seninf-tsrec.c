...
struct tsrec_n_regs_st {
	...
	/* ext ctrl flag */
	unsigned int valid_for_user_intr_en_ext_ctrl;
	unsigned int ext_exp_vsync_en_mask;
};
...
struct tsrec_status_st {
	...
	/* locks (array --- for each tsrec_n) */
	struct mutex *intr_ctrl_lock_arr;
};
...
/*---------------------------------------------------------------------------*/
static inline struct mutex *g_tsrec_n_mutex_lock(const unsigned int tsrec_no,
	struct mutex lock_arr[], const char *caller)
{
	if (unlikely(tsrec_no >= tsrec_status.tsrec_hw_cnt)) {
		TSREC_LOG_INF("[%s]ERROR: invalid tsrec_no:%u (tsrec_hw_cnt:%u)",
			caller, tsrec_no, tsrec_status.tsrec_hw_cnt);
		return NULL;
	}
	return &lock_arr[tsrec_no];
}


static void tsrec_mutex_locks_uninit(struct mutex **p_lock_array)
{
	if (unlikely(p_lock_array == NULL))
		return;

	/* !!! free all dynamic allocated locks !!! */
	if (likely(*p_lock_array != NULL)) {
		TSREC_DEVM_KFREE(*p_lock_array);
		*p_lock_array = NULL;
	}
}


static void tsrec_mutex_locks_init(const unsigned int tsrec_hw_cnt,
	struct mutex **p_lock_array)
{
	unsigned int i;

	if (unlikely(tsrec_hw_cnt == 0 || p_lock_array == NULL))
		return;

	/* !!! allocate array of the lock !!! */
	if (likely(*p_lock_array == NULL)) {
		*p_lock_array = TSREC_KCALLOC(tsrec_hw_cnt, sizeof(struct mutex));
		if (unlikely(*p_lock_array == NULL)) {
			TSREC_LOG_INF(
				"ERROR: p_lock_array dynamic allocate memory failed, return  [tsrec_hw_cnt:%u]\n",
				tsrec_hw_cnt);
			return;
		}

		for (i = 0; i < tsrec_hw_cnt; ++i) {
			/* !!! init each array member !!! */
			TSREC_MUTEX_INIT(&(*p_lock_array)[i]);
		}
	}
}


static inline void tsrec_locks_uninit(void)
{
	tsrec_mutex_locks_uninit(&tsrec_status.intr_ctrl_lock_arr);
}


static inline void tsrec_locks_init(const unsigned int tsrec_hw_cnt)
{
	tsrec_mutex_locks_init(tsrec_hw_cnt, &tsrec_status.intr_ctrl_lock_arr);
}
...
static void tsrec_intr_reset(const unsigned int tsrec_no)
{
	...
	struct mutex *p_intr_ctrl_lock = NULL;
	...
	p_intr_ctrl_lock = g_tsrec_n_mutex_lock(
		tsrec_no, tsrec_status.intr_ctrl_lock_arr, __func__);

	TSREC_MUTEX_LOCK_AFTER_CHK(p_intr_ctrl_lock);

	/* hint, non-valid for user do extra ctrl on interrupt enable */
	p_tsrec_n_regs->valid_for_user_intr_en_ext_ctrl = 0;
	...
	TSREC_MUTEX_UNLOCK_AFTER_CHK(p_intr_ctrl_lock);
...
static void tsrec_intr_ctrl_v_h_cheker(const unsigned int pre_latch_exp_num,
	const unsigned int exp0, const unsigned int exp1, const unsigned int exp2,
	unsigned int *p_exp_v_en, unsigned int *p_exp_h_en)
{
	const unsigned int exp_trig_src = g_tsrec_exp_trig_src();
	const unsigned int user_intr_exp_trig_src_both =
		tsrec_con_mgr.intr_exp_trig_src_both;
	unsigned int exp_v_en = 0, exp_h_en = 0;
	...
	/* setup the value based on the condition */
	if (unlikely(user_intr_exp_trig_src_both)) {
		...
		/* INTR EN based on the type of trigger source */
		if (exp_trig_src == 0) {
			exp_v_en |= (exp0 ? 0x1 : 0);
			exp_v_en |= (exp1 ? 0x2 : 0);
			exp_v_en |= (exp2 ? 0x4 : 0);
		...
		/* => also setup the 1st-exp Vsync INTR EN */
		exp_v_en |= ((pre_latch_exp_num == 0) ? 0x1 : 0);
		exp_v_en |= ((pre_latch_exp_num == 1) ? 0x2 : 0);
		exp_v_en |= ((pre_latch_exp_num == 2) ? 0x4 : 0);
	}

	/* copy the value to the output (update result for caller) */
	*p_exp_v_en = exp_v_en;
	*p_exp_h_en = exp_h_en;
}
...
static void tsrec_intr_ctrl_dynamic_checker(
	const struct tsrec_seninf_cfg_st *p_src_cfg,
	const struct tsrec_n_regs_st *p_tsrec_n_regs,
	unsigned int *p_drv_exp_v_en, unsigned int *p_drv_exp_h_en,
	unsigned int *p_exp_v_en, unsigned int *p_exp_h_en)
{
	unsigned int exp0 = 0, exp1 = 0, exp2 = 0, pre_latch_exp_num;
	unsigned int drv_exp_v_en = 0, drv_exp_h_en = 0, user_exp_v_en;
	unsigned int exp_v_en, exp_h_en;

	/* !!! basic info setup !!! */
	/* => check which exp INTR will be enabled */
	tsrec_intr_ctrl_checker(p_src_cfg, &exp0, &exp1, &exp2);
	/* => get sensor pre-latch exp no */
	pre_latch_exp_num = p_tsrec_n_regs->sensor_hw_pre_latch_exp_num;
	/* => get Vsync EN that user needed */
	user_exp_v_en = p_tsrec_n_regs->ext_exp_vsync_en_mask;

	/* !!! start procedure !!! */
	/* => get INTR EN mask that tsrec drv needed (Vsync & 1st-Hsync) */
	tsrec_intr_ctrl_v_h_cheker(pre_latch_exp_num, exp0, exp1, exp2,
		&drv_exp_v_en, &drv_exp_h_en);
	/* => update result for rewriting the register settings */
	exp_v_en = (drv_exp_v_en | user_exp_v_en);
	exp_h_en = drv_exp_h_en;

	/* copy the value to the output (update result for caller) */
	*p_drv_exp_v_en = drv_exp_v_en;
	*p_drv_exp_h_en = drv_exp_h_en;
	*p_exp_v_en = exp_v_en;
	*p_exp_h_en = exp_h_en;
}
	...
	const unsigned int seninf_idx)
{
	...
	/* hint, valid for user do extra ctrl on interrupt enable */
	p_tsrec_n_regs->valid_for_user_intr_en_ext_ctrl = 1;

	TSREC_MUTEX_UNLOCK_AFTER_CHK(p_intr_ctrl_lock);

	TSREC_LOG_INF(
		"tsrec_no:%u, en:1(%u/%u/%u, code:(%#llx/cust:%#llx,pre-latch:%#x),1stExpNo:%u, trig_src:%u) => intr_en(%#x/%#x, %#x), w_clr:%u => intr_en_bits:%#x, usr_ext_ctrl_valid:%u\n",
		tsrec_no,
		exp0, exp1, exp2,
		p_src_cfg->vc_dt_src_code,
		p_src_cfg->cust_vc_dt_src_code,
		p_src_cfg->sensor_hw_pre_latch_exp_code,
		TSREC_1ST_EXP_NUM, exp_trig_src,
		TSREC_ATOMIC_READ(&tsrec_status.intr_en),
		TSREC_ATOMIC_READ(&tsrec_status.intr_en_2),
		TSREC_ATOMIC_READ(&p_tsrec_n_regs->intr_en),
		TSREC_INTR_W_CLR_EN,
		TSREC_ATOMIC_READ(&tsrec_status.intr_en_bits),
		p_tsrec_n_regs->valid_for_user_intr_en_ext_ctrl);
}
...
int mtk_cam_seninf_tsrec_user_intr_en_dynamic_extra_ctrl(
	const unsigned int seninf_idx, const unsigned int tsrec_no,
	const unsigned int exp0_en, const unsigned int exp1_en,
	const unsigned int exp2_en, const char *caller)
{
	struct tsrec_seninf_cfg_st *p_src_cfg = NULL;
	struct tsrec_n_regs_st *p_tsrec_n_regs = NULL;
	struct mutex *p_intr_ctrl_lock = NULL;
	unsigned long long sys_ts;
	unsigned int drv_exp_v_en, drv_exp_h_en, user_exp_v_en = 0;
	unsigned int exp_v_en, exp_h_en;
	unsigned int valid_or_not = 0, ori_intr_en;
	int ret = 0;
	...
	if (unlikely(p_src_cfg == NULL)) {
		TSREC_LOG_INF(
			"[%s]ERROR: seninf src_cfg[%u] is nullptr (%p) => NOT have exp info for setup, return\n",
			caller, seninf_idx, p_src_cfg);
		return -4;
	}
	ret = g_tsrec_n_regs_st(tsrec_no, &p_tsrec_n_regs, __func__);
	if (unlikely(ret != 0))
	...
	if (unlikely(p_tsrec_n_regs == NULL)) {
		TSREC_LOG_INF(
			"ERROR: tsrec_n_regs_st[%u] is nullptr (%p) => NOT have exp info for setup, return\n",
			tsrec_no, p_tsrec_n_regs);
		return -6;
	}
	p_intr_ctrl_lock = g_tsrec_n_mutex_lock(
	...
	/* check if valid for user doing extra INTR ctrl */
	valid_or_not = p_tsrec_n_regs->valid_for_user_intr_en_ext_ctrl;
	if (unlikely(valid_or_not == 0)) {
		ret = -255;
		goto mtk_cam_seninf_tsrec_user_intr_en_extra_ctrl_end;
	}

	ori_intr_en = TSREC_ATOMIC_READ(&p_tsrec_n_regs->intr_en);

	/* setup info that needed before calling the below functions */
	/* => update/setup user extra exp INTR EN mask */
	user_exp_v_en |= (exp0_en ? 0x1 : 0);
	user_exp_v_en |= (exp1_en ? 0x2 : 0);
	user_exp_v_en |= (exp2_en ? 0x4 : 0);
	p_tsrec_n_regs->ext_exp_vsync_en_mask = user_exp_v_en;

	/* dynamic check & setup exp Vsync and 1st-Hsync EN */
	tsrec_intr_ctrl_dynamic_checker(p_src_cfg, p_tsrec_n_regs,
		&drv_exp_v_en, &drv_exp_h_en, &exp_v_en, &exp_h_en);

	/* update/overwrite exp INTR EN settings */
	sys_ts = ktime_get_boottime_ns();
	mtk_cam_seninf_owr_tsrec_n_intr_en(tsrec_no, exp_v_en, exp_h_en);

mtk_cam_seninf_tsrec_user_intr_en_extra_ctrl_end:
	TSREC_MUTEX_UNLOCK_AFTER_CHK(p_intr_ctrl_lock);


	/* after unlocking, log info */
	if (unlikely(valid_or_not == 0)) {
		TSREC_LOG_INF(
			"[%s]NOTICE: no:%u/inf:%u, NOT valid for INTR extra CTRL (%u), exp(user:(%u/%u/%u)), intr_en:%#x, ret:%d\n",
			caller, tsrec_no, seninf_idx, valid_or_not,
			exp0_en, exp1_en, exp2_en,
			TSREC_ATOMIC_READ(&p_tsrec_n_regs->intr_en),
			ret);
	} else {
		TSREC_LOG_INF(
			"[%s]NOTICE: no:%u/inf:%u, exp(user:(%u/%u/%u)), code:(%#llx/cust:%#llx), pre-latch:%u(1st_exp_no:%u) => mask(drv(%#x/h:%#x)/user:%#x) => intr_en(%#x => %#x), ret:%d, sys_ts:%llu(ns)\n",
			caller, tsrec_no, seninf_idx,
			exp0_en, exp1_en, exp2_en,
			p_src_cfg->vc_dt_src_code,
			p_src_cfg->cust_vc_dt_src_code,
			p_tsrec_n_regs->sensor_hw_pre_latch_exp_num,
			TSREC_1ST_EXP_NUM,
			drv_exp_v_en, drv_exp_h_en, user_exp_v_en,
			ori_intr_en, TSREC_ATOMIC_READ(&p_tsrec_n_regs->intr_en),
			ret, sys_ts);
	}
