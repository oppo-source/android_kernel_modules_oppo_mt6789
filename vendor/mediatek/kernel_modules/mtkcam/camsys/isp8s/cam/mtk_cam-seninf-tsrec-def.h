...
#define TSREC_MUTEX_INIT(p)          ((p)->init = 1)
#define TSREC_MUTEX_LOCK(p)
#define TSREC_MUTEX_UNLOCK(p)
#define TSREC_MUTEX_LOCK_AFTER_CHK(p) \
do { \
	if (likely((p) != NULL)) \
		(p)->init = 3; \
} while (0)
#define TSREC_MUTEX_UNLOCK_AFTER_CHK(p) \
do { \
	if (likely((p) != NULL)) \
		(p)->init = 2; \
} while (0)
...
#define TSREC_MUTEX_INIT(p)          mutex_init(p)
#define TSREC_MUTEX_LOCK(p)          (mutex_lock(p))
#define TSREC_MUTEX_UNLOCK(p)        (mutex_unlock(p))
#define TSREC_MUTEX_LOCK_AFTER_CHK(p) \
do { \
	if (likely((p) != NULL)) \
		mutex_lock(p); \
