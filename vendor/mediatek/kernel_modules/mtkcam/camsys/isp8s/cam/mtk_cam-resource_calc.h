...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
static inline int mtk_raw_overall_pixel_mode_no_bin(struct mtk_cam_res_calc *c)
{
	return c->raw_num * c->raw_pixel_mode;
}
#endif /* OPLUS_FEATURE_CAMERA_COMMON */
					...
					bool is_raw, bool enable_log)
{
	...
	if (is_raw) {
		...
		#else
		overall_pixel_mode = (c->from_rawi) ?  /* bottleneck of dc/m2m */
			mtk_raw_overall_pixel_mode_no_bin(c) :
			mtk_raw_overall_pixel_mode(c);
			w_processed = line_time * freq_Mhz * overall_pixel_mode
		#endif /* OPLUS_FEATURE_CAMERA_COMMON */
