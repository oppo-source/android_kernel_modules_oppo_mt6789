							 ...
							 int max_p1_delay_mul)
{
	...
	dev_info(dev->dev,
		"[%s] STAMP_CNT:0x%08x, SCQ_START_PERIOD:0x%08x (%dms) P1_DELAY:0x%08x (%dms) multi:%d\n",
		 __func__, val,
		 raw_readl(dev, dev->base, REG_CAMCQ_SCQ_START_PERIOD), scq_ms,
				      ...
				      unsigned int fh_cookie)
{
	...
	if (cnt < 32) {
		...
		SET_FIELD(&csr, CAMRAWDMATOP_DMA_DBG_EN, 1);
