...
void mtk_cam_seninf_owr_tsrec_n_intr_en(const unsigned int tsrec_n,
	const unsigned int exp_v_en, const unsigned int exp_h_en)
{
	struct tsrec_w_buffer w_buf = {0};
	unsigned int val = 0;
	...
	if (unlikely(!chk_tsrec_no_valid(tsrec_n, __func__))) {
		TSREC_LOG_INF(
			"ERROR: get non-valid tsrec_n, force return   [tsrec_n:%u, en exp(V:%#x/1st-H:%#x)]\n",
			tsrec_n, exp_v_en, exp_h_en);
		return;
	}

	/* setup value for 1st-Hsync INTR EN */
	val |= (exp_h_en & 0x1) ? (1UL << (0 + TSREC_INT_EN_HSYNC_BASE_BIT)) : 0;
	val |= (exp_h_en & 0x2) ? (1UL << (1 + TSREC_INT_EN_HSYNC_BASE_BIT)) : 0;
	val |= (exp_h_en & 0x4) ? (1UL << (2 + TSREC_INT_EN_HSYNC_BASE_BIT)) : 0;
	/* setup value for Vsync INTR EN */
	val |= (exp_v_en & 0x1) ? (1UL << (0 + TSREC_INT_EN_VSYNC_BASE_BIT)) : 0;
	val |= (exp_v_en & 0x2) ? (1UL << (1 + TSREC_INT_EN_VSYNC_BASE_BIT)) : 0;
	val |= (exp_v_en & 0x4) ? (1UL << (2 + TSREC_INT_EN_VSYNC_BASE_BIT)) : 0;
