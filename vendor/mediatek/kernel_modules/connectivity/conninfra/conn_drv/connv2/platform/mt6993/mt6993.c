...
unsigned int consys_get_hw_ver_mt6993(void)
{
	return CONN_HW_VER_MT6993;
}

unsigned int consys_get_adie_chipid_mt6993(unsigned int drv_type)
{
	return CONNSYS_A_DIE_ID_MT6993;
}
...
static void consys_power_state(void)
{
	...
	consys_debug_top_power_status_mt6993();
}
...
int consys_reset_power_state_mt6993(void)
{
	return consys_power_state_dump(NULL, 0, 0);
}

int consys_power_state_dump_mt6993(char *buf, unsigned int size)
{
	return consys_power_state_dump(buf, size, 1);
}

int consys_enable_power_dump_mt6993(void)
{
	/* Return success because sleep count dump is enable on POS */
	return 0;
}

void consys_set_mcu_control_mt6993(int type, bool onoff)
{
