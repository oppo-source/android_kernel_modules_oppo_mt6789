...
struct consys_base_addr conn_reg_mt6858;

static struct conn_debug_info_mt6858 static_debug_info;
static struct conn_debug_info_mt6858 *debug_info;
static char static_debug_buf[CONSYS_DUMP_BUF_SIZE];
static char *debug_buf;

static void consys_print_log(const char *title, struct conn_debug_info_mt6858 *info)
{
	char temp[13];
	int i;
...
static void consys_print_power_debug(enum conninfra_bus_error_type level)
{
	...
	if (level >= CONNINFRA_BUS_LOG_LEVEL_HOST_ONLY) {
		consys_print_power_debug_dbg_level_0_mt6858_debug_gen(level, debug_info);
		consys_print_log("[CONN_POWER_A]", debug_info);
	}
	if (level >= CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_ON) {
		consys_print_power_debug_dbg_level_1_mt6858_debug_gen(level, debug_info);
		consys_print_log("[CONN_POWER_B]", debug_info);
	}
	if (level >= CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_OFF) {
		consys_print_power_debug_dbg_level_2_mt6858_debug_gen(level, debug_info);
...
static void consys_print_bus_debug(enum conninfra_bus_error_type level)
{
	...
	consys_print_bus_slpprot_debug_dbg_level_0_mt6858_debug_gen(level, debug_info);
	consys_print_log("[slpprot_a]", debug_info);

	if (level >= CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_ON) {
		consys_print_bus_debug_dbg_level_1_mt6858_debug_gen(level, debug_info);
		consys_print_log("[CONN_BUS_B]", debug_info);
	}
	if (level >= CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_OFF) {
		consys_print_bus_debug_dbg_level_2_mt6858_debug_gen(level, debug_info);
		consys_print_log("[CONN_BUS_C]", debug_info);
		consys_print_bus_slpprot_debug_dbg_level_2_mt6858_debug_gen(level, debug_info);
...
int consys_check_conninfra_bus_clock_status_mt6858(void)
{
	...
	while (count < 4) {
		r = consys_bus_hang_clock_detect_mt6858();
		if (r == 0x6)
			break;
...
void consys_debug_init_mt6858(void)
{
	...
	consys_debug_init_mt6858_debug_gen();
}
...
void consys_debug_deinit_mt6858(void)
{
	...
	consys_debug_deinit_mt6858_debug_gen();
}
