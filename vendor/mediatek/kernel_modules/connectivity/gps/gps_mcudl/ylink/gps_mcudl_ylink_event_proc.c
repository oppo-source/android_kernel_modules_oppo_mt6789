...
void gps_mcudl_ylink_event_proc(enum gps_mcudl_yid y_id, enum gps_mcudl_ylink_event_id evt)
{
	...
	case GPS_MCUDL_YLINK_EVT_ID_TEST_CLR_FW_OWN:
		...
		break;
	case GPS_MCUDL_YLINK_EVT_ID_TESE_SET_FW_OWN_FIAL_CHECK:
		(void)gps_mcudl_hal_set_fw_own_fail_handler();
		break;
...
{
	bool conn_is_on = g_gps_conninfa_on;

	MDL_LOGW("conn_is_on=%d", conn_is_on);
	if (!conn_is_on)
		return;
...
void gps_mcudl_ylink_test_set_fw_own_fail_check(void)
{
	...
	gps_mcudl_ylink_event_send(GPS_MDLY_NORMAL, GPS_MCUDL_YLINK_EVT_ID_TESE_SET_FW_OWN_FIAL_CHECK);
}
