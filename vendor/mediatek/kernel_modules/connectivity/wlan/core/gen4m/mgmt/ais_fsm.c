...
#if (CFG_SUPPORT_802_11BE_MLO == 1)
void aisHandleRnrMlParam(struct ADAPTER *prAdapter,
	uint8_t ucConnMldId, struct IE_RNR *rnr,
	uint8_t *pucChannelListNum, struct RF_CHANNEL_INFO *parChnlInfoList)
{
	...
	do {
		...
		if (!kalIsValidChnl(prAdapter->prGlueInfo,
				ucRnrChNum, eBand)) {
			DBGLOG(AIS, INFO, "Invalid RNR MlParam (b:%d,ch:%d)\n",
				eBand, ucRnrChNum);
			goto next;
		}
	...
	uint8_t ucBssIndex, struct MSG_SCN_SCAN_REQ_V2 *prScanReqMsg)
{
	...
	IE_FOR_EACH(ie, ie_len, u2Offset) {
		...
		aisHandleRnrMlParam(prAdapter,
			prBssDesc->rMlInfo.ucMldId, rnr,
