 ...
 */
/*----------------------------------------------------------------------------*/
u_int8_t cnmTimerSanityCheck(struct LINK *prTimerList)
{
	struct LINK_ENTRY *prLinkEntry;
	...
	if (prTimerList == NULL) {
		DBGLOG(CNM, WARN, "Timer List is NULL\n");
		ret = FALSE;
		goto exit;
	}
	...
	LINK_FOR_EACH(prLinkEntry, prTimerList) {
		if (prLinkEntry == NULL) {
			DBGLOG(CNM, WARN, "Timer Entery[%u] is NULL\n",
				u4LoopCnt);
			ret = FALSE;
			goto exit;
		}
		...
		u4LoopCnt++;
		if (u4LoopCnt > prTimerList->u4NumElem) {
			DBGLOG(CNM, WARN,
				"loopCnt=%u>[%u]\n",
				u4LoopCnt, prTimerList->u4NumElem);
			ret = FALSE;
			goto exit;
		}
	}
...
exit:
	if (ret == FALSE)
		kalSendAeeWarning("WLAN", "wlan timer list corrupted");
...
void cnmTimerDestroy(struct ADAPTER *prAdapter)
{
	...
	/* Hrtimer */
	KAL_ACQUIRE_SPIN_LOCK(prAdapter, SPIN_LOCK_HRTIMER_LIST);
	if (cnmTimerSanityCheck(&prAdapter->rHrtimerList))
