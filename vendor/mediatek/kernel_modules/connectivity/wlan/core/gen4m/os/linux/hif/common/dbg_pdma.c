...
static void halNotifyTxTimeoutEvent(struct ADAPTER *prAdapter,
...
static void halNotifyTxTimeoutDetailedEvent(struct ADAPTER *prAdapter,
				 struct MSDU_TOKEN_HISTORY_INFO *prHistory,
				 enum ENUM_OP_MODE eOPMode)
{
	...
	pos += kalScnprintf(pos, end - pos,
		"abnormaltrx=DIR:TX,Event:Timeout,OPMode:%u",
		eOPMode);
...
static bool halIsTxTimeout(struct ADAPTER *prAdapter, uint32_t *u4Token)
{
	...
	if (fgIsTimeout) {
		...
		if (prWifiVar->fgUseDetailedTxTimeoutUEvent)
			halNotifyTxTimeoutDetailedEvent(prAdapter, prHistory,
				 eOPMode);
		else
			halNotifyTxTimeoutEvent(prAdapter, prHistory);
