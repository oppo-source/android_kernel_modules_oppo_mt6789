	 ...
	 uint64_t *throughput, int32_t period)
{
	...
	static unsigned long lastTxBytes[MAX_BSSID_NUM];
	static unsigned long lastRxBytes[MAX_BSSID_NUM];
	unsigned long currentTxBytes = 0;
	unsigned long currentRxBytes = 0;
	...
	for (i = 0; i < MAX_BSSID_NUM; i++) {
		...
		if (lastTxBytes[i] == 0 && lastRxBytes[i] == 0) {
			lastTxBytes[i] = currentTxBytes;
			lastRxBytes[i] = currentRxBytes;
			continue;
		}

		txDiffBytes[i] = (signed long)(currentTxBytes - lastTxBytes[i]);
		rxDiffBytes[i] = (signed long)(currentRxBytes - lastRxBytes[i]);
	...
	*throughput = *throughput * MSEC_PER_SEC;
	do_div(*throughput, period);
	*throughput <<= 3;
...
void mbrTRxPerfEnqueue(struct ADAPTER *prAdapter)
{
	...
	mbrTRxPerfCalculateTput(prAdapter, &prTRxPerfInfo->tput, period);
