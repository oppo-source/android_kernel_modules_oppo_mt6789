			     ...
			     struct station_info *sinfo)
{
	...
	//add for beam switch
	if (MAX_ANTENNA_NUM > 1) {
		char uevent[32] = {'\0'};
		snprintf(uevent, sizeof(uevent), "chainRssi=%d,%d", ai4BSSDescRssi[ucBssIndex][ANTENNA_WF0], ai4BSSDescRssi[ucBssIndex][ANTENNA_WF1]);
		kalSendUevent(prAdapter, uevent);
	}

#define TEMP_LOG_TEMPLATE \
	"link speed=%u/%u, bw=%u/%u, " \
	"MovAvg_rssi=%d, Raw_rssi=[BSSDesc(%d,%d),Data(%d,%d)," \
