...
enum hrtimer_restart kalHrtimerTimeout(struct hrtimer *prHrtimer)
{
	...
	cnmTimerSanityCheck(&prAdapter->rHrtimerList);
			      ...
			      signed long rxDiffPkts[])
{
	...
	for (i = 0; i < MAX_BSSID_NUM; i++) {
		...
		if (txDiffBytes[i] < 0 || rxDiffBytes[i] < 0 ||
		    txDiffPkts[i] < 0 || rxDiffPkts[i]  < 0) {
			/* overflow should not happen */
			DBGLOG(SW4, WARN,
				"[%d]wrong bytes/pkts: tx[%lu][%lu][%ld]/[%lu][%lu][%ld], rx[%lu][%lu][%ld],[%lu][%lu][%ld]\n",
				i, currentTxBytes, lastTxBytes, txDiffBytes[i],
				currentTxPkts, lastTxPkts, txDiffPkts[i],
				currentRxBytes, lastRxBytes, rxDiffBytes[i],
				currentRxPkts, lastRxPkts, rxDiffPkts[i]);
			return WLAN_STATUS_FAILURE;
		}
...
static uint32_t kalPerMonUpdate(struct ADAPTER *prAdapter)
{
	...
	pos += kalSnprintf(pos, end - pos,
		"drv[RM,IL,RI,RIE,NG|RF,PA,PAP,PF,DU,DA#RT,RM,RW,RA,RB|RTO,DT,NS,IB,HS_LS,DD,ME,BD,NI|DR,TE,PE,CE,DN^FE,DE,IE,TME,CM#FB,ID,FD,NL]:");

	pos += kalSnprintf(pos, end - pos,
		"%lu,%lu,%lu,%lu,%lu|%lu,%lu,%lu,%lu,%lu,%lu#%lu,%lu,%lu,%lu,%lu|%lu,%lu,%lu,%lu,%lu_%lu,%lu,%lu,%lu,%lu|%lu,%lu,%lu,%lu,%lu^%lu,%lu,%lu,%lu,%lu#%lu,%lu,%lu,%lu",
...

{
	...
	/*print bss info to check*/
	DBGLOG(REQ, INFO, "current bss of index %d %px\n",linkIdx,
			prDevHandler->ieee80211_ptr->links[linkIdx].client.current_bss ? \
			prDevHandler->ieee80211_ptr->links[linkIdx].client.current_bss : 0);
#endif
