...
/*! \file   hal_wfsys_reset_mt7926.c
 *    \brief  WFSYS reset HAL API for MT7926
 *
 ...
 */
/*----------------------------------------------------------------------------*/
u_int8_t mt7926HalCbtopRguWfRst(struct ADAPTER *prAdapter,
				u_int8_t fgAssertRst)
{
	...
	if (fgAssertRst) {
		...
		fgIsWfdmaIdle = mt7926HalPollWfdmaIdle(prAdapter);
		...
		if (ucPollingCount == 10) {
			DBGLOG(HAL, ERROR,
				"[SER][L0.5]SER Polling WFDMA Idle Fail!\n");
		}

		/* Assert WF subsys reset via MMIO */
		HAL_MCR_RD(prAdapter, CBTOP_RGU_WF_SUBSYS_RST_ADDR, &u4CrVal);

		u4CrVal |= BIT(0);

		HAL_MCR_WR(prAdapter, CBTOP_RGU_WF_SUBSYS_RST_ADDR, u4CrVal);
	} else {
 ...
 */
/*----------------------------------------------------------------------------*/
u_int8_t mt7926HalPollWfsysSwInitDone(struct ADAPTER *prAdapter)
{
	...
	while (TRUE) {
		...
		HAL_MCR_RD(prAdapter,
			CONN_INFRA_RGU_WFSYS_SW_RST_B_ADDR, &u4CrValue);

		if (u4CrValue == MMIO_READ_FAIL)
			DBGLOG(HAL, ERROR, "[SER][L0.5] MMIO read CR fail\n");
		else if (u4CrValue &
			CONN_INFRA_RGU_WFSYS_SW_RST_B_WFSYS_SW_INIT_DONE_MASK)
			break;
...
u_int8_t mt7926HalPollWfdmaIdle(struct ADAPTER *prAdapter)
{
	...
	DBGLOG(HAL, INFO,
		"[SER][L0.5] HOST_DMA0_WPDMA_GLO_CFG [0x%08X] = 0x%08X\n",
		WF_WFDMA_HOST_DMA0_WPDMA_GLO_CFG_ADDR, u4WfdmaGloCfg);
	DBGLOG(HAL, INFO,
		"[SER][L0.5] INT_WRAP_CSR_WFDMA_HIF_MISC [0x%08X] = 0x%08X\n",
		WF_WFDMA_INT_WRAP_CSR_WFDMA_HIF_MISC_ADDR, u4WfdmaHifBusyInt);
	DBGLOG(HAL, INFO,
		"[SER][L0.5] EXT_WRAP_CSR_WFDMA_HIF_MISC [0x%08X] = 0x%08X\n",
 ...
 */
/*----------------------------------------------------------------------------*/
u_int8_t mt7926HalUsbEpctlRstOpt(struct ADAPTER *prAdapter,
...
u_int8_t mt7926HalSetNoBTFwOwnEn(IN int32_t i4Enable)
{
...
/* Provide functions for none-OS build */
