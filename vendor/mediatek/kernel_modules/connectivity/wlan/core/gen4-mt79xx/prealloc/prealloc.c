...
static int __init preallocInit(void)
{
	...
	preallocAddBlock(MEM_ID_RX_DATA, "RX DATA",
			(HIF_RX_COALESCING_BUF_COUNT * u4CardCount),
			HIF_RX_COALESCING_BUFFER_SIZE,
			GFP_KERNEL);
#endif
#if defined(_HIF_USB)
	preallocAddBlock(MEM_ID_TX_CMD, "TX CMD",
			(USB_REQ_TX_CMD_CNT * u4CardCount),
			USB_TX_CMD_BUF_SIZE,
			GFP_KERNEL);
	preallocAddBlock(MEM_ID_TX_DATA_FFA, "TX DATA FFA",
			(USB_REQ_TX_DATA_FFA_CNT * u4CardCount),
			USB_TX_DATA_BUFF_SIZE,
			GFP_KERNEL);
	preallocAddBlock(MEM_ID_TX_DATA, "TX AGG DATA",
			(USB_TC_NUM * USB_REQ_TX_DATA_CNT * u4CardCount),
			USB_TX_DATA_BUFF_SIZE,
			GFP_KERNEL);
	preallocAddBlock(MEM_ID_RX_EVENT, "RX EVENT",
			(USB_REQ_RX_EVENT_CNT * u4CardCount),
			USB_RX_EVENT_BUF_SIZE,
			GFP_KERNEL);
	preallocAddBlock(MEM_ID_RX_DATA, "RX DATA",
			(USB_REQ_RX_DATA_CNT * u4CardCount),
			USB_RX_DATA_BUF_SIZE,
			GFP_KERNEL);
#if CFG_CHIP_RESET_SUPPORT
	preallocAddBlock(MEM_ID_RX_WDT, "RX WDT",
			(USB_REQ_RX_WDT_CNT * u4CardCount),
			USB_RX_WDT_BUF_SIZE,
			GFP_KERNEL);
#endif
#endif
