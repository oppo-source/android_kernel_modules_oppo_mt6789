	...
	void *prTxD)
{
	...
	if (u4PktOption & MSDU_OPT_MANUAL_WTBL_ENTRY) {
		/* DBGLOG(RSN, INFO, "MSDU_OPT_MANUAL_WTBL_ENTRY\n"); */
		prMsduInfo->ucWlanIndex = prMsduInfo->ucWtblEntry;
		HAL_MAC_CONNAC2X_TXD_SET_WLAN_INDEX(
			prTxDesc, prMsduInfo->ucWtblEntry);
	}
	...
	u_int8_t *prTxDescBuffer)
{
	...
	*/
	...
	/* Wlan Index of NAN BMC data packet */
	if ((prBssInfo->eNetworkType == NETWORK_TYPE_NAN) &&
		(prMsduInfo->ucStaRecIndex == STA_REC_INDEX_BMCAST)) {
		prMsduInfo->ucWlanIndex = nanFindMultiCastWidx(prAdapter,
			prMsduInfo->aucEthDestAddr, prMsduInfo->aucEthSrcAddr);
		if (prMsduInfo->ucWlanIndex == WTBL_RESERVED_ENTRY) {
			DBGLOG(NAN, INFO,
				"wtblIdx not found D" MACSTR ", S" MACSTR "\n",
