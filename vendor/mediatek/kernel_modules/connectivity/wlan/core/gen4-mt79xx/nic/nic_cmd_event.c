		     ...
		     IN struct WIFI_EVENT *prEvent)
{
	...
	case ENUM_RADAR_RESP_SUBEVENT_PERIODICAL_EVENT:
		...
		if (prRadarRawData->ucStatus ==
			...
			DBGLOG(NIC, WARN,
				"[%u.%03u][RadarInfo] Reason Code: %d, Burst Index:%d, Thermal:%d, RSSI:%d, CBP_Retry:%d\n",
		...
		struct WIFI_EVENT *prEvent)
{
	...
	case EVENT_LOC_CAPA:
		DBGLOG(RTT, INFO, "RTT EVENT_LOC_CAPA.\n");
		/* command response handling */
		prCmdInfo = nicGetPendingCmdInfo(prAdapter, prEvent->ucSeqNum);
		if (prCmdInfo != NULL) {
			if (prCmdInfo->pfCmdDoneHandler)
	...
	case EVENT_LOC_RESULT:
		DBGLOG(RTT, INFO, "RTT EVENT_LOC_RESULT.\n");
		prLocResult =
		...
		break;
	case EVENT_LOC_DONE:
		DBGLOG(RTT, INFO, "RTT EVENT_LOC_DONE.\n");
		prLocDone =
