...
#define CHNL_DWELL_TIME_ONLINE   50
	...
	for (uCnt = 0; uCnt < SCAN_COUNT; uCnt++) {
		for (u4ChCnt = 0; u4ChCnt < prScanInfo
...
#if (CFG_SUPPORT_WIFI_6G == 1)
			if (ucChannel == prScanInfo->
				rScanRadioInfo[uCnt][u4ChCnt].aucChannelNum &&
				eBand == prScanInfo->
				rScanRadioInfo[uCnt][u4ChCnt].aeChannelBand)
#else
			if (ucChannel == prScanInfo->
				rScanRadioInfo[uCnt][u4ChCnt].aucChannelNum)
#endif
			{
				u2ChIdleSlot =
				prScanInfo->rScanRadioInfo[uCnt][u4ChCnt].
						au2ChannelIdleTime;
	...
	uint8_t ucBssIndex)
{
	...
	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry,
		...
		ucByteNum = prBssDesc->ucChannelNum / 8U;
		ucBitNum = prBssDesc->ucChannelNum % 8U;
