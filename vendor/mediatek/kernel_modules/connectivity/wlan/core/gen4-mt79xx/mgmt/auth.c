...
#if (CFG_SUPPORT_NAN_R4_VENDOR_PAIRING == 1)
	DBGLOG(NAN, INFO, "hdr len:%u, %zu, AuthAlgNum:%u\n",
		prSwRfb->u2HeaderLen, sizeof(struct WLAN_AUTH_FRAME),
		prAuthFrame->u2AuthAlgNum);
	/* Pairing auth */
	if ((prAuthFrame->u2AuthAlgNum == AUTH_ALGORITHM_NUM_PASN) &&
		(kalMemCmp(prAuthFrame->aucBSSID, aucOui,
		VENDOR_OUI_LEN) == 0)) {
		DBGLOG(NAN, INFO, "PASN pkt len:%lu\n", prSwRfb->u2PacketLen);
		DBGLOG_MEM8(NAN, INFO, prAuthFrame, prSwRfb->u2PacketLen);
		nanNdlProcessOob(prAdapter, prSwRfb, OOB_TYPE_AUTH);
		return WLAN_STATUS_SUCCESS;
	}
#endif /* CFG_SUPPORT_NAN_R4_VENDOR_PAIRING */
