			   ...
			   IN struct SW_RFB *prSwRfb)
{
	...
	case ACTION_ADDTS_RSP: {
		...
		if (prSwRfb->u2PacketLen <
			(prSwRfb->u2HeaderLen
			+ (uint16_t)(OFFSET_OF(
			struct ACTION_ADDTS_RSP_FRAME, aucInfoElem))
			+ WLAN_MAC_HEADER_LEN))
			break;
 ...
 *enough to Deq
 ** 2. u8PktTxTime is not 0, if remain medium time is greater than u8ktTxTime,
...
u_int8_t wmmAcmCanDequeue(struct ADAPTER *prAdapter, uint8_t ucAc,
	uint64_t u8PktTxTime, uint8_t ucBssIndex)
{
	struct SOFT_ACM_CTRL *prAcmCtrl = NULL;
	struct WMM_INFO *prWmmInfo =
		aisGetWMMInfo(prAdapter, ucBssIndex);
