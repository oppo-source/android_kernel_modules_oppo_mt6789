...
#if (CFG_WIFI_TESTMODE_FW_REDOWNLOAD == 1)
	if (prAdapter->prGlueInfo->fgWaitTestModeDone == TRUE) {
		DBGLOG(INIT, WARN,
			"FW OWN Skipped due to pending TestMode done\n");
		goto unlock;
	}
#endif
 ...
 */
void halSetMailboxHifStatus(IN struct ADAPTER *prAdapter,
	IN uint32_t u4IntrBits)
{
	struct GL_HIF_INFO *prHifInfo;
	uint8_t fgResult = 0;
...
#if (CFG_SUPPORT_SDIO_FORCE_DRV_OWN == 0)
	uint32_t u4DrvOwnReqTimes = 0;
#endif
	ASSERT(prAdapter);
