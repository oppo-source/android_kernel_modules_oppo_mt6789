							...
							uint32_t cmd_type)
{
	...
	if (cmd_type == LISTMODE_RX_SEG) {
		i4Status = kalIoctl(prGlueInfo,	/* prGlueInfo */
			    wlanoidListMode,	/* pfnOidHandler */
			    RxParserSegHeader,	/* pvInfoBuf */
			    header_size,	/* u4InfoBufLen */
			    FALSE,	/* fgRead */
			    FALSE,	/* fgWaitResp */
			    TRUE,	/* fgCmd */
			    &u4BufLen);	/* pu4QryInfoLen */
...
int glRFTestL0P5(void *data)
{
	...
	if (!prGlueInfo->fgSupportTestMode) {
		...
		prGlueInfo->fgWaitTestModeDone = TRUE;
		...
		if (u4Status != WLAN_STATUS_SUCCESS) {
			prGlueInfo->fgWaitTestModeDone = FALSE;
			return u4Status;
			}
