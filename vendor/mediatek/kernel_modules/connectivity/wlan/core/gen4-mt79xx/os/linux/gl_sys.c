...
int32_t sysInitFs(struct GLUE_INFO *prGlueInfo)
{
	...
	kalSnprintf(prGlueInfo->aucMacAddrOverride, MAC_ADDR_LEN,
		"%s", DEFAULT_MAC_ADDRESS);
	...
	kalSnprintf(rootDir, 10, "wifi%d", u4Idx);
...
int32_t sysCreateMonDbgFs(struct GLUE_INFO *prGlueInfo)
{
	...
	if (u4Idx < CFG_MAX_WLAN_DEVICES)
		kalSnprintf(rootDir, 50, "mtk_mon_dbgfs%d", u4Idx);
	else
#endif
		kalSnprintf(rootDir, 50, "mtk_mon_dbgfs");
