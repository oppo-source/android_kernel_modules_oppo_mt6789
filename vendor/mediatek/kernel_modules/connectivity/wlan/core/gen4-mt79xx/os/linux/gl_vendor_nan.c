...
static struct wfpal_channel nanCastChToVendorFormat(
		...
		if (pNanPublishReq->sdea_params.security_cfg
			...
			if (pNanPublishReq->sdea_params.fgGtkRequired) {
				...
				for (szGtkCipherIdx = 0;
					...
					pNanPublishReq->cipher_suite_list)) &&
					(szGtkCipherIdx <
					NAN_MAX_GTK_CIPHER_SUITE_NUM);
					szCipherIdx++, szGtkCipherIdx++) {
					pNanPublishReq->cipher_suite_list
						[szCipherIdx] =
						g_aucNanGtkCipherSuiteList
						[szGtkCipherIdx];
				}
				nanCmdAddCsid(
					prGlueInfo->prAdapter,
					       ...
					       uint8_t *pcuEvtBuf)
{
	...
	/* AP TODO: need to extract u2SdeaServiceControl */
		...
		uint8_t *pcuEvtBuf)
{
	...
	struct wiphy *prWiphy;
	struct wireless_dev *prWdev;
	struct NanVendorFollowupTxIndMsg *prNanFollowupInd;
	struct NAN_VENDOR_FOLLOW_UP_TX_EVENT *prFollowupEvt;
	size_t message_len = 0;
	uint32_t rStatus = WLAN_STATUS_FAILURE;
	...
	prNanFollowupInd->fwHeader.msgVersion = 1;
	prNanFollowupInd->fwHeader.msgId =
			NAN_MSG_ID_VENDOR_FOLLOWUP_TX_IND;
	prNanFollowupInd->fwHeader.msgLen = message_len;
