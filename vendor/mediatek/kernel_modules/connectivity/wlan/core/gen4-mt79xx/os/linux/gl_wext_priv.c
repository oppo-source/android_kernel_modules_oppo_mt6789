				  ...
				  IN char *pcCommand, IN int i4TotalLen)
{
	...
	i4Recv = sscanf(this_char,
		"%hhu-%hhu-%hhu-%hhu-%hx-%hd-%hd-%hx-%hx-%hx-%hd",
		&(rSniffer.ucModule),
		&(rSniffer.ucAction),
		&(rSniffer.ucFilter),
				   ...
				   IN char *pcCommand, IN int i4TotalLen)
{
	...
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
		"t-neptune-main-mt7902-2046-MT7902_E1_ASIC_L_X86_GEN4M-20250324131001-GOOD.xml\nWiFi Driver Version %u.%u.%u %s\n",
			...
			char *pcCommand, int i4TotalLen)
{
	...
	if (ucType == 0) {
		...
		DBGLOG(REQ, INFO, "Test 11mc RTT CMD_LOC_RANGE_MC_ISTA Stop\n");
