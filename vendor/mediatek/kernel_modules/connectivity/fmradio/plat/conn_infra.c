...
static bool drv_set_own(void)
{
	...
	/* wakeup conninfra, need write key(0x464D) after mt6858  */
	drv_host_write(si, ei->reg_map[CONN_INFRA_WAKEPU_FM], 0x464D0001);
...
static bool drv_clr_own(void)
{
	...
	drv_host_write(si, ei->reg_map[CONN_INFRA_WAKEPU_FM], 0x464D0000);
...
static unsigned int drv_get_get_adie(void)
{
	...
	while (adie == 0) {
		if (retry > 100) {
			WCN_DBG(FM_ERR | CHIP, "cannot get adie\n");
			break;
		}
		adie = conninfra_get_ic_info(CONNSYS_ADIE_CHIPID);
		msleep(50);
		retry++;
	}
