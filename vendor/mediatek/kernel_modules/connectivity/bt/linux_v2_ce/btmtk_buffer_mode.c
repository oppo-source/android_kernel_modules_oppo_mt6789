...
static int btmtk_buffer_mode_set_addr(struct btmtk_buffer_mode_struct *buffer_mode)
{
	...
	if (buffer_mode->bt0_mac[0] == 0x00 && buffer_mode->bt0_mac[1] == 0x00
		...
		BTMTK_INFO("%s: SEND BDADDR = %s", __func__, MAC2STR(buffer_mode->bt0_mac, buffer, sizeof(buffer)));
...
{
	u8 cmd[SET_COMPENSATION_CMD_LEN] = {0x01, 0x93, 0xFC, 0x10,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
	u8 event[SET_COMPENSATION_EVT_LEN] = {0x04, 0x0E, 0x04, 0x01, 0x93, 0xFC, 0x00};
	int ret = 0;

	memcpy(&cmd[SET_COMPENSATION_CMD_PAYLOAD_OFFSET],
		buffer_mode->bt0_ant0_compensation,
		BUFFER_MODE_COMPENSATION_LENGTH);

	BTMTK_INFO_RAW(cmd, SET_COMPENSATION_CMD_LEN, "%s: Send", __func__);
	ret = btmtk_main_send_cmd(buffer_mode->bdev,
			cmd, SET_COMPENSATION_CMD_LEN,
			event, SET_COMPENSATION_EVT_LEN,
			DELAY_TIMES, RETRY_TIMES, BTMTK_TX_PKT_FROM_HOST, CMD_NO_NEED_FILTER);
...
int btmtk_buffer_mode_send(struct btmtk_buffer_mode_struct *buffer_mode)
{
	...
	if (buffer_mode->efuse_mode == BIN_FILE_MODE) {
		...
		if (is_mt7961(buffer_mode->bdev->chip_id) && buffer_mode->bdev->flavor) {
...
void btmtk_buffer_mode_initialize(struct btmtk_dev *bdev, struct btmtk_buffer_mode_struct **buffer_mode)
{
	...
	if (ret < 0) {
		...
		if (btmtk_load_code_from_setting_files(BUFFER_MODE_SWITCH_FILE, bdev->intf_dev, &code_len, bdev) != 0) {
			BTMTK_ERR("%s: btmtk_load_code_from_setting_files %s failed!",
				__func__, BUFFER_MODE_SWITCH_FILE);
			return;
		}
	}
