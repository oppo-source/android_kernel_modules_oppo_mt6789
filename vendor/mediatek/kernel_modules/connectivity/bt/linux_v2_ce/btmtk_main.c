...
#if BT_SWIP_SUPPORT
/* LE audio swip*/
static DEFINE_MUTEX(btmtk_vnd_event_cb_mutex);
...
static atomic_t vnd_module_registered = ATOMIC_INIT(0);
char swip_opcode[2];
static atomic_t swip_opcode_flag = ATOMIC_INIT(0);
		...
		u8 *searchcontent, enum debug_reg_index_len index_length)
{
	...
	if (search_result) {
		...
		save_reg->reg = kcalloc(parsing_result, sizeof(struct debug_reg), GFP_KERNEL);
	...
	for (i = 0; i < save_reg->num; i++) {
		...
		if (search_result) {
			...
			do {
				...
				if (search_result == NULL) {
					BTMTK_INFO("%s: search_result is NULL", __func__);
					break;
				}
		...
		if (temp_len && temp_len < DEBUG_REG_NUM) {
			...
			save_reg->reg[i].content = kcalloc(temp_len, sizeof(u32), GFP_KERNEL);
...
static int btmtk_set_audio_slave(struct btmtk_dev *bdev)
{
	...
	ret = btmtk_main_send_cmd(bdev, audio_cmd, AUDIO_SETTING_CMD_LEN,
			audio_event, AUDIO_SETTING_EVT_LEN, 0, 0, BTMTK_TX_PKT_FROM_HOST, CMD_NO_NEED_FILTER);
	if (ret < 0)
		BTMTK_ERR("%s: failed(%d)", __func__, ret);
...
int btmtk_recv_event(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (atomic_read(&vnd_module_registered) == BTMTK_VND_KO_REG) {
		...
		if (atomic_read(&swip_opcode_flag) == 1 &&
			...
			BTMTK_DBG("%s, don't send to host! Opcode=%02x%02x", __func__,
...
static void btmtk_print_bt_patch_info(struct btmtk_dev *bdev, u8 *fwbuf, u32 fwbuf_len)
{
	...
	if (patchHdr) {
		...
		BTMTK_INFO("[btmtk] Built Time = %s", build_time);
...
//	btmtk_do_gettimeofday(&tv_start);
	...
	do {
		...
		if (patch_flag == WIFI_DOWNLOAD) {
			...
			if (patch_flag == ZB_DOWNLOAD && sectionMap->bin_info_spec.u4SecType != 0x1000) {
				BTMTK_INFO("%s: skip download ZB section, type=0x%x", __func__, sectionMap->bin_info_spec.u4SecType);
				continue;
			}
		...
		if (main_info.dma_dl_flag != -1) {
			dma_flag = main_info.dma_dl_flag;
		}
...
int btmtk_data_length_check(u8 *data, u16 length, u8 type)
{
	...
	if (type == HCI_COMMAND_PKT) {
		...
		if (data_len != length) {
			BTMTK_ERR("CMD length check fail, recv len = 0x%04x, data len = 0x%04x", length, data_len);
			return -1;
		}
	} else if (type == HCI_ACLDATA_PKT) {
		data_len = data[2] + ((data[3] << 8) & 0xff00) + MTK_HCI_ACL_HEAD_SIZE;
		if (data_len != length) {
			BTMTK_ERR("ACL length check fail, recv len = 0x%04x, data len = 0x%04x", length, data_len);
			return -1;
		}
		...
		if (data_len != length) {
			BTMTK_ERR("SCO length check fail, recv len = 0x%04x, data len = 0x%04x", length, data_len);
			return -1;
		}
#endif
		...
		if (data_len != length) {
			BTMTK_ERR("EVENT length check fail, recv len = 0x%04x, data len = 0x%04x", length, data_len);
			return -1;
		}
	} else if (type == HCI_ISO_PKT) {
		data_len = data[2] + (((data[3] & 0x3f) << 8) & 0x3f00) + HCI_ISO_PKT_HEADER_SIZE + 1;
		if (data_len != length) {
			BTMTK_ERR("ISO length check fail, recv len = 0x%04x, data len = 0x%04x", length, data_len);
			return -1;
		}
	} else {
		BTMTK_ERR("invalid parameters, return");
		return -1;
	}
	return 0;
}
...
#if CFG_SUPPORT_BT_DL_ZB_PATCH
		if (is_mt7902(bdev->chip_id)) {
			if (main_info.hif_hook.reg_read(bdev, ZB_ENABLE, &zb_enbale) < 0) {
				BTMTK_ERR("read ZB enbale failed");
			} else if ((zb_enbale & (1 << 0)) == 0) {
				if (btmtk_load_rom_patch_79xx(bdev, ZB_DOWNLOAD) < 0)
		...
		u8 *searchcontent, struct btmtk_dev *bdev)
{
	...
	if (ret) {
		...
		BTMTK_INFO("%s: bt_cfg_content->support_dts_subsys_rst = %d", __func__,
	...
	if (ret) {
		if (kstrtoul(text, 10, &text_value) == 0)
			...
			BTMTK_WARN("%s: kstrtoul failed %s!", __func__, BT_LE_AUDIO_CLK_FW_GPIO);
	} else {
		BTMTK_WARN("%s: search item %s is invalid!", __func__, BT_LE_AUDIO_CLK_FW_GPIO);
	}
#endif
	ret = btmtk_parse_bt_cfg_file(BT_LE_AUDIO_CLK_FW_CO_GPIO, text, searchcontent);
	if (ret) {
		if (kstrtoul(text, 10, &text_value) == 0)
			bt_cfg_content->le_audio_clk_fw_co_gpio = text_value;
		else
...
void btmtk_load_bt_cfg(char *cfg_name, struct device *dev, struct btmtk_dev *bdev)
{
	int ret = 0;
	u32 code_len = 0;
...
static int btmtk_send_set_stp1_cmd(struct btmtk_dev *bdev)
{
	...
	ret = btmtk_main_send_cmd(bdev,
			...
			BTMTK_TX_CMD_FROM_DRV, CMD_NO_NEED_FILTER);
...
static int bt_open(struct hci_dev *hdev)
{
	...
	if (!is_mt66xx(bdev->chip_id)) {
		...
		(void)btmtk_buffer_mode_send(bdev->buffer_mode);
...
int btmtk_vendor_module_register(event_callback cb, unsigned char *cfg_file, int file_len)
{
	...
	strncpy(cfg_file, pp_bdev[0]->bt_cfg_file_name, file_len - 1);
	BTMTK_INFO("%s: vnd_module_registered", __func__);
	return 0;
}
...
int btmtk_vendor_module_unregister(void)
{
	...
	BTMTK_INFO("%s: vnd_module_unregistered", __func__);
	return 0;
}
...
int btmtk_send_vendor_cmd(struct sk_buff *skb)
{
	...
	fstate = btmtk_fops_get_state(g_bdev[hci_index]);
	state = btmtk_get_chip_state(g_bdev[hci_index]);
...
send:
	...
	BTMTK_DBG_RAW(skb_temp->data, skb_temp->len, "%s pkt_type = %d", __func__,
	...
	if (skb_temp->len == FW_COREDUMP_CMD_LEN &&
		...
		btmtk_reset_timer_update(g_bdev[hci_index]);
	}

	btmtk_hci_snoop_save(HCI_SNOOP_TYPE_CMD_STACK, skb_temp->data, skb_temp->len);
	ret = main_info.hif_hook.send_cmd(g_bdev[hci_index], skb_temp, 0, 0,
			(int)BTMTK_TX_PKT_FROM_HOST, CMD_NO_NEED_FILTER);
	if (ret < 0)
...
{
	unsigned char fstate = BTMTK_FOPS_STATE_UNKNOWN;
...
int btmtk_main_cif_initialize(struct btmtk_dev *bdev, int hci_bus)
{
	...
	btmtk_buffer_mode_initialize(bdev, &bdev->buffer_mode);
