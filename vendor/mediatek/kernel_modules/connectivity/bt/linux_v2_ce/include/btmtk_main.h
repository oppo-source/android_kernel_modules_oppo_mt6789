...
static inline u8 *MAC2STR(u8 *addr, u8 *buffer, u32 len)
{
#if CFG_SHOW_FULL_MACADDR
	(void)snprintf(buffer, len, "%02X:%02X:%02X:%02X:%02X:%02X", addr[0], addr[1], addr[2], addr[3], addr[4], addr[5]);
#else
	(void)snprintf(buffer, len, "%02X:%02X:**:**:**:%02X", addr[0], addr[1], addr[5]);
#endif
	return buffer;
}
