...
int btmtk_cmd_length_check(u8 *data, u16 length, u8 type)
{
	...
	if (type == HCI_COMMAND_PKT) {
		data_len = data[3] + 4;
		if (data_len != length) {
			BTMTK_DBG("CMD length check fail, recv len = 0x%04x, data len = 0x%04x, type is 0x%02x", length, data_len, HCI_COMMAND_PKT);
			return -1;
		}
	} else if (type == FWLOG_TYPE) {
		data_len = data[1] + ((data[2] << 8) & 0xff00) + 3;
		if (data_len != length) {
			BTMTK_DBG("CMD length check fail, recv len = 0x%04x, data len = 0x%04x, type is 0x%02x", length, data_len, FWLOG_TYPE);
			return -1;
		}
	}
...
ssize_t btmtk_fops_writefwlog(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (copy_from_user(i_fwlog_buf, buf, count) != 0) {
		BTMTK_DBG("%s: Failed to copy data", __func__);
		ret = -ENODATA;
		goto exit;
	}
...
#endif
	if (strncmp(i_fwlog_buf, "le_audio_co_clk=", strlen("le_audio_co_clk=")) == 0) {
		u8 val = *(i_fwlog_buf + strlen("le_audio_co_clk=")) - '0';

		pp_bdev[hci_idx]->bt_cfg.le_audio_clk_fw_co_gpio = val;
		BTMTK_INFO("%s: set le_audio_co_irq to %d", __func__, pp_bdev[hci_idx]->bt_cfg.le_audio_clk_fw_co_gpio);
		ret = count;
		goto exit;
	}

	if (strncmp(i_fwlog_buf, "woble=", strlen("woble=")) == 0) {
		u8 val = *(i_fwlog_buf + strlen("woble=")) - '0';

		if (val) {
			BTMTK_INFO("%s: woble test enable", __func__);
			btmtk_woble_suspend(pp_bdev[hci_idx]->bt_woble);
		} else {
			BTMTK_INFO("%s: woble test disable", __func__);
			btmtk_woble_resume(pp_bdev[hci_idx]->bt_woble);
		}
		ret = count;
		goto exit;
	}
	...
	if (strncmp(i_fwlog_buf, "dma_dl=", strlen("dma_dl=")) == 0) {
		u8 val = *(i_fwlog_buf + strlen("le_audio_clk=")) - '0';
		bmain_info->dma_dl_flag = val;
		BTMTK_INFO("%s: set dma dl to %d", __func__, bmain_info->dma_dl_flag);
		ret = count;
		goto exit;
	}

	if (strncmp(i_fwlog_buf, "cfg reload", strlen("cfg reload")) == 0) {

		btmtk_load_bt_cfg(pp_bdev[hci_idx]->bt_cfg_file_name, pp_bdev[hci_idx]->intf_dev, pp_bdev[hci_idx]);
		BTMTK_INFO("%s: reload bt cfg", __func__);
		ret = count;
		goto exit;
	}
	...
	for (i = 0; i < count; i++) {
		...
		if (*pos == ' ' || *pos == '\t' || *pos == '\r' || *pos == '\n') {
			...
			&& !(*pos >= 'a' && *pos <= 'f')) {
			BTMTK_DBG("%s: There is an invalid input(%c)", __func__, *pos);
			ret = -EINVAL;
			goto exit;
		...
		if (ret == 0)
			...
			BTMTK_DBG("%s: Convert %s failed(%d)", __func__, temp_str, ret);
	}

	if (o_fwlog_buf[0] != HCI_COMMAND_PKT && o_fwlog_buf[0] != FWLOG_TYPE) {
		BTMTK_DBG("%s: Not support 0x%02X yet", __func__, o_fwlog_buf[0]);
		ret = -EPROTONOSUPPORT;
		goto exit;
	}
	...
	if (o_fwlog_buf[0] == FWLOG_TYPE) {
		while (index < ((o_fwlog_buf[2] << 8) + o_fwlog_buf[1])) {
			...
			case FWLOG_TX:    /* tx raw data */
				...
				break;
			default:
				BTMTK_WARN("%s: Invalid opcode(%d)", __func__, o_fwlog_buf[index]);
				ret = -1;
				goto free_skb;
			}
		}
	...
	ret = bmain_info->hif_hook.send_cmd(pp_bdev[hci_idx], skb, 0, 0,
...
long btmtk_fops_unlocked_ioctlfwlog(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case BT_FWLOG_IOC_ON_OFF:
		...
		BTMTK_DBG("[ON_OFF]arg(%lu) bt_on(0x%x) log_on(0x%x) level(0x%x) log_cur(0x%x)",
	...
	case BT_FWLOG_IOC_SET_LEVEL:
		...
		BTMTK_DBG("[SET_LEVEL]arg(%lu) bt_on(0x%x) log_on(0x%x) level(0x%x) log_cur(0x%x)",
				      arg, g_bt_on, g_log_on, g_log_level, g_log_current);
	...
	case BT_FWLOG_IOC_GET_LEVEL:
		retval = g_log_level;
		BTMTK_DBG("[GET_LEVEL]return %ld", retval);
