...
static int btmtk_set_Woble_APCF_filter_parameter(struct btmtk_dev *bdev)
{
	...
	ret = btmtk_main_send_cmd(bdev, cmd, APCF_FILTER_CMD_LEN,
		event, APCF_FILTER_EVT_HDR_LEN, 0, 0, BTMTK_TX_PKT_FROM_HOST, CMD_NO_NEED_FILTER);
	if (ret < 0)
...
static int btmtk_set_Woble_APCF(struct btmtk_woble *bt_woble)
{
	...
	if (bt_woble->woble_setting_apcf[0].length) {
		...
		ret = btmtk_main_send_cmd(bdev, manufactur_data, APCF_CMD_LEN,
			event, APCF_EVT_HDR_LEN, 0, 0, BTMTK_TX_PKT_FROM_HOST, CMD_NO_NEED_FILTER);
		if (ret < 0) {
			BTMTK_ERR("%s: manufactur_data error ret %d", __func__, ret);
			return ret;
		}
...
static int btmtk_set_Woble_Radio_Off(struct btmtk_woble *bt_woble)
{
	...
	if (bt_woble->woble_setting_radio_off.length) {
		 ...
		 * data type:0x07
		 */
		if ((fstate == BTMTK_FOPS_STATE_CLOSED || fstate == BTMTK_FOPS_STATE_INIT) &&
			state == BTMTK_STATE_SUSPEND && (radio_off[WOBX_SEQ_NUM_INDEX] == 0x03 &&
			radio_off[WOBX_SEQ_NUM_INDEX + 1] == 0x40)) {
			length = length - 1;
...
STOP_TRAFFIC:
	if (send_fail == 1 && retry > 0) {
		BTMTK_INFO("%s: woble , reset done flag = %d, send_fail flag = %d",
...
#else
			data = irq_get_irq_data(bt_woble->wobt_irq);
			if (data)
...
#if BT_SWIP_SUPPORT
static void btmtk_check_woble_pin(struct btmtk_dev *bdev)
{
...
int btmtk_woble_initialize(struct btmtk_dev *bdev)
{
	...
	bt_woble = vmalloc(sizeof(struct btmtk_woble));
	if (!bt_woble) {
		BTMTK_ERR("%s: alloc bt_woble fail", __func__);
		return -ENOMEM;
	}
	memset(bt_woble, 0, sizeof(struct btmtk_woble));
	bt_woble->bdev = bdev;
	...
	if (is_support_unify_woble(bdev)) {
		...
		btmtk_load_woble_setting(bt_woble->woble_setting_file_name,
			...
			bt_woble);
#if BT_SWIP_SUPPORT
		if (bdev && bdev->bt_woble)
			btmtk_check_woble_pin(bdev);
#endif
...
void btmtk_woble_uninitialize(struct btmtk_woble *bt_woble)
{
	...
	if (bt_woble == NULL) {
		BTMTK_ERR("%s: bt_woble == NULL", __func__);
		return;
	}
