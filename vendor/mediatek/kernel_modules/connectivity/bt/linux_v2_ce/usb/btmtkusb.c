...
static const struct usb_device_id btusb_table[] = {
	...
	/* Mediatek MT7961 */
	{ USB_DEVICE_AND_INTERFACE_INFO(0x13d3, 0x3583, 0xe0, 0x01, 0x01) },
	/* Mediatek MT7915 */
...
static struct btmtk_usb_sco *g_sco;
...
static int btmtk_usb_sco_pkt_split_send(struct hci_dev *hdev, struct sk_buff *skb)
{
	int i = 0;
	u16 remain = 0;
	u8 iso_pkt_len = 0;
	u8 multiple = 0;
	u8 offset = HCI_SCO_HDR_SIZE;
	struct sk_buff *skb_tmp = NULL;
	struct urb *urb = NULL;
	u8 *buffer = NULL;
	u8 *iso_buf = NULL;

	iso_pkt_len = g_sco->isoc_alt_pkt_size;
	remain = skb->len - HCI_SCO_HDR_SIZE;
	sco_handle = (skb->data[0] + (skb->data[1] << 8));
	BTMTK_INFO("%s handle %d, pkt_len %d, hdr size %d", __func__,
		sco_handle, iso_pkt_len, HCI_SCO_HDR_SIZE);

	if (skb->len/iso_pkt_len < 1 || skb->len == iso_pkt_len) {
	...
	if ((skb->len - HCI_SCO_HDR_SIZE) % (iso_pkt_len - HCI_SCO_HDR_SIZE)) {
		BTMTK_INFO("%s pkt num add one", __func__);
		multiple += 1;
	}
	multiple += (skb->len - HCI_SCO_HDR_SIZE)/(iso_pkt_len - HCI_SCO_HDR_SIZE);
	skb_tmp = alloc_skb(multiple * iso_pkt_len + BT_SKB_RESERVE, GFP_ATOMIC);
	if (!skb_tmp) {
		BTMTK_ERR("%s alloc skb err", __func__);
		return -ENOMEM;
	}
	iso_buf = kmalloc(multiple * iso_pkt_len + 1, GFP_KERNEL);
	if (!iso_buf) {
		BTMTK_ERR("%s alloc iso_buf err", __func__);
		kfree_skb(skb_tmp);
		return -ENOMEM;
	}
	memset(iso_buf, 0, multiple * iso_pkt_len + 1);
	buffer = iso_buf;
	for (i = 0; i < multiple; i++) {
		*(buffer) = (u8)(sco_handle & 0x00FF);
		*(buffer + 1) = sco_handle >> 8;
		*(buffer + 2) = remain < iso_pkt_len - HCI_SCO_HDR_SIZE
			? remain : iso_pkt_len - HCI_SCO_HDR_SIZE;
		remain -= *(buffer + 2);
		memcpy(buffer + 3, skb->data + offset, *(buffer + 2));
		if (remain == 0)
			*(buffer + 2) = iso_pkt_len - HCI_SCO_HDR_SIZE;
		offset += *(buffer + 2);
		buffer += (3 + *(buffer + 2));
	}
	memcpy(skb_tmp->data, iso_buf, multiple * iso_pkt_len);
	skb_tmp->len = multiple * iso_pkt_len;
	urb = alloc_isoc_urb(hdev, skb_tmp);
	if (IS_ERR(urb)) {
		kfree(iso_buf);
		return PTR_ERR(urb);
	}
...
static int btmtk_usb_set_isoc_interface(bool close)
{
	...
	if (sco_handle && close == false) {
		...
		switch (g_sco->isoc_alt_setting) {
		case ISOC_IF_ALT_6:
			g_sco->isoc_alt_pkt_size = ISOC_HCI_PKT_SIZE_ALT6;
			break;
		case ISOC_IF_ALT_4:
			g_sco->isoc_alt_pkt_size = ISOC_HCI_PKT_SIZE_ALT4;
			break;
		case ISOC_IF_ALT_2:
			g_sco->isoc_alt_pkt_size = ISOC_HCI_PKT_SIZE_ALT2;
			break;
		default:
			g_sco->isoc_alt_pkt_size = ISOC_HCI_PKT_SIZE_ALT2;
			BTMTK_INFO("%s: unknown alt(%d), use default size",
				__func__, g_sco->isoc_alt_pkt_size);
			break;
		}
	...
	} else if (sco_handle == 0 || close == true) {
		u8 count = 0;
		...
		BTMTK_INFO("%s: sco: 0x%04X",
				__func__, sco_handle);
	}
	return 0;
}

static int btmtk_usb_sco_init(struct btmtk_dev *bdev)
{
	if (g_sco == NULL) {
...
static int btmtk_usb_sco_exit(struct btmtk_dev *bdev)
{
	kfree(g_sco);
	g_sco = NULL;
...
static void btusb_ble_isoc_complete(struct urb *urb)
{
	...
	if (urb->status == 0) {
		...
		if (isoc_pkt_len > bdev->iso_threshold) {
			BTMTK_ERR("%s iso_pkt_len(%d) > iso_threshold(%d), discard the packet!!!",
				__func__, isoc_pkt_len, bdev->iso_threshold);
			BTMTK_INFO_RAW(urb->transfer_buffer,
				urb->actual_length,
		...
		// Not transfer iso type to acl type
		if (urb->actual_length > URB_MAX_BUFFER_SIZE) {
			BTMTK_ERR("%s urb->actual_length is invalid!", __func__);
			btmtk_hci_snoop_print(urb->transfer_buffer, urb->actual_length);
			goto ble_iso_resub;
		...
		cif_dev->urb_ble_isoc_buf[0] = HCI_ISO_PKT;
		memcpy(cif_dev->urb_ble_isoc_buf + 1, urb->transfer_buffer, isoc_pkt_len);

		BTMTK_DBG_RAW(cif_dev->urb_ble_isoc_buf,
			isoc_pkt_len + 1, "%s: raw data is :", __func__);

		err = btmtk_recv(hdev, cif_dev->urb_ble_isoc_buf, isoc_pkt_len + 1);
#else
...
#if BTMTK_ISOC_TEST
	err = btmtk_usb_sco_init(bdev);
	if (err < 0) {
		BTMTK_ERR("[ERR] init sco failed!");
...
static int submit_tx_urb(struct hci_dev *hdev, struct urb *urb, int type)
{
	...
	case HCI_SCODATA_PKT:
		btmtk_hci_snoop_save(HCI_SNOOP_TYPE_TX_SCO_HIF,
			urb->transfer_buffer, urb->transfer_buffer_length);
		break;
	case HCI_ISO_PKT:
...
#if (KERNEL_VERSION(5, 6, 0) < LINUX_VERSION_CODE)
static int btusb_send_iso_frame(struct hci_dev *hdev, struct sk_buff *skb)
{
	struct urb *urb = NULL;
	struct btmtk_dev *bdev = hci_get_drvdata(hdev);
	...
	unsigned char *skb_tmp = NULL;
	int ret = 0;
	int isoc_pkt_padding = bdev->iso_threshold - skb->len;
...
static int btusb_send_frame(struct hci_dev *hdev, struct sk_buff *skb, bool flag)
{
	...
	case HCI_ACLDATA_PKT:
		if (skb->data[0] == 0x00 && skb->data[1] == 0x44) {
			...
			if (cif_dev->iso_channel == 0 || bdev->iso_threshold == 0) {
...
exit:
	...
	{
		ret = btusb_send_iso_frame(hdev, skb);
		return ret;
	}
#endif
...
#if BTMTK_ISOC_TEST
		/* For sco loop back test, if sco packet size
		 * is an integer multiple of isoc_alt_pkt_size
		 * we can directly send skb to firmware, otherwise
		 * we need split packet
		 */
		if (!(skb->len % g_sco->isoc_alt_pkt_size)) {
			BTMTK_INFO("%s pkt no need split", __func__);
			goto send_sco_data;
		}
		return btmtk_usb_sco_pkt_split_send(hdev, skb);
...
static inline int __set_isoc_interface(struct hci_dev *hdev, int altsetting)
{
	...
	BTMTK_INFO("setting interface alt = %d, interface = %d", altsetting, cif_dev->new_isoc_altsetting_interface);
...
#if BTMTK_ISOC_TEST
/* Add code for vts sco loop back mode test, when sco link
 * set up, usb alternate setting should be set afterwards
 * and loop back test need send 3 tci cmd to firmware
 */
static void btmtk_set_sco_work(struct sk_buff *skb, struct btmtk_dev *bdev)
{
	int ret = 0;
	struct sk_buff *skb_tmp = NULL;

	u8 mtk_tci_sco_cmd[TCI_MTK_USB_SCO_NUM_CMD_LEN] = {0x01, 0xF8, 0xFC, 0x01, 0x01};
	u8 mtk_tci_usb_cmd1[TCI_MTK_USB_SCO_CMD_LEN] = {0x01, 0xF7, 0xFC, 0x04, 0x02, 0x00, 0x01, 0x01};
	u8 mtk_tci_usb_cmd2[TCI_MTK_USB_SCO_CMD_LEN] = {0x01, 0xF7, 0xFC, 0x04, 0x02, 0x00, 0x00, 0x00};
	sco_handle = skb->data[3] + (skb->data[4] << 8);
	g_sco->isoc_alt_setting = ISOC_IF_ALT_6;
	test_and_set_bit(BTUSB_ISOC_RUNNING, &bdev->flags);


	skb_tmp = alloc_skb(TCI_MTK_USB_SCO_CMD_LEN + BT_SKB_RESERVE,
		GFP_ATOMIC);
	if (!skb_tmp) {
		BTMTK_ERR("%s allocate skb failed!!", __func__);
		return;
	}
	/* send tci cmd before switch usb intf alt*/
	memcpy(skb_tmp->data, mtk_tci_usb_cmd1, TCI_MTK_USB_SCO_CMD_LEN);
	skb_tmp->len = TCI_MTK_USB_SCO_CMD_LEN;
	hci_skb_pkt_type(skb_tmp) = HCI_COMMAND_PKT;
	ret = btusb_send_frame(bdev->hdev, skb_tmp, 0);
	if (ret < 0) {
		BTMTK_ERR("%s send tci usb cmd 1 failed!!", __func__);
		return;
	}

	skb_tmp = alloc_skb(TCI_MTK_USB_SCO_NUM_CMD_LEN + BT_SKB_RESERVE,
		GFP_ATOMIC);
	if (!skb_tmp) {
		BTMTK_ERR("%s allocate skb failed!!", __func__);
		return;
	}
	memcpy(skb_tmp->data, mtk_tci_sco_cmd, TCI_MTK_USB_SCO_NUM_CMD_LEN);
	skb_tmp->len = TCI_MTK_USB_SCO_NUM_CMD_LEN;
	hci_skb_pkt_type(skb_tmp) = HCI_COMMAND_PKT;
	ret = btusb_send_frame(bdev->hdev, skb_tmp, 0);
	if (ret < 0) {
		BTMTK_ERR("%s send tci sco num cmd failed!!", __func__);
		return;
	}
	...
	memcpy(skb_tmp->data, mtk_tci_usb_cmd2, TCI_MTK_USB_SCO_CMD_LEN);
	skb_tmp->len = TCI_MTK_USB_SCO_CMD_LEN;
	hci_skb_pkt_type(skb_tmp) = HCI_COMMAND_PKT;
	ret = btusb_send_frame(bdev->hdev, skb_tmp, 0);
	if (ret < 0) {
		BTMTK_ERR("%s send tci usb cmd 2 failed!!", __func__);
		return;
	}
	/* Set USB alternate setting */
	ret = btmtk_usb_set_isoc_interface(false);
	if (ret < 0) {
		BTMTK_INFO("%s set interface err %d", __func__, ret);
		return;
	}
	return;
}

static int btmtk_handle_sco_work(struct sk_buff *skb, struct btmtk_dev *bdev)
{
	int ret = 0;
	u8 evt_tci_usb_power[TCI_MTK_USB_SCO_EVT_LEN] = { 0x0E, 0x04, 0x01, 0xF7, 0xFC, 0x00 };
	u8 evt_tci_usb_sco_num[TCI_MTK_USB_SCO_EVT_LEN] = { 0x0E, 0x04, 0x01, 0xF8, 0xFC, 0x00 };

	if (skb->data[1] == 0x0B &&
		(skb->data[3] + (skb->data[4] << 8) == 0x0E00)) {
		BTMTK_INFO("%s set sco work", __func__);
		btmtk_set_sco_work(skb, bdev);
	} else if (skb->len == TCI_MTK_USB_SCO_EVT_LEN &&
		(memcmp(skb->data, evt_tci_usb_power, TCI_MTK_USB_SCO_EVT_LEN) == 0)) {
		BTMTK_INFO("%s tci usb sco disable low power event, drop it!", __func__);
		ret = 1;
	} else if (skb->len == TCI_MTK_USB_SCO_EVT_LEN &&
		(memcmp(skb->data, evt_tci_usb_sco_num, TCI_MTK_USB_SCO_EVT_LEN) == 0)) {
		BTMTK_INFO("%s tci usb sco num event, drop it!", __func__);
		ret = 1;
	}
	return ret;
}
...
static struct usb_host_interface *btusb_find_altsetting(struct btmtk_dev *bdev,
		int alt)
{
	struct btmtk_usb_dev *cif_dev = (struct btmtk_usb_dev *)bdev->cif_dev;
	struct usb_interface *intf = cif_dev->isoc;
...
static void btusb_work(struct work_struct *work)
{
	...
	if (bdev->sco_num > 0) {
...
#if (CFG_SUPPORT_DVT == 0 || CFG_SUPPORT_HW_DVT == 0 || CFG_SUPPORT_BLUEZ == 1)
		if (bdev->isoc_altsetting == HCI_NOTIFY_ENABLE_SCO_CVSD) {
		...
		} else if (bdev->isoc_altsetting == HCI_NOTIFY_ENABLE_SCO_TRANSP) {
			if (btusb_find_altsetting(bdev, 6))
				new_alts = 6;
			else if (btusb_find_altsetting(bdev, 3) &&
				hdev->sco_mtu >= 72)
				new_alts = 3;
			else
				new_alts = 1;
...
#else
		/* Boots test sco loopback need set altsetting */
		new_alts = cif_dev->new_isoc_altsetting;
#endif /* CFG_SUPPORT_DVT == 0 || CFG_SUPPORT_HW_DVT == 0 || CFG_SUPPORT_BLUEZ ==1 */
		...
		if (!test_and_set_bit(BTUSB_ISOC_RUNNING, &bdev->flags)) {
			if (btusb_submit_isoc_urb(hdev, GFP_KERNEL) < 0) {
				...
				BTMTK_INFO("%s submit isoc urb", __func__);
				return;
			}
		}
	} else {
...
static int btmtk_usb_subsys_reset(struct btmtk_dev *bdev)
{
	...
	if (bdev->bt_cfg.support_dts_subsys_rst) {
		...
		if (bdev->chip_id == 0x7961 || bdev->chip_id == 0x7902) {
			btmtk_cif_read_uhw_register(bdev, BT_BACKDOOR_RET, &val);
			BTMTK_INFO("%s: read 1 BT_BACKDOOR_RET CR : 0x%08x", __func__, val);
			val |= 0x30000;
			BTMTK_INFO("%s: write 1 BT_BACKDOOR_RET CR : 0x%08x", __func__, val);
			btmtk_cif_write_uhw_register(bdev, BT_BACKDOOR_RET, val);
		}
		msleep(20);
		if (bdev->chip_id == 0x7961 || bdev->chip_id == 0x7902) {
			btmtk_cif_read_uhw_register(bdev, BT_BACKDOOR_RET, &val);
			BTMTK_INFO("%s: read 2 BT_BACKDOOR_RET CR : 0x%08x", __func__, val);
			val &= 0xFFFCFFFF;
			BTMTK_INFO("%s: write 2 BT_BACKDOOR_RET CR : 0x%08x", __func__, val);
			btmtk_cif_write_uhw_register(bdev, BT_BACKDOOR_RET, val);
		}
		...
		/* Read reset CR */
		ret = btmtk_cif_read_uhw_register(bdev, bt_subsys_cr, &val);
		if (ret == -ENODEV) {
			BTMTK_INFO("%s, now usb disconnect happen, stop subsys chip reset", __func__);
			return ret;
		}
	}
	...
	do {
		/* polling re-init CR */
		ret = btmtk_cif_read_uhw_register(bdev, BT_MISC, &val);
		if (ret == -ENODEV) {
			BTMTK_INFO("%s, usb disconnect happen, stop subsys chip reset", __func__);
			return ret;
		}
...
static int btmtk_usb_enter_standby(void)
{
	...
	for (i = 0; i < intf_num; i++) {
		...
		if (pp_bdev[i] == NULL || pp_bdev[i]->hdev == NULL)
			continue;
		...
		if (cif_dev->udev->speed > USB_SPEED_HIGH && ret == 0) {
			if (is_mt7961(bdev->chip_id)) {
				btmtk_handle_mutex_unlock(bdev);
				ret = btmtk_usb_read_register(bdev, BT_RADIO_OFF_DONE, &val);
				if (ret < 0)
					...
					ret = btmtk_usb_write_register(bdev, BT_RADIO_OFF_DONE, val);
					if (ret < 0)
...
int btmtk_usb_event_filter(struct btmtk_dev *bdev, struct sk_buff *skb)
{
	 ...
	 * set up, usb alternate setting should be set afterwards
	 */
	if (btmtk_handle_sco_work(skb, bdev))
		return 1;
#endif
