...
int main(int argc, char *argv[])
{
    ...
    /* SIGSTOP may not be catched */
    err = sigaction(SIGKILL, &sigact, NULL);
    if (!err)
        BPRINT_E("sigaction failed, err = %d", err);
    ...
    while ((opt = getopt(argc, argv, "c:f:p:k:l:")) != -1) {
        switch (opt) {
            ...
            /* flow control */
            case 'f':
                flow_control = strtol(optarg, NULL, 10);
