		...
		const int cmd_len, const uint8_t *event, const int event_len, u32 *value)
{
	...
	*value = (bdev->io_buf[READ_PINMUX_EVT_REAL_LEN - 1] << 24) +
	...
	BTMTK_INFO_DEV(bdev, "%s, value=0x%08x", __func__, *value);
	return ret;
}
		...
		int cmd_len, const uint8_t *event, const int event_len, u32 value)
{
	...
	BTMTK_INFO_DEV(bdev, "%s begin, value = 0x%08x", __func__, value);
...
static int btmtk_set_audio_pin_mux_7961(struct btmtk_dev *bdev)
{
	...
	for (i = 0; i < PINMUX_REG_NUM; i++) {
		...
		if (ret) {
			BTMTK_ERR_DEV(bdev, "%s, btmtk_read_pin_mux_setting error(%d)", __func__, ret);
			goto exit;
		}
		if (i == 0) {
			pinmux &= 0x00FFFFFF;
		...
		if (ret) {
			BTMTK_ERR_DEV(bdev, "%s, btmtk_write_pin_mux_setting error(%d)", __func__, ret);
			goto exit;
		}
		...
		BTMTK_INFO_DEV(bdev, "%s, confirm pinmux register 0x%02x pinmux 0x%08x", __func__,
...
static int btmtk_subsys_reset(struct btmtk_dev *bdev)
{
	...
	bdev->main_info.hif_hook.read_uhw_register(bdev, BT_BDR_RET_7961, &val);
	BTMTK_INFO_DEV(bdev, "%s: read 1 BT_BDR_RET_7961 CR : 0x%08x", __func__, val);
	val |= 0x30000;
	BTMTK_INFO_DEV(bdev, "%s: write 1 BT_BDR_RET_7961 CR : 0x%08x", __func__, val);
	bdev->main_info.hif_hook.write_uhw_register(bdev, BT_BDR_RET_7961, val);

	msleep(20);
	bdev->main_info.hif_hook.read_uhw_register(bdev, BT_BDR_RET_7961, &val);
	BTMTK_INFO_DEV(bdev, "%s: read 2 BT_BDR_RET_7961 CR : 0x%08x", __func__, val);
	val &= 0xFFFCFFFF;
	BTMTK_INFO_DEV(bdev, "%s: write 2 BT_BDR_RET_7961 CR : 0x%08x", __func__, val);
	bdev->main_info.hif_hook.write_uhw_register(bdev, BT_BDR_RET_7961, val);
...
int btmtk_cif_chip_7961_register(struct btmtk_dev *bdev)
{
	int retval = 0;
	...
	bdev->main_info.hif_hook_chip.bt_set_pinmux = btmtk_set_audio_pin_mux_7961;
	bdev->main_info.hif_hook_chip.get_fw_info = btmtk_get_fw_info_7961;
	bdev->main_info.hif_hook_chip.patched = 1;
