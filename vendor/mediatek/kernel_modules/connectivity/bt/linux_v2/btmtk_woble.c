...
{
	if (!DONGLE_FILE_IS_VALID(bdev, bdev->bt_woble->woble_setting_file_idx)) {
		BTMTK_INFO_DEV(bdev, "%s: file is valid RETURN", __func__);
		return 0;
	}

	if (bdev->bt_cfg.support_unify_woble)
		return bdev->main_info.hif_hook_chip.support_woble;
	else
		return 0;
}
...
void btmtk_woble_wake_lock(struct btmtk_dev *bdev)
{
	...
	if (bdev->bt_cfg.support_woble_wakelock) {
		...
		__pm_stay_awake(bdev->main_info.woble_ws);
...
void btmtk_woble_wake_unlock(struct btmtk_dev *bdev)
{
	...
	if (bdev->bt_cfg.support_woble_wakelock) {
		BTMTK_INFO_DEV(bdev, "%s: enter", __func__);
		__pm_relax(bdev->main_info.woble_ws);
		BTMTK_INFO_DEV(bdev, "%s: exit", __func__);
	}
}
...
int btmtk_send_apcf_reserved(struct btmtk_dev *bdev)
{
	...
	if (is_support_unify_woble(bdev)) {
		...
		BTMTK_WARN_DEV(bdev, "%s: not support for 0x%x", __func__, bdev->chip_id);
...
static int btmtk_load_woble_setting(char *bin_name,
		struct device *dev, struct st_woble *bt_woble)
{
	int code_len = 0;
	int err;
	struct btmtk_dev *bdev = NULL;

	if (bt_woble == NULL || bt_woble->bdev == NULL) {
	...
	if (err) {
		BTMTK_ERR_DEV(bdev, "woble_setting btmtk_load_code_from_setting_files failed!!");
		goto LOAD_END;
	}

	err = btmtk_load_fw_cfg_setting(bdev, "APCF",
			bt_woble->woble_setting_apcf, WOBLE_SETTING_COUNT, bdev->setting_file, FW_CFG_INX_LEN_2);
	if (err)
		goto LOAD_END;

	err = btmtk_load_fw_cfg_setting(bdev, "APCF_ADD_MAC",
			bt_woble->woble_setting_apcf_fill_mac, WOBLE_SETTING_COUNT,
			bdev->setting_file, FW_CFG_INX_LEN_2);
	if (err)
		goto LOAD_END;

	err = btmtk_load_fw_cfg_setting(bdev, "APCF_ADD_MAC_LOCATION",
			bt_woble->woble_setting_apcf_fill_mac_location, WOBLE_SETTING_COUNT,
			bdev->setting_file, FW_CFG_INX_LEN_2);
	if (err)
		goto LOAD_END;

	err = btmtk_load_fw_cfg_setting(bdev, "RADIOOFF", &bt_woble->woble_setting_radio_off, 1,
			bdev->setting_file, FW_CFG_INX_LEN_2);
	if (err)
		goto LOAD_END;
	...
	case 0:
		err = btmtk_load_fw_cfg_setting(bdev, "WAKEUP_TYPE_LEGACY", &bt_woble->woble_setting_wakeup_type, 1,
			bdev->setting_file, FW_CFG_INX_LEN_2);
		break;
	case 1:
		err = btmtk_load_fw_cfg_setting(bdev, "WAKEUP_TYPE_WAVEFORM", &bt_woble->woble_setting_wakeup_type, 1,
			bdev->setting_file, FW_CFG_INX_LEN_2);
		break;
	case 2:
		err = btmtk_load_fw_cfg_setting(bdev, "WAKEUP_TYPE_IR", &bt_woble->woble_setting_wakeup_type, 1,
			bdev->setting_file, FW_CFG_INX_LEN_2);
		break;
	default:
		BTMTK_WARN_DEV(bdev, "%s: unify_woble_type unknown(%d)", __func__, bdev->bt_cfg.unify_woble_type);
	}
	if (err)
		BTMTK_WARN_DEV(bdev, "%s: Parse unify_woble_type(%d) failed", __func__, bdev->bt_cfg.unify_woble_type);

	err = btmtk_load_fw_cfg_setting(bdev, "RADIOOFF_STATUS_EVENT",
			&bt_woble->woble_setting_radio_off_status_event, 1, bdev->setting_file, FW_CFG_INX_LEN_2);
	if (err)
		goto LOAD_END;

	err = btmtk_load_fw_cfg_setting(bdev, "RADIOOFF_COMPLETE_EVENT",
			&bt_woble->woble_setting_radio_off_comp_event, 1, bdev->setting_file, FW_CFG_INX_LEN_2);
	if (err)
		goto LOAD_END;

	err = btmtk_load_fw_cfg_setting(bdev, "RADIOON",
			&bt_woble->woble_setting_radio_on, 1, bdev->setting_file, FW_CFG_INX_LEN_2);
	if (err)
		goto LOAD_END;

	err = btmtk_load_fw_cfg_setting(bdev, "RADIOON_STATUS_EVENT",
			&bt_woble->woble_setting_radio_on_status_event, 1, bdev->setting_file, FW_CFG_INX_LEN_2);
	if (err)
		goto LOAD_END;

	err = btmtk_load_fw_cfg_setting(bdev, "RADIOON_COMPLETE_EVENT",
			&bt_woble->woble_setting_radio_on_comp_event, 1, bdev->setting_file, FW_CFG_INX_LEN_2);
	if (err)
		goto LOAD_END;

	err = btmtk_load_fw_cfg_setting(bdev, "APCF_RESUME",
...
{
	BTMTK_INFO_DEV(bdev, "%s: enter", __func__);
	__pm_wakeup_event(bdev->main_info.irq_ws, WAIT_POWERKEY_TIMEOUT);
	BTMTK_INFO_DEV(bdev, "%s: exit", __func__);
}
#else
static int btmtk_send_woble_read_BDADDR_cmd(struct btmtk_dev *bdev)
{
	...
	/* if (!DONGLE_FILE_IS_VALID(bdev, bdev->bt_woble->woble_setting_file_idx)) {
	 *	BTMTK_INFO_DEV(bdev, "%s: is valid file return ", __func__);
	 *	return 0;
	 * } cc_temp
	 */
...
static int btmtk_send_woble_suspend_cmd(struct btmtk_dev *bdev)
{
	...
	BTMTK_INFO_DEV(bdev, "%s: not support woble, send radio off cmd", __func__);
	ret = btmtk_send_cmd_to_fw(bdev,
...
static int btmtk_set_Woble_APCF_filter_parameter(struct btmtk_dev *bdev)
{
	...
	ret = btmtk_send_cmd_to_fw(bdev,
		...
		0, 0, BTMTK_TX_PKT_FROM_HOST, CMD_NO_NEED_FILTER);
	if (ret < 0)
		BTMTK_ERR_DEV(bdev, "%s: end ret %d", __func__, ret);
	else
		ret = 0;
 ...
 */
static int btmtk_set_Woble_APCF(struct st_woble *bt_woble)
{
	int ret = 0;
	u8 i = 0;
	...
	BTMTK_INFO_DEV(bdev, "%s: woble_setting_apcf[0].length %d",
	...
	if (bt_woble->woble_setting_apcf[0].length) {
		for (i = 0; i < WOBLE_SETTING_COUNT; i++) {
			...
			BTMTK_INFO_DEV(bdev, "%s: apcf_fill_mac[%d].content[0] = 0x%02x", __func__, i,
					bt_woble->woble_setting_apcf_fill_mac[i].content[0]);
			BTMTK_INFO_DEV(bdev, "%s: apcf_fill_mac_location[%d].length = %d", __func__, i,
			...
			if ((bt_woble->woble_setting_apcf_fill_mac[i].content[0] == 1) &&
				...
				BTMTK_INFO_DEV(bdev, "%s: apcf[%d], add local BDADDR to location %d", __func__, i,
						(*bt_woble->woble_setting_apcf_fill_mac_location[i].content));
			}
#if CFG_SHOW_FULL_MACADDR
			BTMTK_INFO_RAW(bdev,
				bt_woble->woble_setting_apcf[i].content,
				bt_woble->woble_setting_apcf[i].length,
			...
			if (ret < 0) {
				BTMTK_ERR_DEV(bdev, "%s: manufactur_data error ret %d", __func__, ret);
				return ret;
			}
		}
	} else { /* use default */
		BTMTK_INFO_DEV(bdev, "%s: use default manufactur data", __func__);
...
static int btmtk_set_Woble_Radio_Off(struct st_woble *bt_woble)
{
	int ret = 0;
	int length = 0;
	...
	BTMTK_INFO_DEV(bdev, "%s: woble_setting_radio_off.length %d", __func__,
	...
	if (bt_woble->woble_setting_radio_off.length) {
		...
		if (!radio_off) {
			BTMTK_ERR_DEV(bdev, "%s: alloc memory fail (radio_off)",
				__func__);
			ret = -ENOMEM;
			goto Finish;
		}
		...
		BTMTK_INFO_RAW(bdev, radio_off, length, "Send radio off");
		...
		BTMTK_INFO_DEV(bdev, "%s: use default radio off cmd", __func__);
...
static int btmtk_set_Woble_Radio_On(struct st_woble *bt_woble)
{
	int ret = 0;
	struct btmtk_dev *bdev = bt_woble->bdev;
	...
	BTMTK_INFO_DEV(bdev, "%s: woble_setting_radio_on.length %d", __func__,
	...
	if (bt_woble->woble_setting_radio_on.length) {
		...
		BTMTK_INFO_RAW(bdev, bt_woble->woble_setting_radio_on.content,
		...
		BTMTK_WARN_DEV(bdev, "%s: use default radio on cmd", __func__);
...
static int btmtk_del_Woble_APCF_index(struct btmtk_dev *bdev)
{
	...
	ret = btmtk_send_cmd_to_fw(bdev,
		...
		0, 0, BTMTK_TX_PKT_FROM_HOST, CMD_NO_NEED_FILTER);
	if (ret < 0)
		BTMTK_ERR_DEV(bdev, "%s: got error %d", __func__, ret);
...
static int btmtk_set_Woble_APCF_Resume(struct st_woble *bt_woble)
{
	...
	BTMTK_INFO_DEV(bdev, "%s, enter, bt_woble->woble_setting_apcf_resume[0].length= %d",
	...
	if (bt_woble->woble_setting_apcf_resume[0].length) {
		BTMTK_INFO_DEV(bdev, "%s: handle leave woble apcf from file", __func__);
		for (i = 0; i < WOBLE_SETTING_COUNT; i++) {
			if (!bt_woble->woble_setting_apcf_resume[i].length)
				continue;

			BTMTK_INFO_RAW(bdev, bt_woble->woble_setting_apcf_resume[i].content,
			...
			ret = btmtk_main_send_cmd(bdev,
				...
				0, 0, BTMTK_TX_PKT_FROM_HOST, CMD_NO_NEED_FILTER);
			if (ret < 0) {
				BTMTK_ERR_DEV(bdev, "%s: Send apcf resume fail %d", __func__, ret);
				return ret;
			}
		}
	} else { /* use default */
		BTMTK_WARN_DEV(bdev, "%s: use default apcf resume cmd", __func__);
		ret = btmtk_del_Woble_APCF_index(bdev);
		if (ret < 0)
			BTMTK_ERR_DEV(bdev, "%s: btmtk_del_Woble_APCF_index return fail %d", __func__, ret);
	}
	BTMTK_INFO_DEV(bdev, "%s, end", __func__);
	return ret;
}
...
static int btmtk_handle_leaving_WoBLE_state(struct st_woble *bt_woble)
{
#if !WAKEUP_BT_IRQ
	int ret = 0;
	unsigned char fstate = 0;
	...
	BTMTK_INFO_DEV(bdev, "not support woble mode for wakeup bt irq");
	return 0;
#else
	fstate = btmtk_fops_get_state(bdev);
	if (!bdev->bt_cfg.support_woble_for_bt_disable) {
		if (fstate != BTMTK_FOPS_STATE_OPENED) {
			BTMTK_WARN_DEV(bdev, "%s: fops is not opened, return", __func__);
			return 0;
		}
	}

	if (fstate != BTMTK_FOPS_STATE_OPENED) {
		BTMTK_WARN_DEV(bdev, "%s: fops is not open yet(%d), need to start traffic before leaving woble",
				__func__, fstate);
...
Finish:
	if (ret < 0) {
		BTMTK_INFO_DEV(bdev, "%s: woble_resume_fail!!!", __func__);
	} else {
		...
		if (fstate != BTMTK_FOPS_STATE_OPENED) {
			...
			BTMTK_WARN_DEV(bdev, "%s: fops is not open(%d), need to stop traffic after leaving woble",
					__func__, fstate);
...
static int ATTRIBUTE_NO_KCFI btmtk_handle_entering_WoBLE_state(struct st_woble *bt_woble)
{
#if !WAKEUP_BT_IRQ
	int ret = 0;
	unsigned char fstate = 0;
	...
	if (!bdev->bt_cfg.support_woble_for_bt_disable) {
		if (fstate != BTMTK_FOPS_STATE_OPENED) {
			BTMTK_WARN_DEV(bdev, "%s: fops is not open yet(%d)!, return", __func__, fstate);
			return 0;
		}
	}
	...
	if (atomic_read(&bdev->main_info.chip_reset) || atomic_read(&bdev->main_info.subsys_reset)) {
		BTMTK_ERR_DEV(bdev, "%s chip_reset is %d, subsys_reset is %d", __func__,
			atomic_read(&bdev->main_info.chip_reset), atomic_read(&bdev->main_info.subsys_reset));
		goto Finish;
	}
...
SEND_CMD_RETRY:
	...
	if (ret < 0) {
		BTMTK_ERR_DEV(bdev, "%s: reset power_on fail return", __func__);
		goto Finish;
	}

	if (fstate != BTMTK_FOPS_STATE_OPENED) {
		BTMTK_WARN_DEV(bdev, "%s: fops is not open yet(%d), need to start traffic before enter woble",
				__func__, fstate);
		/* start traffic to recv event*/
		ret = bdev->main_info.hif_hook.open(bdev->hdev);
		if (ret < 0 && ret != -EBUSY) {
	...
	if (is_support_unify_woble(bdev)) {
		...
		do {
			...
			if (pFunc && bdev->bt_cfg.unify_woble_type == 1) {
				...
				BTMTK_INFO_DEV(bdev, "%s: Invoke %s, buf = %d, sret = %zd", __func__,
					func_name, buf, sret);

			} else {
				BTMTK_WARN_DEV(bdev, "%s: No Exported Func Found [%s]", __func__, func_name);
			}
		} while (0);
#endif
...
STOP_TRAFFIC:
	if (send_fail == 1 && retry > 0) {
		BTMTK_INFO_DEV(bdev, "%s: woble , reset done flag = %d, send_fail flag = %d",
					__func__, test_bit(BT_WOBLE_RESET_DONE, &bdev->flags), send_fail);
		...
		if (!test_bit(BT_WOBLE_RESET_DONE, &bdev->flags)) {
			BTMTK_ERR_DEV(bdev, "%s: wait event timeout", __func__);
			goto Finish;
		}
		...
		if (test_bit(BT_WOBLE_SKIP_WHOLE_CHIP_RESET, &bdev->flags)) {
			...
			BTMTK_ERR_DEV(bdev, "%s: L0.5 fail, return", __func__);
			goto Finish;
		}
		BTMTK_INFO_DEV(bdev, "%s: try send cmd, retry = %d", __func__, retry);
		goto SEND_CMD_RETRY;

	}
	if (fstate != BTMTK_FOPS_STATE_OPENED) {
		BTMTK_WARN_DEV(bdev, "%s: fops is not open(%d), need to stop traffic after enter woble",
				__func__, fstate);
		/* stop traffic to stop recv data from fw*/
		ret = bdev->main_info.hif_hook.close(bdev->hdev);
		if (ret < 0) {
...
Finish:
	...
	BTMTK_INFO_DEV(bdev, "%s: end ret = %d, power_state =%d", __func__, ret, bdev->power_state);
	return ret;
#endif
}

int btmtk_woble_suspend(struct st_woble *bt_woble)
{
	int ret = 0;
	unsigned char fstate = 0;
	struct btmtk_dev *bdev = NULL;
	...
	if (!is_support_unify_woble(bdev) && (fstate != BTMTK_FOPS_STATE_OPENED)) {
		BTMTK_WARN_DEV(bdev, "%s: when not support woble, in bt off state, do nothing!", __func__);
		goto exit;
	}

	ret = btmtk_handle_entering_WoBLE_state(bt_woble);
	if (ret)
		BTMTK_ERR_DEV(bdev, "%s: btmtk_handle_entering_WoBLE_state return fail %d", __func__, ret);
	...
	if (bdev->bt_cfg.support_woble_by_eint) {
		if (bt_woble->wobt_irq != 0 && atomic_read(&(bt_woble->irq_enable_count)) == 0) {
			...
			if (irq_get_irq_data(bt_woble->wobt_irq))
				...
				BTMTK_INFO_DEV(bdev, "%s:irq_get_irq_data return is NULL", __func__);
#endif
			if (desc)
				desc->irq_data.chip->irq_ack(&desc->irq_data);
			else
				BTMTK_INFO_DEV(bdev, "%s:can't get desc", __func__);

			BTMTK_INFO_DEV(bdev, "%s, enable BT IRQ:%d", __func__, bt_woble->wobt_irq);
			irq_set_irq_wake(bt_woble->wobt_irq, 1);
			enable_irq(bt_woble->wobt_irq);
			atomic_inc(&(bt_woble->irq_enable_count));
		} else
			BTMTK_INFO_DEV(bdev, "%s, irq_enable count:%d",
						__func__, atomic_read(&(bt_woble->irq_enable_count)));
	}
...
int btmtk_woble_resume(struct st_woble *bt_woble)
{
	int ret = 0;
	unsigned char fstate = 0;
	struct btmtk_dev *bdev = NULL;
	...
	bdev = bt_woble->bdev;
	if (!bdev) {
		BTMTK_INFO_DEV(bdev, "%s: bdev is NULL", __func__);
		return 0;
	}
	...
	if (bdev->power_state == BTMTK_DONGLE_STATE_ERROR) {
		BTMTK_INFO_DEV(bdev, "%s: In BTMTK_DONGLE_STATE_ERROR(Could suspend caused), do assert", __func__);
	...
	if (ret < 0) {
		BTMTK_ERR_DEV(bdev, "%s: btmtk_handle_leaving_WoBLE_state return fail %d", __func__, ret);
	...
	if (bdev->bt_cfg.reset_stack_after_woble
		...
		bdev->main_info.reset_stack_flag = HW_ERR_CODE_RESET_STACK_AFTER_WOBLE;

	(void)btmtk_send_hw_err_to_host(bdev);
	BTMTK_INFO_DEV(bdev, "%s: end(%d), reset_stack_flag = %d, fstate = %d", __func__, ret,
			bdev->main_info.reset_stack_flag, fstate);
...
static irqreturn_t btmtk_woble_isr(int irq, struct st_woble *bt_woble)
{
	struct btmtk_dev *bdev = NULL;
	...
	BTMTK_INFO_DEV(bdev, "disable BT IRQ, call wake lock");
	__pm_wakeup_event(bdev->main_info.eint_ws, WAIT_POWERKEY_TIMEOUT);
...
static int btmtk_RegisterBTIrq(struct st_woble *bt_woble)
{
	struct btmtk_dev *bdev = bt_woble->bdev;
	struct device_node *eint_node = NULL;
	int interrupts[2];
	char of_node[BTMTK_DONGLE_TYPE_MAX][32] = {{0}};
	int idx;
	enum btmtk_dongle_type dongle_type;

	if (bdev->dongle_index == 0)
		dongle_type = BTMTK_DONGLE_TYPE_DEFAULT;
	else
		dongle_type = BTMTK_DONGLE_TYPE_DONGLE_DEFAULT;

	(void)snprintf(of_node[BTMTK_DONGLE_TYPE_DONGLE_INDEX], sizeof(of_node),
			"mediatek,woble_eint_%d", bdev->dongle_index);
	//snprintf(of_node[BTMTK_DONGLE_TYPE_DONGLE_DEFAULT], sizeof(of_node), "mediatek,woble_eint");

	BTMTK_DBG_DEV(bdev, "%s begin", __func__);
	idx = BTMTK_DONGLE_TYPE_DONGLE_INDEX;
	while (idx < dongle_type) {
		eint_node = of_find_compatible_node(NULL, NULL, of_node[idx]);
		if (eint_node)
			break;
		idx++;
	}
	if (idx == dongle_type) {
		bt_woble->wobt_irq = 0;
		BTMTK_INFO_DEV(bdev, "can't find woble_eint compatible node");
		return -1;
	}

	BTMTK_INFO_DEV(bdev, "Get woble_eint compatible node");
	bt_woble->wobt_irq = irq_of_parse_and_map(eint_node, 0);
	BTMTK_INFO_DEV(bdev, "woble_irq number:%d", bt_woble->wobt_irq);
	if (bt_woble->wobt_irq) {
		...
		if (request_irq(bt_woble->wobt_irq, (void *)btmtk_woble_isr,
			...
			BTMTK_INFO_DEV(bdev, "WOBTIRQ LINE NOT AVAILABLE!!");
		else {
			BTMTK_INFO_DEV(bdev, "disable BT IRQ");
			disable_irq_nosync(bt_woble->wobt_irq);
		}

	} else
		BTMTK_INFO_DEV(bdev, "can't find woble_eint irq");

	BTMTK_DBG_DEV(bdev, "%s end", __func__);
	return 0;
}

static int btmtk_woble_input_init(struct st_woble *bt_woble)
{
	int ret = 0;
	struct btmtk_dev *bdev = bt_woble->bdev;
	...
	if (!bt_woble->WoBLEInputDev || IS_ERR(bt_woble->WoBLEInputDev)) {
		BTMTK_ERR_DEV(bdev, "input_allocate_device error");
		return -ENOMEM;
	}
	...
	if (ret < 0) {
		...
		BTMTK_ERR_DEV(bdev, "input_register_device %d", ret);
		return ret;
	}
...
static void btmtk_woble_input_deinit(struct st_woble *bt_woble)
{
...
static void btmtk_free_woble_setting_file(struct st_woble *bt_woble)
{
...
int btmtk_woble_initialize(struct btmtk_dev *bdev)
{
	int err = 0;
	struct st_woble *bt_woble;
	...
	bt_woble = vmalloc(sizeof(struct st_woble));
	if (bt_woble == NULL) {
		BTMTK_ERR_DEV(bdev, "vmalloc failed");
		return -1;
	}
	memset(bt_woble, 0, sizeof(struct st_woble));

	bt_woble->bdev = bdev;
	bdev->bt_woble = bt_woble;
	bt_woble->woble_setting_file_idx = BTMTK_DONGLE_TYPE_DONGLE_INDEX;
	/* Need to add Woble flow */
	if (is_support_unify_woble(bdev)) {
		(void)snprintf(bt_woble->woble_setting_file_name[BTMTK_DONGLE_TYPE_DONGLE_INDEX],
				MAX_BIN_FILE_NAME_LEN, "%s_%x_idx_%d.%s",
				WOBLE_CFG_NAME_PREFIX, bdev->chip_id & 0xffff,
				bdev->dongle_index, WOBLE_CFG_NAME_SUFFIX);
		(void)snprintf(bt_woble->woble_setting_file_name[BTMTK_DONGLE_TYPE_DONGLE_DEFAULT],
				MAX_BIN_FILE_NAME_LEN, "%s_%x.%s",
				WOBLE_CFG_NAME_PREFIX, bdev->chip_id & 0xffff, WOBLE_CFG_NAME_SUFFIX);
		(void)snprintf(bt_woble->woble_setting_file_name[BTMTK_DONGLE_TYPE_DEFAULT],
				MAX_BIN_FILE_NAME_LEN, "%s.%s",
				WOBLE_CFG_NAME_PREFIX, WOBLE_CFG_NAME_SUFFIX);

		while (bt_woble->woble_setting_file_idx < BTMTK_DONGLE_TYPE_MAX) {
			err = btmtk_load_woble_setting(
					bt_woble->woble_setting_file_name[bt_woble->woble_setting_file_idx],
					bdev->intf_dev, bt_woble);
			if (err == 0)
				break;
			BTMTK_ERR_DEV(bdev, "load file %s failed",
					bt_woble->woble_setting_file_name[bt_woble->woble_setting_file_idx]);
			bt_woble->woble_setting_file_idx++;
		}

		if (bt_woble->woble_setting_file_idx == BTMTK_DONGLE_TYPE_MAX) {
			return err;
		}
		...
		if (bdev->main_info.reset_stack_flag) {
			...
			if (err < 0) {
				BTMTK_ERR_DEV(bdev, "reset power on failed!");
				goto err;
			}
		}
	}
...
void btmtk_woble_uninitialize(struct btmtk_dev *bdev)
{
	...
	if (bt_woble == NULL) {
		BTMTK_ERR_DEV(bdev, "%s: bt_woble == NULL", __func__);
		return;
	}
	...
	if (bdev->bt_cfg.support_woble_by_eint) {
		if (bt_woble->wobt_irq != 0 && atomic_read(&(bt_woble->irq_enable_count)) == 1) {
			BTMTK_INFO_DEV(bdev, "disable BT IRQ:%d", bt_woble->wobt_irq);
			atomic_dec(&(bt_woble->irq_enable_count));
			disable_irq_nosync(bt_woble->wobt_irq);
		} else
			BTMTK_INFO_DEV(bdev, "irq_enable count:%d", atomic_read(&(bt_woble->irq_enable_count)));
		if (bt_woble->wobt_irq) {
...
#if BT_SWIP_SUPPORT
void btmtk_update_woble_copin(struct btmtk_dev *bdev, int gpio)
{
	struct st_woble *bt_woble = bdev->bt_woble;
	u32 length = 0, idx = 0, opcode = 0;

	for (idx = 0; idx < bt_woble->woble_setting_wakeup_type.length;) {
		length = bt_woble->woble_setting_wakeup_type.content[idx];
		opcode = bt_woble->woble_setting_wakeup_type.content[idx + 1];
		if (opcode == 0x10) {
			bt_woble->woble_setting_wakeup_type.content[idx + 2] = (char)gpio;
			break;
		} else {
			idx += (length + 1);
		}
