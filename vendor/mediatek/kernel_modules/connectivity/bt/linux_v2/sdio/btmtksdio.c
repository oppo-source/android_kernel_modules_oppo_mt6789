...
static void btmtk_sdio_dump_debug_sop(struct btmtk_dev *bdev)
{
	...
	btmtk_sdio_dump_debug_register(bdev, bdev->debug_sop.debug_sop_reg_dump);
		...
		int delay, int retry, int pkt_type, bool flag)
{
	...
	if ((skb->len == fw_assert_cmd.len &&
		...
		BTMTK_INFO_RAW(bdev, skb->data, skb->len, "%s: Trigger FW assert, dump CR", __func__);
#if BTMTK_SDIO_DEBUG
		rx_debug_print();
#endif
...
static int btmtk_cif_recv_evt(struct btmtk_dev *bdev)
{
	...
	if (sdio_header_length != rx_length) {
		...
		BTMTK_INFO_RAW(bdev, cif_dev->transfer_buf, rx_length, "%s: raw data is :", __func__);
		btmtk_send_assert_cmd(bdev);
		return -EIO;
	}

	BTMTK_DBG_RAW(bdev, cif_dev->transfer_buf, rx_length, "%s: raw data is :", __func__);
	...
	if (hci_type == HCI_ISO_PKT) {
		...
		/* make sure memcpy done before send to hci layer */
		smp_wmb();
		BTMTK_DBG_RAW(cif_dev->bdev, cif_dev->transfer_buf, hci_pkt_len, "%s: raw data is :", __func__);
	}
#if 0
	switch (hci_type) {
		...
		int delay, int retry, int pkt_type, bool flag)
{
	...
	BTMTK_DBG_RAW(cif_dev->bdev, skb->data, skb->len, "%s, send, len = %d", __func__, skb->len);
...
static int btmtk_tx_pkt(struct btmtk_sdio_dev *cif_dev, struct sk_buff *skb)
{
	...
	BTMTK_DBG_RAW(cif_dev->bdev, cif_dev->sdio_packet, len,
...
static int btmtk_sdio_interrupt_process(struct btmtk_dev *bdev)
{
	struct btmtk_sdio_dev *cif_dev = (struct btmtk_sdio_dev *)bdev->cif_dev;
	int ret = 0;
	u32 u32ReadCRValue = 0, u32SM1RValue = 0;
	...
	if (u32ReadCRValue & FIRMWARE_INT_BIT9) {
		btmtk_sdio_readl(PH2DSM1R, &u32SM1RValue, cif_dev->func);
		btmtk_sdio_writel(PH2DSM1R, u32SM1RValue | PH2DSM1R_SDIO_STATUS, cif_dev->func);
	}

	if (u32ReadCRValue & FIRMWARE_INT_BIT10) {
		btmtk_sdio_readl(PH2DSM1R, &u32SM1RValue, cif_dev->func);
		btmtk_sdio_writel(PH2DSM1R, u32SM1RValue & (~PH2DSM1R_SDIO_STATUS), cif_dev->func);
	}
...
LOAD_PATCH_RETRY:
	...
	cif_dev->patched = bdev->main_info.hif_hook_chip.patched;
...
reset_retry:
	...
	do {
		 ...
		 */
		btmtk_sdio_keep_driver_own(cif_dev, 1);
		if (!bdev->main_info.hif_hook_chip.patched)
			break;
		(void)btmtk_sdio_readl(PD2HRM0R, &u32ReadCRValue, cif_dev->func);
		msleep(DELAY_TIMES);
		retry--;
	...
	/* write CHCR[3] 0 */
	ret = btmtk_sdio_readl(CHCR, &u32ReadCRValue, cif_dev->func);
	BTMTK_INFO("%s read CHCR 0x%08X, ret = %d", __func__, u32ReadCRValue, ret);
...
suspend_end:
	...
	if (ret == 0)
		...
		/* When return failure, suspend will abort without calling the resume callback.
...
int btmtk_cif_register(void)
{
	...
	ret = sdio_register();
	if (ret)
		BTMTK_ERR("%s, *** SDIO registration fail(%d)! ***", __func__, ret);
	else
