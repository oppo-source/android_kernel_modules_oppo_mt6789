...
#define BTMTK_ERR_DEV(bdev, fmt, ...)	 \
	do { \
		if (btmtk_log_lvl >= BTMTK_LOG_LVL_ERR && bdev) \
			PRINTF_INFO("[btmtk_err][%d][%s] ***"fmt"***\n", \
				bdev->dongle_index, bdev->hdev ? bdev->hdev->name : "hciX", ##__VA_ARGS__); \
	} while (0)
#define BTMTK_WARN_DEV(bdev, fmt, ...)	\
	do { \
		if (btmtk_log_lvl >= BTMTK_LOG_LVL_WARN && bdev) \
			PRINTF_INFO("[btmtk_warn][%d][%s] "fmt"\n", \
				bdev->dongle_index, bdev->hdev ? bdev->hdev->name : "hciX", ##__VA_ARGS__); \
	} while (0)
#define BTMTK_INFO_DEV(bdev, fmt, ...)	\
	do { \
		if (btmtk_log_lvl >= BTMTK_LOG_LVL_INFO && bdev) \
			PRINTF_INFO("[btmtk_info][%d][%s] "fmt"\n", \
				bdev->dongle_index, bdev->hdev ? bdev->hdev->name : "hciX", ##__VA_ARGS__); \
	} while (0)
#define BTMTK_DBG_DEV(bdev, fmt, ...)	 \
	do { \
		if (btmtk_log_lvl >= BTMTK_LOG_LVL_DBG && bdev) \
			PRINTF_INFO("[btmtk_dbg][%d][%s] "fmt"\n", \
				bdev->dongle_index, bdev->hdev ? bdev->hdev->name : "hciX", ##__VA_ARGS__); \
	} while (0)
...
#define BTMTK_INFO_RAW(bdev, p, l, fmt, ...)						\
	do {	\
		if (btmtk_log_lvl >= BTMTK_LOG_LVL_INFO) {	\
			int cnt_ = 0;	\
			...
			if (l <= HCI_SNOOP_MAX_BUF_SIZE) {	\
				PR_CONT("[btmtk_info][%u][%s] "fmt"%s\n", \
						bdev->dongle_index, bdev->hdev?bdev->hdev->name:"hciX", ##__VA_ARGS__, _raw_buf_);	\
			} else {	\
				PR_CONT("[btmtk_info][%u][%s] "fmt"%s (prtail)\n", \
						bdev->dongle_index, bdev->hdev?bdev->hdev->name:"hciX", ##__VA_ARGS__, _raw_buf_);	\
			}	\
		}	\
	} while (0)

#define BTMTK_DBG_RAW(bdev, p, l, fmt, ...)						\
	do {	\
		if (btmtk_log_lvl >= BTMTK_LOG_LVL_DBG) {	\
			int cnt_ = 0;	\
			int len_ = (l <= HCI_SNOOP_MAX_BUF_SIZE ? l : HCI_SNOOP_MAX_BUF_SIZE);	\
			...
			_raw_buf_[3*cnt_] = '\0'; \
			if (l <= HCI_SNOOP_MAX_BUF_SIZE) {	\
				PR_CONT("[btmtk_debug][%u][%s] "fmt"%s\n", \
						bdev->dongle_index, bdev->hdev?bdev->hdev->name:"hciX", ##__VA_ARGS__, _raw_buf_);	\
			} else {	\
				PR_CONT("[btmtk_debug][%u][%s] "fmt"%s (prtail)\n", \
						bdev->dongle_index, bdev->hdev?bdev->hdev->name:"hciX", ##__VA_ARGS__, _raw_buf_); \
			}	\
		}	\
	} while (0)

#define BTMTK_DBG_RAW_ALL(bdev, p, l, fmt, ...)				\
	do {	\
		if (btmtk_log_lvl >= BTMTK_LOG_LVL_DBG) {	\
			int cnt_ = 0;	\
			int pos = 0;	\
			...
			for (pos = 0; pos < l; ) {	\
				for (cnt_ = 0; cnt_ < len_ && pos < l; ++cnt_, ++pos) { \
					(void)snprintf(_raw_buf_+3*cnt_, 4, "%02X ", ptr[pos]);	\
				}	\
				_raw_buf_[3*cnt_] = '\0'; \
				PR_CONT("[btmtk_debug][%u][%s][cur_pos:%d] "fmt"%s\n", \
						bdev->dongle_index, bdev->hdev?bdev->hdev->name:"hciX", (int)pos, ##__VA_ARGS__, _raw_buf_);\
			}	\
		}	\
	} while (0)
...
#define BT_DRV_OWN_WAKELOCK_ENABLE "DRV_OWN_WAKELOCK"
#endif
...
enum btmtk_dongle_type {
	BTMTK_DONGLE_TYPE_DONGLE_INDEX = 0,
	BTMTK_DONGLE_TYPE_DONGLE_DEFAULT,
	BTMTK_DONGLE_TYPE_DEFAULT,

	BTMTK_DONGLE_TYPE_MAX
};
...
struct st_bt_cfg  {
	unsigned char	*bt_cfg_file_name[BTMTK_DONGLE_TYPE_MAX];
	unsigned int	bt_cfg_file_idx;
...
struct st_debug_sop {
	unsigned char		debug_sop_file_name[BTMTK_DONGLE_TYPE_MAX][MAX_BIN_FILE_NAME_LEN];
	unsigned int		debug_sop_file_idx;
...
#define DUMP_TIME_STAMP(__str) \
	do { \
		...
		BTMTK_INFO_DEV(bdev, "%s: %d, %s - DUMP_TIME_STAMP UTC: %d-%02d-%02d %02d:%02d:%02d.%06u", \
