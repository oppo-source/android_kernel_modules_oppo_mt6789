...
s32 rx_skb_enqueue(struct btmtk_dev *bdev, struct sk_buff *skb)
{
	s32 ret = 0;
	...
	if (!is_rx_queue_res_available(skb->len + 1)) {
		if (bdev)
			BTMTK_INFO_RAW(bdev, skb->data, skb->len, "%s: rx packet drop!!! len[%d] %02x", __func__,
...
int32_t btmtk_receive_data(struct hci_dev *hdev, u8 *buf, u32 count)
{
	...
	//BTMTK_DBG_RAW(bdev, buf, read_bytes, "%s, len[%d]", __func__, read_bytes);
...
void print_dump_packet(struct bt_dump_packet *p_packet)
{
	...
	BTMTK_INFO_RAW(bdev, p_packet->data, copysize, "Dump: Time:%02d:%02d:%02d.%06u,
