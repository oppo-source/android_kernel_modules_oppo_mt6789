...
static void aw_monitor_get_air_pressure(struct aw_device *aw_dev)
{
	int ret;
	int cali_f0 = 0;
	int air_pressure = 0;

	ret = aw882xx_dev_get_cali_f0(aw_dev, &cali_f0);
	if (ret) {
		aw_dev_err(aw_dev->dev, "get cali_f0 failed");
	}
	ret = aw882xx_dev_get_air_pressure(aw_dev, &air_pressure);
	if (ret) {
		aw_dev_err(aw_dev->dev, "get air_pressure failed");
	}

	aw_dev_info(aw_dev->dev, "cali_f0 = %d, air_pressure = %d",
			cali_f0, air_pressure);
}
...
static int aw_monitor_work(struct aw_device *aw_dev)
{
	...
	if (aw882xx->need_f0_cali) {
		aw_monitor_get_air_pressure(aw_dev);
	}
