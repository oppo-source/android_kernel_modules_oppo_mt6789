...
static bool is_single_cali = true; /*if mutli_dev cali false, single dev cali true: default*/
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
		scnprintf(fd_buf, sizeof(fd_buf) - 1, "payload@@AW882xx SPK%u: re out range value: %d",
				aw_dev->channel + 1, re);
		mm_fb_audio(OPLUS_AUDIO_EVENTID_SPK_ERR, MM_FB_KEY_RATELIMIT_5MIN, FEEDBACK_DELAY_60S, FB_HIGH, fd_buf);
#endif /*CONFIG_OPLUS_FEATURE_MM_FEEDBACK*/
		goto store_cali_re_failed;
	}
...
int aw_cali_parse_f0_dt(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		...
		aw_dev_info(aw_dev->dev, "read aw-f0-min failed, use default %d \n", AW_CALI_F0_DEFAULT_MIN);
	}

	ret = of_property_read_u32(aw_dev->dev->of_node, "aw-f0-max", &aw_dev->f0_max);
	if (ret < 0) {
		aw_dev->f0_max = AW_CALI_F0_DEFAULT_MAX;
		aw_dev_info(aw_dev->dev, "read aw-re-max failed, use default %d \n ", AW_CALI_F0_DEFAULT_MAX);
	}
	...
	aw_dev_info(aw_dev->dev, "f0 min: %d, f0 max: %d", aw_dev->f0_min, aw_dev->f0_max);
