...
static void init_project_version(void)
{
	...
	if (np) {
		...
		ret = of_property_read_u32(np, "nMidPlatVer",
				&(local_ProjectInfoCDT.nDataMidPlat.nVersion));
		if (ret) {
			pr_err("init_project_version_newcdt nMidPlatVer fail");
		    local_ProjectInfoCDT.nDataMidPlat.nVersion = MIDPLAT_INFO_UNSUPPORTED;
			return;
		}
		pr_err("init_project_version_newcdt MidPlatVersion = %d", local_ProjectInfoCDT.nDataMidPlat.nVersion);

		for (i = 0; i < OPLUS_FEAUTRE_MAX; i++) {
			sprintf(feature_node, "MidPlatInfo%d", i);
			ret = of_property_read_u32(np, feature_node, &minfo);
			local_ProjectInfoCDT.nDataMidPlat.infoMidPlat[i] = minfo;
			pr_err("init_project_version_newcdt infoMidPlat[%d] = %d", i, minfo);

			if (ret) {
				pr_err("init_project_version_newcdt infoMidPlat[%d]: %s %u fail",
					i, feature_node, minfo);
			}
		}
...
bool get_midplat_version(void)
{
	init_project_version();

	return g_project ? g_project->nDataMidPlat.nVersion : -EINVAL;
}
EXPORT_SYMBOL(get_midplat_version);

int get_midplat_feature_info(OPLUS_MID_PLAT_FEATURE_T feature)
{
	if (feature < 0 || feature > OPLUS_FEAUTRE_MAX) {
		pr_err("%s:invalid param\n", __func__);
		return -1;
	}

	init_project_version();
	if (g_project->nDataMidPlat.nVersion == MIDPLAT_INFO_NORMAL) {
