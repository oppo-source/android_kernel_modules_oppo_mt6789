...
static int oplus_dsi_panel_parse_intarr(const struct device_node *node, const char *name, int *value, int length)
{
	...
	if (!value || !node) {
		OPLUS_DSI_WARN("value is %d node is %d\n", value, node);
		return -EINVAL;
	}
	...
	rc = of_property_read_u32_array(node, name, value, length);
	if (rc == 0) {
...
int oplus_dsi_panel_parser_base(struct device_node *node, void *ext_param_dev, void *ctx_dev)
{
	...
	rc = utils->get_bool(node, "oplus,is-support-dbi", &ext_param->is_support_dbi);
	if (utils->get_bool(node, "oplus,pmic_reset_enable", &ctx->oplus_pmic_reset_enable)) {
		ctx->vddr_aod_enable_gpio = devm_gpiod_get(ctx->dev, "pmic-reset", GPIOD_OUT_HIGH);
		if (IS_ERR(ctx->vddr_aod_enable_gpio)) {
			rc = PTR_ERR(ctx->vddr_aod_enable_gpio);
			OPLUS_DSI_ERR("failed to get pmic-reset gpio, rc=%d\n", rc);
			ctx->vddr_aod_enable_gpio = NULL;
			ctx->oplus_pmic_reset_enable = false;
		}
	}
...
int oplus_dsi_panel_parser_timing_base(struct device_node *node, void *ext_param_dev, void *ctx_dev)
{
	...
	OPLUS_DSI_ERR("oplus,is-support-dmr count = %d \n", ext_param->is_support_dmr);
	...
	rc = utils->get_int(node, "mtk,skip_vblank", &ext_param->skip_vblank);
	rc = utils->get_int(node, "mtk,real_te_duration", &ext_param->real_te_duration);
