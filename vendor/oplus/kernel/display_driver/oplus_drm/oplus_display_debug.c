...
extern bool fc_dmr_sn_read_enable;
int oplus_display_set_serial_number_read_enable(void *buf)
{
	unsigned int *fc_dmr_read_sn = buf;
	fc_dmr_sn_read_enable = (*fc_dmr_read_sn);
	OPLUS_DSI_INFO("fc_dmr_sn_read_enable = %d", fc_dmr_sn_read_enable);
	return 0;
}
