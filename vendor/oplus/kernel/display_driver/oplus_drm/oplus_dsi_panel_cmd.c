...
const char *dsi_cmd_map[DSI_CMD_ID_MAX] = {
	...
	"oplus,dsi-panel-btb-switch-command",
	"oplus,dsi-panel-cmd-default-switch-page-command",
};
...
const char *dsi_cmd_state_map[DSI_CMD_ID_MAX] = {
	...
	"oplus,dsi-panel-btb-switch-command-state",
	"oplus,dsi-panel-cmd-default-switch-page-command-state",
};
...
{
	int rc = 0;
	enum dsi_cmd_set_state cmd_state1, cmd_state2;
	...
	cmd_state2 = lcm_timing_cmds[cmd_set_id].state;
	cmd_state1 = cmd_state2;
	if (count == 0) {
	...
	if (cmd_func) {
		...
		case DSI_CMD_FUNC_HOST:
			cmd_state1 = DSI_CMD_SET_STATE_LP;
			break;
		case DSI_CMD_FUNC_PACK_GCE:
			cmd_state1 = DSI_CMD_SET_STATE_HS;
			break;
		case DSI_CMD_FUNC_GCE:
		case DSI_CMD_FUNC_GCE2:
			cmd_state1 = DSI_CMD_SET_STATE_LP_GCE;
			break;
		default:
			break;
		}
	...
	if (cmd_state1 != DSI_CMD_SET_STATE_LP) {
		mtk_dsi = dsi;
		...
		dev_dsi = dsi;
		mtk_dsi = container_of(dev_dsi->host, struct mtk_dsi, host);
	}

	if (mtk_dsi_cmd_version() == DSI_CMD_V2) {
		oplus_dsi_panel_send_cmd_v2(mtk_dsi, cmd_set_id, table, handle, cmd_state2, count);
	} else {
		oplus_dsi_panel_send_cmd_v1(dsi, cmd_set_id, table, handle, cmd_state1, cmd_func, count);
	}
...
int oplus_dsi_panel_send_cmd_v1(void *dsi, enum dsi_cmd_id cmd_set_id, struct dsi_cmd_table *table,
		void *handle, enum dsi_cmd_set_state cmd_state, enum dsi_cmd_func cmd_func, unsigned int count)
{
	int i, rc = 0;
	struct mtk_ddic_cmd *ddic_cmd = send_cmd_to_ddic.mtk_ddic_cmd_table;
...
EXPORT_SYMBOL(oplus_dsi_panel_send_cmd_v1);

int oplus_dsi_panel_send_cmd_v2(void *dsi, enum dsi_cmd_id cmd_set_id, struct dsi_cmd_table *table,
		void *handle, enum dsi_cmd_set_state cmd_state, unsigned int count) {
	int i, k, rc = 0;
	struct mtk_dsi_cmd_msg oplus_dsi_cmd_msgs = { 0 };
	struct mtk_dsi_cmd_option cmd_opt = { 0 };
	unsigned int cmd_start = 0;
	bool is_batch_command = false;
	bool in_batch_group = false;

	oplus_dsi_cmd_msgs.is_rd = 0;
	oplus_dsi_cmd_msgs.rd_to_slot = 0;
	switch (cmd_state) {
	/* GCE hs */
	case DSI_CMD_SET_STATE_HS:
		if (handle) {
			cmd_opt.flags = MTK_MIPI_DSI_GCE_INPUT_HANDLE_READY |MTK_MIPI_DSI_SKIP_CRTC_EN | MTK_MIPI_DSI_CRTC_ID;
			oplus_dsi_cmd_msgs.transfer_mode = PACKET_HS_MODE;
		} else {
			cmd_opt.flags = MTK_MIPI_DSI_GCE_CREATE_HANDLE | MTK_MIPI_DSI_GCE_BLOCKING_FLUSH | MTK_MIPI_DSI_GCE_USE_CFG_THREAD
			| MTK_MIPI_DSI_CRTC_ID |MTK_MIPI_DSI_SKIP_CRTC_EN;
			oplus_dsi_cmd_msgs.transfer_mode = PACKET_HS_MODE;
		}
		break;
	/* CPU lp */
	case DSI_CMD_SET_STATE_LP:
		cmd_opt.flags = MTK_MIPI_DSI_CMD_BY_CPU | MTK_MIPI_DSI_CRTC_ID |MTK_MIPI_DSI_SKIP_CRTC_EN;
		oplus_dsi_cmd_msgs.transfer_mode = PACKET_LP_MODE;
		break;
	/* GCE lp */
	case DSI_CMD_SET_STATE_LP_GCE:
		if (handle) {
			cmd_opt.flags = MTK_MIPI_DSI_GCE_INPUT_HANDLE_READY |MTK_MIPI_DSI_SKIP_CRTC_EN | MTK_MIPI_DSI_CRTC_ID;
	...
	/* Pre-interface for send cmd */
	oplus_dsi_panel_send_cmd_pre(NULL, cmd_set_id, cmd_state);

	for (i = 0; i < count; i++) {
		/* current command meets the conditions */
		oplus_dsi_panel_send_cmd_line_pre(NULL, NULL, cmd_set_id, i);
		bool is_batch_command = table[i].cmd_msg.flags & MIPI_DSI_MSG_BATCH_COMMAND;
		if (is_batch_command) {
			/* the first command, or the command before this command does not meet the conditions */
			if (i == 0 || !in_batch_group) {
				cmd_start = i;
			}
		} else {
			/* Check whether there is a valid command group before */
			if (i > 0 && in_batch_group) {
				unsigned int cmd_num = (i - cmd_start) + 1;
				oplus_dsi_cmd_msgs.is_package = 1;
				oplus_dsi_cmd_msgs.cmd_num = cmd_num;
				OPLUS_DSI_DEBUG_DCS("send cmd mutli, cmd_start=%d, cmd_num=%d\n", cmd_start, cmd_num);
				oplus_dsi_cmd_msgs.cmd_msg = kzalloc(cmd_num * sizeof(struct mipi_dsi_msg), GFP_KERNEL);
				if (!oplus_dsi_cmd_msgs.cmd_msg) {
					OPLUS_DSI_ERR("Failed to allocate memory for oplus_dsi_cmd_msgs->cmd_msg\n");
					kfree(oplus_dsi_cmd_msgs.cmd_msg);
					oplus_dsi_cmd_msgs.cmd_msg = NULL;
					return -ENOMEM;
				}
				for (k = cmd_start; k <= i; k++) {
					oplus_dsi_cmd_msgs.cmd_msg[k - cmd_start].tx_len = table[k].count;
					oplus_dsi_cmd_msgs.cmd_msg[k - cmd_start].tx_buf = table[k].para_list;
					const char *tx_buf = oplus_dsi_cmd_msgs.cmd_msg[k - cmd_start].tx_buf;
				}
				mtk_mipi_dsi_cmd(dsi, handle, &cmd_opt, &oplus_dsi_cmd_msgs);
				if (oplus_dsi_cmd_msgs.cmd_msg) {
					kfree(oplus_dsi_cmd_msgs.cmd_msg);
					oplus_dsi_cmd_msgs.cmd_msg = NULL;
				}
				if (table[i].post_wait_ms > 0) {
					if (handle && (cmd_state != DSI_CMD_SET_STATE_LP)) {
						cmdq_pkt_sleep(handle, CMDQ_US_TO_TICK(table[i].post_wait_ms * 1000), CMDQ_GPR_R14);
					} else {
						usleep_range(table[i].post_wait_ms * 1000, table[i].post_wait_ms * 1000 + 100);
				...
				oplus_dsi_cmd_msgs.is_package = 0;
				oplus_dsi_cmd_msgs.cmd_num = 1;
				oplus_dsi_cmd_msgs.cmd_msg = kzalloc(sizeof(struct mipi_dsi_msg), GFP_KERNEL);
				if (!oplus_dsi_cmd_msgs.cmd_msg) {
					OPLUS_DSI_ERR("Failed to allocate memory for oplus_dsi_cmd_msgs->cmd_msg\n");
					kfree(oplus_dsi_cmd_msgs.cmd_msg);
					oplus_dsi_cmd_msgs.cmd_msg = NULL;
				...
				oplus_dsi_cmd_msgs.cmd_msg->tx_len = table[i].count;
				oplus_dsi_cmd_msgs.cmd_msg->tx_buf = table[i].para_list;
				mtk_mipi_dsi_cmd(dsi, handle, &cmd_opt, &oplus_dsi_cmd_msgs);
				if (oplus_dsi_cmd_msgs.cmd_msg) {
					kfree(oplus_dsi_cmd_msgs.cmd_msg);
					oplus_dsi_cmd_msgs.cmd_msg = NULL;
		...
		in_batch_group = is_batch_command;
	}
	if (oplus_dsi_cmd_msgs.cmd_msg) {
		kfree(oplus_dsi_cmd_msgs.cmd_msg);
		oplus_dsi_cmd_msgs.cmd_msg = NULL;
...
EXPORT_SYMBOL(oplus_dsi_panel_send_cmd_v2);
