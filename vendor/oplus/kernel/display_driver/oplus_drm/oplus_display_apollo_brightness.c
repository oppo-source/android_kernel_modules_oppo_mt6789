...
extern u16 mtk_get_gpr(struct mtk_drm_crtc *mtk_crtc, struct cmdq_pkt *handle);
	...
	if (cur_vsync != 0) {
		...
		if (time_gap_ns >= 0 && time_gap_ns <= cur_vsync) {
			...
			if (time_gap_ns < limit_superior_ns) {
				if (oplus_have_previous_adjacent_frame(te_tag_ns, cur_vsync) && transfer_time_us > time_gap_ns/1000) {
				...
				if (delay_us > 0) {
					...
					cmdq_pkt_sleep(cmdq_handle, CMDQ_US_TO_TICK(delay_us), mtk_get_gpr(mtk_crtc, cmdq_handle));
...
void oplus_sync_panel_brightness(struct drm_crtc *crtc)
{
	...
	OPLUS_DSI_TRACE_BEGIN("sync_panel_brightness level(%d)", mtk_crtc->oplus_apollo_br->oplus_pending_backlight);
	mtk_drm_setbacklight_without_lock(crtc, mtk_crtc->oplus_apollo_br->oplus_pending_backlight, 0, 0x1 << SET_BACKLIGHT_LEVEL);
	OPLUS_DSI_TRACE_END("sync_panel_brightness level(%d)", mtk_crtc->oplus_apollo_br->oplus_pending_backlight);
}
...
void oplus_sync_panel_brightness_video(struct drm_crtc *crtc, struct cmdq_pkt *cmdq_handle)
{
	...
	OPLUS_DSI_TRACE_BEGIN("sync_panel_brightness_video level(%d)", mtk_crtc->oplus_apollo_br->oplus_pending_backlight);
	...
	OPLUS_DSI_TRACE_END("sync_panel_brightness_video level(%d)", mtk_crtc->oplus_apollo_br->oplus_pending_backlight);
}
