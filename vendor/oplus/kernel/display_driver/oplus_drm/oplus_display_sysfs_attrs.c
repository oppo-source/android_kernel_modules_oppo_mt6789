...
/*for demura factory sn read always from panel*/
unsigned int fc_dmr_sn_read_enable = 0x0;
EXPORT_SYMBOL(fc_dmr_sn_read_enable);
int cmd_id_debug;
...
int oplus_btb_sn_parse(struct device_node *node, void *lcm_ctx)
{
	int rc = 0;
	bool config = false;
	u32 sn_index = 0;
	unsigned int sn_reg = 0;
	unsigned int sn_read_count = 0;
	struct dsi_panel_lcm *ctx = lcm_ctx;
	struct oplus_btb_sn *btb_sn_params = &ctx->btb_sn;

	if (of_property_read_bool(node, "oplus,dsi-btb-sn-enabled")) {
		config = true;
	} else {
		config = false;
	}
	btb_sn_params->btb_sn_support = config;
	if(btb_sn_params->btb_sn_support == false) {
		OPLUS_DSI_WARN("this panel not support read btb_sn\n");
		goto error;
	}

	if (of_property_read_bool(node, "oplus,dsi-btb-sn-switch-page")) {
		btb_sn_params->is_switch_page = true;
	} else {
		btb_sn_params->is_switch_page = false;
	}

	rc = of_property_read_u32(node, "oplus,dsi-btb-sn-reg", &sn_reg);
	if (rc == 0) {
		btb_sn_params->btb_sn_reg = sn_reg;
		OPLUS_DSI_INFO("oplus,dsi-btb-sn-reg=0x%02X\n", sn_reg);
	} else {
		btb_sn_params->btb_sn_reg = 0;
		OPLUS_DSI_WARN("Parsing failed, default read_reg=0x%02X\n",
				btb_sn_params->btb_sn_reg);
	}

	rc = of_property_read_u32(node, "oplus,dsi-btb-sn-index",
			&sn_index);
	if (rc == 0) {
		btb_sn_params->btb_sn_index = sn_index;
		OPLUS_DSI_INFO("oplus,dsi-btb-sn-read-count=%d\n", sn_read_count);
	} else {
		btb_sn_params->btb_sn_index = 0;
		OPLUS_DSI_WARN("Parsing failed, default read_count=%d\n",
				btb_sn_params->btb_sn_index);
	}

	rc = of_property_read_u32(node, "oplus,dsi-btb-sn-read-count",
			&sn_read_count);
	if (rc == 0) {
		btb_sn_params->btb_sn_count = sn_read_count;
		OPLUS_DSI_INFO("oplus,dsi-btb-sn-read-count=%d\n", sn_read_count);
	} else {
		btb_sn_params->btb_sn_count = 0;
		OPLUS_DSI_WARN("Parsing failed, default read_count=%d\n",
				btb_sn_params->btb_sn_count);
	}
...
error:
	btb_sn_params->btb_sn_support = false;
	OPLUS_DSI_WARN("config=0x%X\n", btb_sn_params->btb_sn_support);
...
int oplus_panel_btb_sn_read(struct drm_crtc *crtc, void *dsi, void *lcm_ctx, char *buf)
{
	int count = 0;
	char para[64] = {0};
	...
	if(btb_sn_params->btb_sn_support == 0) {
		OPLUS_DSI_WARN("not support\n");
		return 0;
	} else {
		mutex_lock(&g_oplus_sn_lock);
		if (btb_sn_params->is_switch_page) {
			mtk_drm_send_lcm_cmd_prepare(crtc, &cmdq_handle);
			oplus_dsi_panel_send_cmd(dsi, DSI_CMD_PANEL_BTB_SWITCH, cmdq_handle, DSI_CMD_FUNC_DEFAULT);
			mtk_drm_send_lcm_cmd_flush(crtc, &cmdq_handle, 0);
		}
		oplus_ddic_dsi_read_cmd(btb_sn_params->btb_sn_reg, btb_sn_params->btb_sn_count, para);
		if (btb_sn_params->is_switch_page) {
			mtk_drm_send_lcm_cmd_prepare(crtc, &cmdq_handle2);
			oplus_dsi_panel_send_cmd(dsi, DSI_CMD_DEFAULT_SWITCH_PAGE, cmdq_handle2, DSI_CMD_FUNC_DEFAULT);
			mtk_drm_send_lcm_cmd_flush(crtc, &cmdq_handle2, 0);
		}
		memcpy(buf, para, btb_sn_params->btb_sn_count);
		mutex_unlock(&g_oplus_sn_lock);
	}
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	if (serial_number == 0 || fc_dmr_sn_read_enable == 1) {
...
static ssize_t oplus_get_btb_sn_number(struct kobject *obj,
		struct kobj_attribute *attr, char *buf)
{
	...
	crtc = list_first_entry(&(ddev)->mode_config.crtc_list,
		typeof(*crtc), head);
	if (!crtc) {
		OPLUS_DSI_ERR("get_panel_btb_sn find crtc fail\n");
		return 0;
	}
	...
	if (oplus_display_brightness == 0) {
		OPLUS_DSI_INFO("backlight is 0, skip get btb sn!\n");
		return 0;
	}

	oplus_panel_btb_sn_read(crtc, dsi, ctx, btb_buffer);
	OPLUS_DSI_INFO("Read btb sn=0x%s, DA=0x%02X, DB=0x%02X, DC=0x%02X\n",
			btb_buffer, m_da, m_db, m_dc);
	return scnprintf(buf, PAGE_SIZE, "Get panel btb sn: %s", btb_buffer + ctx->btb_sn.btb_sn_index);
}
...
static int oplus_dsi_panel_command_id(const char *lcm_cmd_name)
{
	int i = 0;

	if (!strncmp("lcm_cmd_map", lcm_cmd_name, strlen("lcm_cmd_map"))) {
		OPLUS_DSI_INFO("cmd_id=%d", DSI_CMD_ID_MAX);
		cmd_id_debug = DSI_CMD_ID_MAX;
		return DSI_CMD_ID_MAX;
	}
	for (; i < DSI_CMD_ID_MAX; i ++) {
		if (!strncmp(dsi_cmd_map[i], lcm_cmd_name, strlen(dsi_cmd_map[i]))) {
		...
		const char *buf, size_t count)
{
	...
	if (!strncmp("dump", buf, strlen("dump"))) {
		...
		return count;
	} else if (!strncmp("flush", buf, strlen("flush"))) {
		OPLUS_DSI_INFO("flush_end\n");
		flush_end = true;
	} else if (!strncmp("send", buf, strlen("send"))) {
		OPLUS_DSI_INFO("send\n");
		send = true;
	} else if (!strncmp("dsi_hs_mode", buf, strlen("dsi_hs_mode"))) {
	...
	if (flush_start && !flush_end) {
		...
		while (sscanf(buf, "%x%n", &value, &step) > 0) {
			if (value > 0xFF) {
	...
	OPLUS_DSI_DEBUG_DCS("flush_start=%d, flush_end=%d, dsi_cmd[%d][%d]: %s\n",
			flush_start, flush_end, cmd_id, cmd_lines, buf);
	ret = count;
...
static OPLUS_ATTR(panel_btb_sn, S_IRUGO|S_IWUSR, oplus_get_btb_sn_number, NULL);
