...
#define OPLUS_ADFR_CONFIG_SETBACKLIGHT_DELAY                 (BIT(10))
...
#define OPLUS_ADFR_GET_SETBACKLIGHT_DELAY_CONFIG(config)     ((config) & OPLUS_ADFR_CONFIG_SETBACKLIGHT_DELAY)
...
bool oplus_adfr_setbacklight_delay_is_enabled(void *drm_crtc)
{
	struct drm_crtc *crtc = drm_crtc;
	struct oplus_adfr_params *p_oplus_adfr_params = NULL;
	...
	return (bool)(OPLUS_ADFR_GET_SETBACKLIGHT_DELAY_CONFIG(p_oplus_adfr_params->config));
}
EXPORT_SYMBOL(oplus_adfr_setbacklight_delay_is_enabled);
...
static bool oplus_adfr_software_marker_is_enabled(void *oplus_adfr_params)
{
	...
	if (!oplus_adfr_is_supported(p_oplus_adfr_params)) {
		ADFR_DEBUG("ADFR is not supported, software marker is also not supported\n");
		return false;
	}
...
/* handle irq function */
int oplus_adfr_irq_handler(void *mtk_drm_crtc, unsigned int irq_type)
{
	unsigned int refresh_rate = 120;
	/* The initial value is 0, but we don't care about the first calculation error */
	static unsigned long te_irq_timestamp_us = 0;
	static unsigned long mutex_sof_timestamp_us = 0;
	...
	if (!mtk_crtc) {
		ADFR_ERR("invalid mtk_crtc param\n");
		return -EINVAL;
	}

	crtc = &(mtk_crtc->base);
	if (!crtc || !crtc->state || !&(crtc->state->mode)) {
	...
	if (irq_type == OPLUS_ADFR_TE_IRQ) {
		te_irq_timestamp_us = (unsigned long)ktime_to_us(ktime_get());
		ADFR_DEBUG("te_irq_timestamp_us:%lu\n", te_irq_timestamp_us);
		OPLUS_ADFR_TRACE_INT("te_irq_timestamp_us", te_irq_timestamp_us);

		/* software corner mark */
		if (oplus_adfr_software_marker_is_enabled(p_oplus_adfr_params)) {
			delta = te_irq_timestamp_us - mutex_sof_timestamp_us;
			if (delta < 45000) {
	...
	} else if (irq_type == OPLUS_ADFR_MUTEX_SOF) {
		mutex_sof_timestamp_us = (unsigned long)ktime_to_us(ktime_get());
		ADFR_DEBUG("mutex_sof_timestamp_us:%lu\n", mutex_sof_timestamp_us);
		OPLUS_ADFR_TRACE_INT("mutex_sof_timestamp_us", mutex_sof_timestamp_us);
