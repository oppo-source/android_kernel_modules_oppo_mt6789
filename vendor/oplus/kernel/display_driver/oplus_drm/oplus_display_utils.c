...
void oplus_ddic_dsi_send_cmd(unsigned int len, char *data)
{
	if (mtk_dsi_cmd_version() == DSI_CMD_V2) {
		oplus_ddic_dsi_send_cmd_v2(len, data);
	} else {
		oplus_ddic_dsi_send_cmd_v1(len, data);
	}
}
EXPORT_SYMBOL(oplus_ddic_dsi_send_cmd);

void oplus_ddic_dsi_send_cmd_v1(unsigned int len, char *data)
{
...
EXPORT_SYMBOL(oplus_ddic_dsi_send_cmd_v1);

void oplus_ddic_dsi_send_cmd_v2(unsigned int len, char *data)
{
	unsigned int i = 0, j = 0;
	...
	OPLUS_DSI_ERR("Start send cmd, len=%d\n", len);
	if (len > OPLUS_MIPI_TX_MAX_LEN) {
		OPLUS_DSI_ERR("Invalid cmd len: %d\n", len);
		return;
	}

	cmd_opt.flags = MTK_MIPI_DSI_CMD_EXTERNAL | MTK_MIPI_DSI_GCE_BLOCKING_FLUSH | MTK_MIPI_DSI_CRTC_ID;
	oplus_dsi_cmd_msgs.is_rd = 0;
	oplus_dsi_cmd_msgs.rd_to_slot = 0;
	oplus_dsi_cmd_msgs.is_package = 0;
	oplus_dsi_cmd_msgs.cmd_num = 1;
	oplus_dsi_cmd_msgs.cmd_msg = vmalloc(sizeof(struct mipi_dsi_msg));
	if (!oplus_dsi_cmd_msgs.cmd_msg) {
		OPLUS_DSI_ERR("Failed to allocate memory for oplus_dsi_cmd_msgs->cmd_msg\n");
		vfree(oplus_dsi_cmd_msgs.cmd_msg);
	...
	for (j = 0; j < len; j++) {
		cnt += snprintf(buf + cnt, 4, "%02X ",	tx[0] + j);
	}
	OPLUS_DSI_ERR("cmd: len=%d, tx_buf=[%s]\n", len, buf);
	oplus_dsi_cmd_msgs.cmd_msg->tx_len = len;
	oplus_dsi_cmd_msgs.cmd_msg->tx_buf = tx;
	mtk_mipi_dsi_cmd(NULL, NULL, &cmd_opt, &oplus_dsi_cmd_msgs);
	if (oplus_dsi_cmd_msgs.cmd_msg) {
		vfree(oplus_dsi_cmd_msgs.cmd_msg);
		oplus_dsi_cmd_msgs.cmd_msg = NULL;
	}
...
EXPORT_SYMBOL(oplus_ddic_dsi_send_cmd_v2);

void oplus_ddic_dsi_read_cmd(u8 reg, int len, char *data)
{
	if (mtk_dsi_cmd_version() == DSI_CMD_V2) {
		oplus_ddic_dsi_read_cmd_v2(NULL, reg, len, data);
	} else {
		oplus_ddic_dsi_read_cmd_v1(reg, len, data);
	}
}
EXPORT_SYMBOL(oplus_ddic_dsi_read_cmd);

void oplus_ddic_dsi_read_cmd_v1(u8 reg, int len, char *data)
{
...
EXPORT_SYMBOL(oplus_ddic_dsi_read_cmd_v1);

void oplus_ddic_dsi_read_cmd_v2(void *dsi, u8 reg, int len, char *data)
{
	int i;
	char print_buf[OPLUS_DSI_CMD_PRINT_BUF_SIZE] = {0};
	struct mtk_dsi_cmd_option cmd_opt = { 0 };
	struct mtk_dsi_cmd_msg reg_cmd_msg = { 0 };
	struct mipi_dsi_msg msg = { 0 };

	if(!data) {
		OPLUS_DSI_ERR("data empty");
		return;
	}
	...
	msg.tx_buf = &reg;
	reg_cmd_msg.is_rd = 1;
	reg_cmd_msg.read_scn = READ_COMMON_SCN;
	reg_cmd_msg.transfer_mode = PACKET_LP_MODE;
	reg_cmd_msg.cmd_msg = &msg;
	reg_cmd_msg.cmd_msg->rx_buf = data;
	cmd_opt.flags = MTK_MIPI_DSI_CMD_EXTERNAL | MTK_MIPI_DSI_CRTC_ID | MTK_MIPI_DSI_GCE_BLOCKING_FLUSH;
	cmd_opt.crtc_id = 0;

	OPLUS_DSI_INFO("Start read reg=0x%02X, len=%d\n", reg, len);
	mtk_mipi_dsi_cmd(dsi, NULL, &cmd_opt, &reg_cmd_msg);
	for (i = 0; i < len; i++)
		OPLUS_DSI_INFO("new_read_ddic, rx_data[%d] = 0x%x\n", i, data[i]);
	OPLUS_DSI_INFO("End\n");
}
EXPORT_SYMBOL(oplus_ddic_dsi_read_cmd_v2);
...
int oplus_panel_power_dump(void *ctx)
{
	int power_count, i = 0;
	struct dsi_panel_lcm *lcm_ctx = ctx;
	struct dsi_vreg *vreg_target = NULL;

	power_count = lcm_ctx->power_info.count;
	for (i = 0; i < power_count; i++) {
		vreg_target = &lcm_ctx->power_info.vregs[i];
		if (vreg_target->vreg_type == 0) {
			OPLUS_DSI_ERR("ldo check (%d)\n", regulator_get_voltage(vreg_target->vreg));
		}
	}
	OPLUS_DSI_ERR("power check on\n");
	gpio_dump_regs_range(79, 79);
	gpio_dump_regs_range(88, 89);
	gpio_dump_regs_range(105, 105);

	return 0;
}
EXPORT_SYMBOL(oplus_panel_power_dump);
