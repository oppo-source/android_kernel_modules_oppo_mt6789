	...
	void *handle, unsigned int en, struct mtk_dsi_cmd_option *cmd_opt)
{
	...
	spr_msg = kzalloc(sizeof(struct mipi_dsi_msg) * cmd_count, GFP_KERNEL);
	if (!spr_msg) {
		OPLUS_DSI_ERR("kzalloc failed\n");
		return -ENOMEM;
	}
	for (i = 0; i < cmd_count; i++) {
		spr_msg[i].tx_len = lcm_all_cmd_table[cmd_id].para_table[i].count;
		spr_msg[i].tx_buf = lcm_all_cmd_table[cmd_id].para_table[i].para_list;
	}

	struct mtk_dsi_cmd_msg panel_spr_tb_v2 = {
	...
	OPLUS_DSI_DEBUG("mtk_dsi_ddic_cmd send dsi_cmd[%d]: %s\n", cmd_id, dsi_cmd_map[cmd_id]);
	cb(dsi, handle, cmd_opt, &panel_spr_tb_v2);
