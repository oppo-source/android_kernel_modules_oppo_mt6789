...
int oplus_display_panel_get_vendor(void *buf)
{
	...
	memcpy(p_info->version, mtk_crtc->panel_ext->params->vendor,
	       sizeof(mtk_crtc->panel_ext->params->vendor) > (PANEL_IOCTL_BUF_MAX - 1) ?
	       (PANEL_IOCTL_BUF_MAX - 1) : (sizeof(mtk_crtc->panel_ext->params->vendor) + 1));
	memcpy(p_info->manufacture, mtk_crtc->panel_ext->params->manufacture,
	       sizeof(mtk_crtc->panel_ext->params->manufacture) > (PANEL_IOCTL_BUF_MAX - 1) ?
	       (PANEL_IOCTL_BUF_MAX - 1) : (sizeof(mtk_crtc->panel_ext->params->manufacture) + 1));
