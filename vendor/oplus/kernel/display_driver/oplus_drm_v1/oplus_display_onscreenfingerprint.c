...
inline bool oplus_ofp_is_supported(void)
{
...
int oplus_ofp_init(void *node)
{
	...
	ret = of_property_read_u32(devnode, "oplus,ofp-fp-type", &value);
	if (ret) {
	...
	if (oplus_ofp_is_supported()) {
		...
		ret = of_property_read_u32(devnode, "oplus,ofp-longrui-aod-config", &value);
		if (ret) {
...
int oplus_ofp_hbm_set_with_mode_change(bool mode_change) {
	struct oplus_ofp_params *p_oplus_ofp_params = oplus_ofp_get_params();
	...
	p_oplus_ofp_params->hbm_handle_with_mode_change = true;
	OFP_INFO("hbm_handle_with_mode_change true\n");
		...
		if (hbm_en) {
			if (p_oplus_ofp_params->aod_unlocking == true) {
				if (panel_ext->oplus_ofp_aod_off_insert_black > 0) {
					...
					if (panel_ext->oplus_ofp_hbm_on_need_wait_extra_te) {
						OFP_DEBUG("fps_before_hbm_handle=%d, refresh_rate=%d\n", p_oplus_ofp_params->fps_before_hbm_handle, refresh_rate);
						if (p_oplus_ofp_params->hbm_handle_with_mode_change == true) {
							if ((p_oplus_ofp_params->fps_before_hbm_handle == 60) && (refresh_rate == 120)) {
								te_count++;
								OFP_INFO("fps change, wait more te\n");
							}
						}
					}
				} else if (panel_ext->oplus_ofp_need_to_sync_data_in_aod_unlocking) {
...
int oplus_ofp_hbm_handle(void *drm_crtc, void *mtk_crtc_state, void *cmdq_pkt)
{
	...
	if (!crtc || !state || !cmdq_handle || !p_oplus_ofp_params || !panel_ext) {
		OFP_ERR("Invalid params\n");
		return -EINVAL;
	}
	...
	OFP_DEBUG("hbm_en = %d, oplus_display_brightness=%d\n", hbm_en, oplus_display_brightness);

	if (mtk_crtc_is_frame_trigger_mode(crtc) || oplus_ofp_video_mode_aod_fod_is_enabled()) {
		if ((!state->prop_val[CRTC_PROP_DOZE_ACTIVE] && (hbm_en & OPLUS_OFP_PROPERTY_DIM_LAYER
			|| hbm_en & OPLUS_OFP_PROPERTY_FINGERPRESS_LAYER) && oplus_display_brightness != 0)
			|| (state->prop_val[CRTC_PROP_DOZE_ACTIVE] && (hbm_en & OPLUS_OFP_PROPERTY_FINGERPRESS_LAYER)
		...
		} else if ((!(hbm_en & OPLUS_OFP_PROPERTY_DIM_LAYER) && !(hbm_en & OPLUS_OFP_PROPERTY_FINGERPRESS_LAYER))
		...
		p_oplus_ofp_params->fps_before_hbm_handle = panel_ext->dyn_fps.vact_timing_fps;
		p_oplus_ofp_params->hbm_handle_with_mode_change = false;
	}
