...
static atomic_t cfbt_timer_initialized = ATOMIC_INIT(0);
...
int set_cfbt_timer_initialized(int value)
{
	if (value != 0 && value != 1) {
		return -EINVAL;
	}
	atomic_set(&cfbt_timer_initialized, value);
	return 0;
}

int get_cfbt_timer_initialized(void)
{
	return atomic_read(&cfbt_timer_initialized);
}
...
void __process_cfbt_timeout(struct timer_list *t)
{
	...
	struct cfbt_frame_group *group = NULL;
	...
	timer_data = from_timer(timer_data, t, timer);
	if (!timer_data)
		return;

	group = timer_data->grp;
	if (!group)
		goto out_release;
...
out_release:
	atomic_set(&timer_data->in_use, 0);
}
...
bool initialize_timer(void)
{
	...
	for (index = 0; index < MAX_TIMER_COUNT; index++) {
		...
		atomic_set(&cfbt_timers[index].in_use, 0);
...
struct timer_data *allocate_timer(void)
{
	...
	for (i = 0; i < MAX_TIMER_COUNT; i++) {
		...
		if (!timer_pending(&t->timer) && t->initialized && atomic_cmpxchg(&t->in_use, 0, 1) == 0) {
