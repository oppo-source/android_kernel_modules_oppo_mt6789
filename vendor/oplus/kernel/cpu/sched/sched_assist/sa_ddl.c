...
void oplus_set_task_ddl(struct task_struct *task, u64 ddl)
{
	...
	} else if (task->se.on_rq && !task_on_cpu(rq, task) && !task->se.sched_delayed) {
		...
		struct task_struct *curr, bool *preempt, bool *nopreempt)
{
	...
	if (ddl_p) {
		...
		u64 now = rq_clock(rq);
		if (IS_ERR_OR_NULL(ots_p) || p->se.sched_delayed)
			return;
...
void oplus_enqueue_ddl_node(struct rq *rq, struct task_struct *p)
{
	...
	/*
	 * Task enqueued with ENQUEUE_DELAYED flag is handled in
	 * android_rvh_enqueue_task_handler, so we just ignore it here.
	 */
	if (!p || !oplus_get_task_ddl(p) || IS_ERR_OR_NULL(ots)
			|| p->se.sched_delayed || test_bit(OTS_STATE_DDL_ENQUEUE_DELAYED, &ots->state))
		return;
...
static struct oplus_task_struct *pick_ddl_task(struct rq *rq, bool *active_preempted)
{
	...
	for (node = left; node; node = rb_next(node)) {
		...
		if (ots->task && ots->task->se.sched_delayed)
			continue;
...
static void android_rvh_enqueue_task_handler(void *unused, struct rq *rq,
		struct task_struct *p, int flags)
{
	...
	if (IS_ERR_OR_NULL(ots) || !test_task_is_fair(p) || !oplus_get_task_ddl(p))
		return;

	/*
	 * Now we are in enqueue_delayed path, deal with the ddl status.
	 * Enqueued with ENQUEUE_DELAYED path maybe miss catched in enqueue_entity,
	 * so we handle the ENQUEUE_DELAYED enqueue here.
	 */
	if (flags & ENQUEUE_DELAYED) {
		...
		set_bit(OTS_STATE_DDL_ENQUEUE_DELAYED, &ots->state);

		spin_lock_irqsave(&orq->ddl_lock, irqflag);
		smp_mb__after_spinlock();
		if (!oplus_rbnode_empty(&ots->ddl_node)) {
			/* Requeue the entity in ddl tree */
			rb_erase_cached(&ots->ddl_node, &orq->ddl_root);
			rb_add_cached(&ots->ddl_node, &orq->ddl_root, runnable_at_comp);
			spin_unlock_irqrestore(&orq->ddl_lock, irqflag);
			return;
...
static void register_oplus_ddl_hooks(void)
{
	register_trace_android_rvh_enqueue_task(android_rvh_enqueue_task_handler, NULL);
}
