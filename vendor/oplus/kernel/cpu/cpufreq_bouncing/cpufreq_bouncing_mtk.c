...
static void cb_reset_qos(int cpu)
{
	...
	if (
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_CPU_QOS_ARBITER)
	oplus_update_freq_qos_request(QOS_OWNER_CB, &cb->qos_req, FREQ_QOS_MAX_DEFAULT_VALUE) < 0
#else
	freq_qos_update_request(&cb->qos_req, FREQ_QOS_MAX_DEFAULT_VALUE) < 0
...
#else
				freq_qos_add_request(&pol->constraints, &cb->qos_req,
					FREQ_QOS_MAX, FREQ_QOS_MAX_DEFAULT_VALUE) < 0
#endif /* CONFIG_OPLUS_FEATURE_CPU_QOS_ARBITER */
) {
