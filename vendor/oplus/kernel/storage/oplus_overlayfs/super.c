...
{
	int err;
	pr_err("by zhangxueqiang test ovl_init start\n");
	ovl_inode_cachep = kmem_cache_create("oplus_ovl_inode",
					     sizeof(struct ovl_inode), 0,
					     (SLAB_RECLAIM_ACCOUNT|
					      SLAB_ACCOUNT),
					     ovl_inode_init_once);
	if (ovl_inode_cachep == NULL) {
		pr_err("by zhangxueqiang test ovl_init ovl_inode_cachep is null\n");
		return -ENOMEM;
	}

	err = register_filesystem(&ovl_fs_type);
	if (!err) {
		pr_err("by zhangxueqiang test ovl_init register_filesystem success\n");
		return 0;
	}

	kmem_cache_destroy(ovl_inode_cachep);
	pr_err("by zhangxueqiang test ovl_init destroy module\n");
	return err;
}
