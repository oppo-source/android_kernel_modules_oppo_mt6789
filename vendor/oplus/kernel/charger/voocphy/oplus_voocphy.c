...
#define ALLOW_FAST_CHARGING_VBAT    4350
bool oplus_voocphy_check_fastchg_real_allow(void)
{
	struct oplus_voocphy_manager *chip = g_voocphy_chip;
	int batt_soc = 0;
	...
	if (batt_soc >= chip->vooc_low_soc && batt_soc <= chip->vooc_high_soc
		...
		if((soc < 0) && (oplus_chg_get_batt_volt() > ALLOW_FAST_CHARGING_VBAT))
			ret = false;//When the power meter is not initialized successfully at power on
		else
			ret = true;
	} else {
		ret = false;
