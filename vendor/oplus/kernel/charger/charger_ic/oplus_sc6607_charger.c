...
static bool oplus_sc6607_check_chrdet_status(void)
{
	...
	if (!chg_chip->camera_on && !g_chip->power_good) {
		mdelay(250);
		if (chg_chip->camera_on) {
			return true;
		}
...
#endif
{
	...
	if (chip->tcpc != NULL) {
		...
		if (!IS_ERR_OR_NULL(&chip->tcpc->pd_port)) {
			if (chip->pd_type == PD_CONNECT_NONE && chip->tcpc->pd_port.pd_connect_state != PD_CONNECT_NONE) {
				mutex_lock(&chip->charger_pd_lock);
				chip->pd_type = chip->tcpc->pd_port.pd_connect_state;
				mutex_unlock(&chip->charger_pd_lock);
				...
				sc6607_inform_charger_type(g_chip);
				pr_err("tanshiyou PD Notify ready\r\n");
			}
		}
	} else {
