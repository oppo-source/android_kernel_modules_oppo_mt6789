...
static void should_drain_all_pages(void *data,
...
static void rmqueue_pcplist_override_batch(void *ignore, int *batch)
{
...
static int register_dstate_opt_vendor_hooks(void)
{
	...
	ret = register_trace_android_vh_rmqueue_pcplist_override_batch(rmqueue_pcplist_override_batch, NULL);
	if (ret != 0) {
		pr_err("register_trace_android_vh_rmqueue_pcplist_override_batch failed! ret=%d\n", ret);
		goto out;
	}
...
{
	unregister_trace_android_vh_rmqueue_pcplist_override_batch(rmqueue_pcplist_override_batch, NULL);
