...
static unsigned int do_single_lsc_cjtele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_cjtele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_cjtele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_cjtele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
// static int do_ois_cjtele(struct EEPROM_DRV_FD_DATA *pdata, struct ois_cal_data *data);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x01AC010F, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x000000D0, 0x0000074C, do_single_lsc_cjtele},
		{0x00000001, 0x00000020, 0x0000000E, do_2a_gain_cjtele}, //Start address, block size is useless
		{0x00000001, 0x00000900, 0x00000DB4, do_pdaf_cjtele},
		{0x00000000, 0x00000FAE, 0x00000550, do_stereo_data},
		{0x00000001, 0x00000000, 0x00008000, do_dump_all}, //temp 4000,
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_cjtele}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT cjtele_op_eeprom = {
	.name = "cjtele_op_eeprom",
	...
	.enable_preload = 1,
	.preload_size = 0x8000,
	// .parsing_ois_cal_data_from_preload = do_ois_cjtele,
};
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (pCamCalData->DataVer >= CAM_CAL_TYPE_NUM) {
		...
		printk("cjtele_eeprom Read Failed\n");
		show_cmd_error_log(pCamCalData->Command);
		return err;
	}
	if (block_size != CAM_CAL_SINGLE_LSC_SIZE)
		printk("cjtele_eeprom block_size(%d) is not match (%d)\n",
	...
	if (table_size > 0) {
		...
		printk("cjtele_eeprom u4Offset=%d u4Length=%d", start_addr, table_size);
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	printk("cjtele_eeprom S2aBitEn=0x%02x", pCamCalData->Single2A.S2aBitEn);
	...
	if (0x1 & AWBAFConfig) {
		...
		if (read_data_size > 0)	{
			...
			printk("cjtele_eeprom Light source calibration 5100K value R/G:%d, B/G:%d",rgCalValue, bgCalValue);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			printk("cjtele_eeprom Read CalGain Failed\n");
			show_cmd_error_log(pCamCalData->Command);
		}
		/* AWB Unit Gain (5000K) */
		printk("cjtele_eeprom 5000K AWB\n");
		awb_offset = 0x0020;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				awb_offset, 8, (unsigned char *)&CalGain);
		if (read_data_size > 0)	{
			printk("cjtele_eeprom Read CalGain OK %x\n", read_data_size);
			CalR  = CalGain & 0xFFFF;
			...
			if (CalR > CalG)
				...
				if (CalG > CalB)
					...
					tempMax = CalB;
			printk("cjtele_eeprom UnitR:%d, UnitG:%d, UnitB:%d, New Unit Max=%d",
		...
		if (read_data_size > 0)	{
			printk("cjtele_eeprom Read FacGain OK\n");
			FacR  = FacGain & 0xFFFF;
			...
			if (FacR > FacG)
				...
				if (FacG > FacB)
					...
					tempMax = FacB;
			printk("cjtele_eeprom GoldenR:%d, GoldenG:%d, GoldenB:%d, New Golden Max=%d",
					FacR, FacG, FacB, tempMax);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			printk("cjtele_eeprom Read FacGain Failed\n");
			show_cmd_error_log(pCamCalData->Command);
		}
		...
		tempMax = 0;
		printk("cjtele_eeprom 4000K AWB\n");
		awb_offset = 0x0032;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				awb_offset, 8, (unsigned char *)&CalGain);
		if (read_data_size > 0)	{
		...
		CalR = 0; CalGr = 0; CalGb = 0; CalG = 0; CalB = 0;
		tempMax = 0;
		printk("cjtele_eeprom 2850K AWB\n");
		debug_log( "3100K AWB\n");
		...
		if (read_data_size > 0)	{
			...
			rgCalValue  = CalValue & 0xFFFF;
			bgCalValue = (CalValue >> 16) & 0xFFFF;
			printk("cjtele_eeprom Light source calibration value 3100 R/G:%d, B/G:%d",rgCalValue, bgCalValue);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			printk("cjtele_eeprom Read CalGain Failed\n");
		...
		if (read_data_size > 0)	{
			...
			CalG  = ((CalGr + CalGb) + 1) >> 1;
			CalB  = (CalGain >> 48) & 0xFFFF;
			printk("cjtele_eeprom CalR:%d, CalB:%d",CalR, CalB);
			CalR  = CalR * rgCalValue / 1000;
			CalB  = CalB * bgCalValue / 1000;
			if (CalR > CalG)
				/* R > G */
	...
	if (0x2 & AWBAFConfig) {
		...
		// #ifdef DEBUG_CALIBRATION_LOAD
		printk("======================AF CAM_CAL==================\n");
		printk("cjtele_eeprom [AFInf] = %d\n", AFInf);
		printk("cjtele_eeprom [AFMacro] = %d\n", AFMacro);
		printk("cjtele_eeprom [AF_50cm] = %d\n", AF_50cm);
		printk("======================AF CAM_CAL==================\n");
		// #endif
	}
	return err;
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	for (int i = 0; i < sizeof(arr) / sizeof(arr[0]); i++) {
		printk("cjtele_eeprom  Reading segment %d: start_addr =%x, block_size =%d\n", i , arr[i][0], arr[i][1]);
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
	...
	if (pCamCalData->PDAF.Size_of_PDAF == block_size) {
		err = CAM_CAL_ERR_NO_ERR;
		printk("cjtele_eeprom  PDAF data read ok\n");
	} else {
		printk("cjtele_eeprom  ERROR: PDAF data size is not match %d %d\n", pCamCalData->PDAF.Size_of_PDAF, block_size);
	}

	// printk("PDAF.Data values:\n");
	// for (int i = 0; i < pCamCalData->PDAF.Size_of_PDAF; i++) {
	// 	printk("cjtele_eeprom  PDAF.Data[%d] = 0x%02X\n", i, pCamCalData->PDAF.Data[i]);
	// }
