...
static unsigned int do_single_lsc_cjmain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_cjmain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_cjmain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_cjmain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x01AF0144, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000001, 0x00000000, 0x00000002, do_module_version},
		{0x00000001, 0x00000000, 0x00000011, do_part_number},
		{0x00000001, 0x00002500, 0x0000074C, do_single_lsc_cjmain},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_cjmain}, //Start address, block size is useless
		{0x00000001, 0x00002D00, 0x00001AA2, do_pdaf_cjmain},
		...
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_cjmain}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT cjmain_op_eeprom = {
	.name = "cjmain_op_eeprom",
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	for (int i = 0; i < sizeof(arr) / sizeof(arr[0]); i++) {
		printk("cjmain_eeprom  Reading segment %d: start_addr =%x, block_size =%d\n", i , arr[i][0], arr[i][1]);
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
	...
	if (pCamCalData->PDAF.Size_of_PDAF == block_size) {
		err = CAM_CAL_ERR_NO_ERR;
		printk("cjmain_eeprom  PDAF data read ok\n");
	} else {
		printk("cjmain_eeprom  ERROR: PDAF data size is not match %d %d\n", pCamCalData->PDAF.Size_of_PDAF, block_size);
	}

	// printk("PDAF.Data values:\n");
	// for (int i = 0; i < pCamCalData->PDAF.Size_of_PDAF; i++) {
	// 	printk("cjmain_eeprom  PDAF.Data[%d] = 0x%02X\n", i, pCamCalData->PDAF.Data[i]);
	// }
