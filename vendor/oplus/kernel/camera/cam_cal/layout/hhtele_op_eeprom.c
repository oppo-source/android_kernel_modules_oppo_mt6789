...
static unsigned int do_single_lsc_hhtele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_hhtele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_hhtele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_hhtele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static int do_ois_hhtele(struct EEPROM_DRV_FD_DATA *pdata, struct ois_cal_data *data);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	...
	0x00000006, 0x01B1012F, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00001070, 0x0000074C, do_single_lsc_hhtele},
		{0x00000001, 0x00000020, 0x0000000E, do_2a_gain_hhtele}, //Start address, block size is useless
		{0x00000001, 0x000017D0, 0x00001028, do_pdaf_hhtele},
		...
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_hhtele}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT hhtele_op_eeprom = {
	.name = "hhtele_op_eeprom",
	...
	.enable_preload = 1,
	.preload_size = 0x8000,
	.parsing_ois_cal_data_from_preload = do_ois_hhtele,
};
...
static int do_ois_hhtele(struct EEPROM_DRV_FD_DATA *pdata, struct ois_cal_data *data)
{
	...
	u8 low_byte;
	u8 high_byte;
	const unsigned int OIS_INDEX_SIZE = 2; // 2 bytes per index
	const unsigned int OIS_TLB_SIZE = 258; // 129 * 2 bytes
	const unsigned int OIS_BLOCK_SIZE = OIS_INDEX_SIZE + OIS_TLB_SIZE; // 2 + 258 = 260 bytes
	const unsigned int OIS_BLOCK_COUNT = 9; // 9 blocks in total
	if (pdata->pdrv->config_info.mp_eeprom_preload == NULL)
		return -1;
	offset = 0x5A90;
	length = 2341;
	memcpy(data, pdata->pdrv->config_info.mp_eeprom_preload + offset, length);
	for (block = 0; block < OIS_BLOCK_COUNT; block++) {
		block_start = &(data->ois_table[0]) + block * OIS_BLOCK_SIZE;
		tlb_start = block_start + OIS_INDEX_SIZE;
		// mirror ois_TLB
		for (i = 0; i < 64; i++) {
			front_idx = i * 2;
			back_idx = (128 - i) * 2;

			low_byte = tlb_start[front_idx];
			high_byte = tlb_start[front_idx + 1];

			tlb_start[front_idx] = tlb_start[back_idx];
			tlb_start[front_idx + 1] = tlb_start[back_idx + 1];

			tlb_start[back_idx] = low_byte;
			tlb_start[back_idx + 1] = high_byte;
		}
	}
	return 0;
}
