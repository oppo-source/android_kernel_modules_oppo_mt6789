...
#define DRIVER_NAME                  "dw9800s_24081"
#define DW9800S_I2C_SLAVE_ADDR        0x18

#define LOG_INF(format, args...)                                               \
	pr_info(DRIVER_NAME " [%s] " format, __func__, ##args)

#define DW9800S_NAME				 "dw9800s_24081"
...
#define DW9800S_VIN_MIN			(2800000)
#define DW9800S_VIN_MAX			(2800000)
#define DW9800S_RESONANCE_REG 0x07
#define DW9800S_RESONANCE_VAL 0x54
...
static int dw9800s_init(struct dw9800s_device *dw9800s)
{
	...
	int retry_cnt = 100;
	do{
		af_reg_val = i2c_smbus_read_byte_data(client, DW9800S_RESONANCE_REG);
		LOG_INF("DW9800S_RESONANCE_REG(0x%x) \n", af_reg_val);
		mdelay(1);
		retry_cnt--;
	} while (af_reg_val != DW9800S_RESONANCE_VAL && retry_cnt > 0);
...
static int dw9800s_power_on(struct dw9800s_device *dw9800s)
{
	...
	min = DW9800S_VIN_MIN;
	max = DW9800S_VIN_MAX;
	ret = regulator_set_voltage(dw9800s->vin, min, max);
	if (ret) {
...
static const struct of_device_id dw9800s_of_table[] = {
	{ .compatible = "oplus,dw9800s_24081" },
	{ },
};
