 ...
 *============================================================================
 ****************************************************************************/
#include "hhtelemipiraw_Sensor.h"

#define HHTELE_EEPROM_READ_ID	0xA1
#define HHTELE_EEPROM_WRITE_ID	0xA0
#define HHTELE_AF_WRITE_ID	0x5A
#define HHTELE_MAX_OFFSET		0x4000
#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "hhtele_camera_sensor"
...
static int hhtele_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhtele_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhtele_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhtele_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhtele_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhtele_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhtele_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt, u64 sof_ts);
static int hhtele_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void get_sensor_cali(void* arg);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int hhtele_get_otp_qcom_pdaf_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhtele_get_otp_qcom_pdaf_offset_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhtele_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhtele_control(struct subdrv_ctx *ctx,
...
static void calculate_prsh_length_lines(struct subdrv_ctx *ctx,
	...
	enum SENSOR_SCENARIO_ID_ENUM scenario_id);
static int hhtele_disable_fast_mode(struct subdrv_ctx *ctx);
/* STRUCT */

struct mtk_sensor_ctle_param hhtele_static_ctle_param = {
	.eq_bw = 0x3,
};

static struct eeprom_map_info hhtele_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, hhtele_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, hhtele_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, hhtele_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, hhtele_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, hhtele_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, hhtele_get_otp_checksum_data},
	{SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, hhtele_get_min_shutter_by_scenario_adapter},
	{SENSOR_FEATURE_SET_AWB_GAIN, hhtele_set_awb_gain},
	{SENSOR_FEATURE_GET_OTP_QCOM_PDAF_DATA, hhtele_get_otp_qcom_pdaf_data},
	{SENSOR_FEATURE_GET_OTP_QCOM_PDAF_OFFSET_DATA, hhtele_get_otp_qcom_pdaf_offset_data},
	{SENSOR_FEATURE_SET_AWB_GAIN, hhtele_set_awb_gain},
	{SENSOR_FEATURE_SET_GAIN, hhtele_set_gain},
};
...
static struct SET_PD_BLOCK_INFO_T imgsensor_partial_pd_info = {
	...
	.iMirrorFlip = IMAGE_V_MIRROR,//0:IMAGE_NORMAL,1:IMAGE_H_MIRROR,2:IMAGE_V_MIRROR,3:IMAGE_HV_MIRROR
	.i4FullRawW = 4096,
	.i4FullRawH = 3072,
	.i4ModeIndex = 0,
	.i4VCPackNum = 1,
...
static struct SET_PD_BLOCK_INFO_T imgsensor_partial_pd_info_hdr = {
	.i4OffsetX = 16,
	.i4OffsetY = 32,
	.i4PitchX = 8,
	.i4PitchY = 16,
...
static struct subdrv_mode_struct mode_struct[] = {
	{/* B11-S3 4096x3072 @30.1FPS QBIN(VBIN) with All-PD VB_max seamless D*/
		...
		.mode_setting_table = hhtele_preview_setting,
		.mode_setting_len = ARRAY_SIZE(hhtele_preview_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhtele_seamless_preview,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhtele_seamless_preview),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 7500,
		.framelength = 3888,
	...
	{/* Reg_B9-S1_QBIN(VBIN)_4096x3072_30.1FPS with PDAF VB_max */
		...
		.mode_setting_table = hhtele_capture_setting,
		.mode_setting_len = ARRAY_SIZE(hhtele_capture_setting),
	...
	{/* Reg_B10-S5_QBIN(VBIN)_4096x2304_30.1FPS with PDAF VB_max*/
		...
		.mode_setting_table = hhtele_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(hhtele_normal_video_setting),
		.seamless_switch_group = 3,
		.seamless_switch_mode_setting_table = hhtele_seamless_normal_video,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhtele_seamless_normal_video),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 7500,
		.framelength = 3888,
	...
	{/* B-1 QBIN_4096x2304 @60FPS with PDAF VB_max */
		...
		.mode_setting_table = hhtele_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(hhtele_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* B2 4096x3072 @60FPS QBIN(VBIN) with Partial-PD VB_max */
		...
		.mode_setting_table = hhtele_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(hhtele_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* L4-S3 4096x3072 @30.1FPS QBIN 2exp-LBMF with Partial-PD VB_max seamless D*/
		...
		.mode_setting_table = hhtele_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(hhtele_custom1_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhtele_seamless_custom1,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhtele_seamless_custom1),
		...
		.pdaf_cap = TRUE,
		.imgsensor_pd_info = &imgsensor_partial_pd_info_hdr,
		.ae_binning_ratio = 1000,
		.fine_integ_line = 0,
		.csi_param = {
			.cphy_settle = 58,
	...
	{/* F5-S3 4096x3072 @30.1FPS Full-RMSC-Crop QBC with PDAF VB_max */
		...
		.mode_setting_table = hhtele_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(hhtele_custom2_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhtele_seamless_custom2,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhtele_seamless_custom2),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 8960,
		.framelength = 3256,
	...
	{/* D Full RMSC 15fps bayer with PDAF VB_max */
		...
		.mode_setting_table = hhtele_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(hhtele_custom3_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhtele_seamless_custom3,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhtele_seamless_custom3),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 878400000,
	...
	{/* D Full RMSC 15fps Qbayer with PDAF VB_max */
		...
		.mode_setting_table = hhtele_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(hhtele_custom4_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhtele_seamless_custom4,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhtele_seamless_custom4),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 878400000,
	...
	{/* B11-S3 4096x3072 @30.1FPS QBIN(VBIN) with All-PD VB_max seamless D*/
		...
		.mode_setting_table = hhtele_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(hhtele_custom5_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = hhtele_seamless_custom5,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhtele_seamless_custom5),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 7500,
		.framelength = 3888,
	...
	{/* L4-S3 4096x3072 @30.1FPS QBIN 2exp-LBMF with Partial-PD VB_max seamless D*/
		...
		.mode_setting_table = hhtele_custom6_setting,
		.mode_setting_len = ARRAY_SIZE(hhtele_custom6_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = hhtele_seamless_custom6,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhtele_seamless_custom6),
		.hdr_mode = HDR_RAW_LBMF,
		.raw_cnt = 2,
		.exp_cnt = 2,
		.pclk = 878400000,
	...
	{/* V10 1664x1248 @30.1FPS QBIN(VBIN)-V2H2 */
		...
		.mode_setting_table = hhtele_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(hhtele_custom7_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* F6-S5 4096x2304 @30.1FPS Full-RMSC_Crop w/ PD VB_MAX seamless reg_A-1/B */
		...
		.mode_setting_table = hhtele_custom8_setting,
		.mode_setting_len = ARRAY_SIZE(hhtele_custom8_setting),
		.seamless_switch_group = 3,
		.seamless_switch_mode_setting_table = hhtele_seamless_custom8,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhtele_seamless_custom8),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 8960,
		.framelength = 3256,
	...
	{/*C-5 QBIN-V2H2 2048x1152_240FPS w/o PDAF VB_max*/
		...
		.mode_setting_table = hhtele_custom9_setting,
		.mode_setting_len = ARRAY_SIZE(hhtele_custom9_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* F5-S3 4096x3072 @30FPS Full-RMSC-Crop with PDAF VB_max */
		...
		.mode_setting_table = hhtele_custom10_setting,
		.mode_setting_len = ARRAY_SIZE(hhtele_custom10_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhtele_seamless_custom10,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhtele_seamless_custom10),
		.hdr_mode = HDR_NONE,
		.pclk = 878400000,
		.linelength = 8960,
		.framelength = 3256,
...
static struct subdrv_static_ctx static_ctx = {
	...
	.ana_gain_type = 0,
	.ana_gain_step = 1,
	.ana_gain_table = hhtele_ana_gain_table,
	.ana_gain_table_size = sizeof(hhtele_ana_gain_table),
	.tuning_iso_base = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 6,
	...
	.init_setting_table = hhtele_init_setting,
	.init_setting_len = ARRAY_SIZE(hhtele_init_setting),
	.mode = mode_struct,
	...
	.ctle_param = &hhtele_static_ctle_param,
};
...
static struct subdrv_ops ops = {
	...
	.disable_fast_mode = hhtele_disable_fast_mode,
};
...
const struct subdrv_entry hhtele_mipi_raw_entry = {
	.name = "hhtele_mipi_raw",
	.id = HHTELE_SENSOR_ID,
	.pw_seq = pw_seq,
...
static u16 hhtele_ob_optimize_30db[] = {
	0x3B00, 0x08,
	0x3B01, 0x8A,
	0x38D0, 0x08,
	0x38D1, 0x52,
...
static u16 hhtele_ob_optimize_36db[] = {
	0x3B00, 0x03,
	0x3B01, 0xE1,
	0x38D0, 0x00,
	0x38D1, 0xE8,
...
static unsigned int read_hhtele_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != hhtele_eeprom_info[meta_id].meta)
		return -1;

	if (size != hhtele_eeprom_info[meta_id].size)
		return -1;

	addr = hhtele_eeprom_info[meta_id].start;
	readsize = hhtele_eeprom_info[meta_id].size;
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, HHTELE_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
static int hhtele_get_otp_qcom_pdaf_data(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, HHTELE_EEPROM_WRITE_ID >> 1,
            addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, HHTELE_EEPROM_READ_ID >> 1, reg, 0x0E);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, HHTELE_EEPROM_READ_ID >> 1, reg, 0x00);
    }
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if ((pStereodata->uSensorId == HHTELE_SENSOR_ID) && ((data_length - 2 == CALI_DATA_SLAVE_LENGTH)
            ||(data_length - 2 == CALI_DATA_MASTER_LENGTH)||(data_length - 2 == CALI_DATA_SLAVE_TELE_LENGTH))
            && ((data_base == HHTELE_STEREO_START_ADDR)||(data_base == HHTELE_STEREO_120CM_START_ADDR)
            ||(data_base == HHTELE_STEREO_105CM_START_ADDR))) {
        ...
        } else if ((pStereodata->uSensorId == HHTELE_SENSOR_ID) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            && (data_base == HHTELE_AESYNC_START_ADDR)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                read_cmos_eeprom_8(ctx, HHTELE_AESYNC_START_ADDR),
                read_cmos_eeprom_8(ctx, HHTELE_AESYNC_START_ADDR+1),
                read_cmos_eeprom_8(ctx, HHTELE_AESYNC_START_ADDR+2),
                read_cmos_eeprom_8(ctx, HHTELE_AESYNC_START_ADDR+3),
                read_cmos_eeprom_8(ctx, HHTELE_AESYNC_START_ADDR+4),
                read_cmos_eeprom_8(ctx, HHTELE_AESYNC_START_ADDR+5),
                read_cmos_eeprom_8(ctx, HHTELE_AESYNC_START_ADDR+6),
                read_cmos_eeprom_8(ctx, HHTELE_AESYNC_START_ADDR+7));
        ...
        LOG_INF("hhtele write_Module_data pStereodata is null\n");
        return -1;
    }
    return ret;
}
...
static int hhtele_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	case EEPROM_STEREODATA_MT_MAIN:
		read_hhtele_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA,
				(BYTE *)feature_return_para_32, *len);
		break;
	case EEPROM_STEREODATA_MT_MAIN_105CM:
		read_hhtele_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA_105CM,
				(BYTE *)feature_return_para_32, *len);
		break;
	case EEPROM_STEREODATA_TT_MASETR_120CM:
		read_hhtele_eeprom_info(ctx, EEPROM_META_STEREO_MW_MAIN_DATA,
				(BYTE *)feature_return_para_32, *len);
		break;
	}
	return 0;
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, HHTELE_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "hhtele read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "hhtele read_otp_info end\n");
}
...
static u16 hhtele_feedback_awbgain[] = {
	0x0B8E, 0x01,
	0x0B8F, 0x00,
	0x0B90, 0x02,
	0x0B91, 0x28,
...
static void feedback_awbgain(struct subdrv_ctx *ctx, kal_uint32 r_gain, kal_uint32 b_gain)
{
	...
	hhtele_feedback_awbgain[5] = r_gain_int;
	hhtele_feedback_awbgain[7] = (r_gain - r_gain_int * 512) / 2;
	hhtele_feedback_awbgain[9] = b_gain_int;
	hhtele_feedback_awbgain[11] = (b_gain - b_gain_int * 512) / 2;
	subdrv_i2c_wr_regs_u8(ctx, hhtele_feedback_awbgain,
		ARRAY_SIZE(hhtele_feedback_awbgain));
}
...
static int open(struct subdrv_ctx *ctx)
{
	...
	adaptor_i2c_wr_u8_u8(ctx->i2c_client, HHTELE_AF_WRITE_ID >> 1, 0x02, 0x0);
	// wait for af active
	mdelay(3);
...
void hhtele_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
		enum SENSOR_SCENARIO_ID_ENUM scenario_id,
...
int hhtele_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64 *feature_data = (u64 *) para;
	hhtele_get_min_shutter_by_scenario(ctx,
...
static int hhtele_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len) {
	...
	if(ctx->current_scenario_id == SENSOR_SCENARIO_ID_NORMAL_PREVIEW ||
		...
		if (rg_gain > 0x3DF8) {
			if(wr_ob_type != 1) {
				wr_ob_type = 1;
				subdrv_i2c_wr_regs_u8(ctx, hhtele_ob_optimize_36db,
					ARRAY_SIZE(hhtele_ob_optimize_36db));
			}
		} else {
			if(wr_ob_type != 2) {
				wr_ob_type = 2;
				subdrv_i2c_wr_regs_u8(ctx, hhtele_ob_optimize_30db,
					ARRAY_SIZE(hhtele_ob_optimize_30db));
			}
		}
	} else {
		wr_ob_type = 0;
...
int hhtele_control(struct subdrv_ctx *ctx,
			enum SENSOR_SCENARIO_ID_ENUM scenario_id,
			MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
