 ...
 *============================================================================
 ****************************************************************************/
#include "hhmainmipiraw_Sensor.h"

#define HHMAIN_EEPROM_READ_ID	0xA0
#define HHMAIN_EEPROM_WRITE_ID	0xA1
#define HHMAIN_AF_WRITE_ID	0x18
#define HHMAIN_MAX_OFFSET		0x8000
#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "hhmain_camera_sensor"
...
static int hhmain_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhmain_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhmain_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhmain_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhmain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhmain_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhmain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt, u64 sof_ts);
static int hhmain_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int hhmain_get_otp_qcom_pdaf_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);

static int hhmain_get_otp_qcom_pdaf_offset_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhmain_set_video_mode(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhmain_disable_fast_mode(struct subdrv_ctx *ctx);
/* STRUCT */

struct mtk_sensor_ctle_param hhmain_static_ctle_param = {
	.eq_bw = 0x3,
};

struct mtk_sensor_ctle_param hhmain_cdr_0x9 = {
	.cdr_delay = 0x9,
	.eq_bw = 0x3,
};

static struct eeprom_map_info hhmain_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, hhmain_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, hhmain_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, hhmain_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, hhmain_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, hhmain_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, hhmain_get_otp_checksum_data},
	{SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, hhmain_get_min_shutter_by_scenario_adapter},
	{SENSOR_FEATURE_SET_AWB_GAIN, hhmain_set_awb_gain},
	{SENSOR_FEATURE_GET_OTP_QCOM_PDAF_DATA, hhmain_get_otp_qcom_pdaf_data},
	{SENSOR_FEATURE_GET_OTP_QCOM_PDAF_OFFSET_DATA, hhmain_get_otp_qcom_pdaf_offset_data},
	{SENSOR_FEATURE_SET_VIDEO_MODE, hhmain_set_video_mode},
};

static u32 hhmain_dcg_ratio_table_ratio4[] = {4000};
...
static struct subdrv_mode_struct mode_struct[] = {
	{/* Reg_B4-S4 4096x3072 @30fps QBIN(VBIN) with PDAF VB_max seamless A-1*/
		...
		.mode_setting_table = hhmain_preview_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_preview_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhmain_seamless_preview,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhmain_seamless_preview),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/* Reg_B-1_QBIN(VBIN)_4096x3072_30fps */
		...
		.mode_setting_table = hhmain_capture_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_capture_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = hhmain_seamless_capture,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhmain_seamless_capture),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/*Reg_B7-S8_4096x2304_30FPS*/
		.frame_desc = frame_desc_vid,
		.num_entries = ARRAY_SIZE(frame_desc_vid),
		.mode_setting_table = hhmain_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_normal_video_setting),
		.seamless_switch_group = 4,
		.seamless_switch_mode_setting_table = hhmain_seamless_normal_video,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhmain_seamless_normal_video),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/* Reg_G-1_4096x2304_60FPS */
		...
		.mode_setting_table = hhmain_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_hs_video_setting),
	...
	{   /* G7-S3 4096x2304 @60FPS QBIN DCG-HDR RAW12 w/ Partial-PD VB_max*/
		...
		.mode_setting_table = hhmain_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		...
		.dcg_info = {
			...
			.dcg_gain_table = hhmain_dcg_ratio_table_ratio4,
			.dcg_gain_table_size = sizeof(hhmain_dcg_ratio_table_ratio4),
		},
	...
	{/* Reg_H-1  QBIN(VBIN)-V2H2 FHD 2048x1152_240FPS*/
		...
		.mode_setting_table = hhmain_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* Reg_B-2 QBIN(VBIN)_4096x3072 24FPS */
		...
		.mode_setting_table = hhmain_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom2_setting),
		.seamless_switch_group = 3,
		.seamless_switch_mode_setting_table = hhmain_seamless_custom2,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhmain_seamless_custom2),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/* Reg_A-1 8192x6144_30FPS remosaic*/
		...
		.mode_setting_table = hhmain_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom3_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhmain_seamless_custom3,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhmain_seamless_custom3),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/* Reg-L1-S4 QBIN(VBIN) 4096x3072_2-exp LBMF 30FPS with PDAF VB_max */
		...
		.mode_setting_table = hhmain_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom4_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhmain_seamless_custom4,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhmain_seamless_custom4),
	...
	{/* Reg-F3-S2 QRMSC_4096x3072_30FPS for insensor zoom--bayer*/
		...
		.mode_setting_table = hhmain_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom5_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = hhmain_seamless_custom5,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhmain_seamless_custom5),
		.hdr_mode = HDR_NONE,
		.pclk = 2196000000,
		.linelength = 11344,
		.framelength = 6451,
	...
	{/*Reg H-3_QBIN-V2H2 FHD 2048x1152_480FPS */
		...
		.mode_setting_table = hhmain_custom6_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom6_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*L2-S4 4096x3072 @30FPS Izoom 2exp-LBMF Qbayer w/ PDAF  VB_max seamless reg_A-1*/
		...
		.mode_setting_table = hhmain_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom7_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhmain_seamless_custom7,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhmain_seamless_custom7),
		.hdr_mode = HDR_RAW_LBMF,
		.raw_cnt = 2,
		.exp_cnt = 2,
		.pclk = 2196000000,
	...
	{/*Reg_B5-S2 4096x3072 @60FPS QBIN(VBIN) with PDAF VB_max*/
		...
		.mode_setting_table = hhmain_custom8_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom8_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*G4-S8 4096x2304 30FPS DCG AP Merge*/
		.frame_desc = frame_desc_cus9,
		.num_entries = ARRAY_SIZE(frame_desc_cus9),
		.mode_setting_table = hhmain_custom9_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom9_setting),
		.seamless_switch_group = 4,
		.seamless_switch_mode_setting_table = hhmain_seamless_custom9,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhmain_seamless_custom9),
	...
	{/* Reg_G3-S2 4096x2304 @60FPS QBIN DCG-HDR RAW10 w/ Partial-PD VB_max*/
		...
		.mode_setting_table = hhmain_custom10_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom10_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_RAW_DCG_RAW,
	...
	{/*reg_B23 1984x1488 @30FPS QBIN(VBIN) RST=20.8ms (QBin-Crop)*/
		.frame_desc = frame_desc_cus11,
		.num_entries = ARRAY_SIZE(frame_desc_cus11),
		.mode_setting_table = hhmain_custom11_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom11_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* F9-S4 8192x6144 @30FPS Full-RAW w/ PD VB_max seamless reg_B4-S4 */
		...
		.mode_setting_table = hhmain_custom12_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom12_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhmain_seamless_custom12,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhmain_seamless_custom12),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{   /* Reg_F10-S4 4096x3072 @30FPS Full-RAW-Crop w/ PD VB_max QBC */
		.frame_desc = frame_desc_cus13,
		.num_entries = ARRAY_SIZE(frame_desc_cus13),
		.mode_setting_table = hhmain_custom13_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom13_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhmain_seamless_custom13,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhmain_seamless_custom13),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2196000000,
	...
	{/* Reg_V3  2048x1536 @30FPS QBIN(VBIN)-V2H2 VB_max*/
		...
		.mode_setting_table = hhmain_custom14_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom14_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* Reg_V4 2048x1536 @15FPS QBIN(VBIN)-V2H2 w/ PD*/
		...
		.mode_setting_table = hhmain_custom15_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom15_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* reg_B8 4096x2304 @120FPS QBIN Partial-PD VB_max*/
		...
		.mode_setting_table = hhmain_custom16_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom16_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* G11-S4 4096x3072 30FPS DCG AP Merge */
		.frame_desc = frame_desc_cus17,
		.num_entries = ARRAY_SIZE(frame_desc_cus17),
		.mode_setting_table = hhmain_custom17_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom17_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhmain_seamless_custom17,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhmain_seamless_custom17),
		.hdr_mode = HDR_RAW_DCG_RAW,
		.raw_cnt = 2,
		.exp_cnt = 2,
		.pclk = 2196000000,
	...
	{/*B-2-1 Izoom 2exp-LBMF 4096x3072 @24FPS w/ PDAF  VB_max RST=13.56ms~15.88ms (seamless  B-2)*/
		...
		.mode_setting_table = hhmain_custom18_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom18_setting),
		.seamless_switch_group = 3,
		.seamless_switch_mode_setting_table = hhmain_seamless_custom18,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhmain_seamless_custom18),
		.hdr_mode = HDR_RAW_LBMF,
		.raw_cnt = 2,
		.exp_cnt = 2,
		.pclk = 2196000000,
	...
	{/*B-2-2 QBIN(VBIN) 2exp-LBMF 4096x3072 @24FPS w/ PDAF  VB_max RST=13.56m~15.88ms (seamless reg_B-2)*/
		...
		.mode_setting_table = hhmain_custom19_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom19_setting),
		.seamless_switch_group = 3,
		.seamless_switch_mode_setting_table = hhmain_seamless_custom19,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhmain_seamless_custom19),
		.hdr_mode = HDR_RAW_LBMF,
		.raw_cnt = 2,
		.exp_cnt = 2,
		.pclk = 2196000000,
	...
	{/* F10-S8 4096x2304 @30FPS Full-RMSC-Crop w/ PD VB_max*/
		.frame_desc = frame_desc_cus20,
		.num_entries = ARRAY_SIZE(frame_desc_cus20),
		.mode_setting_table = hhmain_custom20_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom20_setting),
		.seamless_switch_group = 4,
		.seamless_switch_mode_setting_table = hhmain_seamless_custom20,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhmain_seamless_custom20),
		.hdr_mode = HDR_NONE,
		.pclk = 2196000000,
		.linelength = 11344,
		.framelength = 6451,
	...
	{	/*reg_B24 3840x2160 @120FPS QBIN-Partial PD VB_MAX*/
		.frame_desc = frame_desc_cus21,
		.num_entries = ARRAY_SIZE(frame_desc_cus21),
		.mode_setting_table = hhmain_custom21_setting,
		.mode_setting_len = ARRAY_SIZE(hhmain_custom21_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	...
	.ana_gain_type = 0,
	.ana_gain_step = 1,
	.ana_gain_table = hhmain_ana_gain_table,
	.ana_gain_table_size = sizeof(hhmain_ana_gain_table),
	.tuning_iso_base = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 6,
	...
	.init_setting_table = hhmain_init_setting,
	.init_setting_len = ARRAY_SIZE(hhmain_init_setting),
	.mode = mode_struct,
	...
	.ctle_param = &hhmain_static_ctle_param,
};
...
static struct subdrv_ops ops = {
	...
	.disable_fast_mode = hhmain_disable_fast_mode,
};
...
const struct subdrv_entry hhmain_mipi_raw_entry = {
	.name = "hhmain_mipi_raw",
	.id = HHMAIN_SENSOR_ID,
	.pw_seq = pw_seq,
...
static unsigned int read_hhmain_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != hhmain_eeprom_info[meta_id].meta)
		return -1;

	if (size != hhmain_eeprom_info[meta_id].size)
		return -1;

	addr = hhmain_eeprom_info[meta_id].start;
	readsize = hhmain_eeprom_info[meta_id].size;
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, HHMAIN_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
	...
	ret = adaptor_i2c_wr_p8(ctx->i2c_client, HHMAIN_EEPROM_WRITE_ID >> 1,
			addr, para, len);
...
static kal_int32 write_1st_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
	...
	adaptor_i2c_wr_u8(ctx->i2c_client, (0xB0 | HHMAIN_EEPROM_WRITE_ID) >> 1, reg, 0x0);

	reg = 0x06ca;
	if (enable) {
		adaptor_i2c_wr_u8(ctx->i2c_client, (0xB0 | HHMAIN_EEPROM_WRITE_ID) >> 1, reg, 0x2);
	}
	else {
		adaptor_i2c_wr_u8(ctx->i2c_client, (0xB0 | HHMAIN_EEPROM_WRITE_ID) >> 1, reg, 0x0);
	}

	adaptor_i2c_rd_u8(ctx->i2c_client, HHMAIN_EEPROM_WRITE_ID >> 1, reg, &flag);
...
static kal_int32 write_2nd_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
	...
	if (enable) {
		adaptor_i2c_wr_u8(ctx->i2c_client, HHMAIN_EEPROM_WRITE_ID >> 1, reg, 0x0E);
	}
	else {
		adaptor_i2c_wr_u8(ctx->i2c_client, HHMAIN_EEPROM_WRITE_ID >> 1, reg, 0x00);
	}
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
	u8 flag = 0;
	int ret = 0;
	adaptor_i2c_rd_u8(ctx->i2c_client, HHMAIN_EEPROM_READ_ID >> 1, 0x000D, &flag);
	if (flag == 0x01) {
	...
	ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if (((pStereodata->uSensorId == HHMAIN_SENSOR_ID) && ((data_length - 2) == CALI_DATA_MASTER_LENGTH))
				&& (data_base == HHMAIN_STEREO_START_ADDR || data_base == HHMAIN_STEREO_MT_START_ADDR
				|| data_base == HHMAIN_STEREO_MT_105CM_START_ADDR)) {
		...
		} else if ((pStereodata->uSensorId == HHMAIN_SENSOR_ID) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
				&& (data_base == HHMAIN_AESYNC_START_ADDR)) {
			...
			LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
				read_cmos_eeprom_8(ctx, HHMAIN_AESYNC_START_ADDR),
				read_cmos_eeprom_8(ctx, HHMAIN_AESYNC_START_ADDR+1),
				read_cmos_eeprom_8(ctx, HHMAIN_AESYNC_START_ADDR+2),
				read_cmos_eeprom_8(ctx, HHMAIN_AESYNC_START_ADDR+3),
				read_cmos_eeprom_8(ctx, HHMAIN_AESYNC_START_ADDR+4),
				read_cmos_eeprom_8(ctx, HHMAIN_AESYNC_START_ADDR+5),
				read_cmos_eeprom_8(ctx, HHMAIN_AESYNC_START_ADDR+6),
				read_cmos_eeprom_8(ctx, HHMAIN_AESYNC_START_ADDR+7));
		...
		LOG_INF("hhmain write_Module_data pStereodata is null\n");
		return -1;
	}
	return ret;
}
...
static int hhmain_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	case EEPROM_STEREODATA_MT_MAIN:
		read_hhmain_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA,
				(BYTE *)feature_return_para_32, *len);
		break;
	case EEPROM_STEREODATA_MT_MAIN_105CM:
		read_hhmain_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA_105CM,
				(BYTE *)feature_return_para_32, *len);
		break;
	case EEPROM_STEREODATA_MW_MAIN:
	default:
		read_hhmain_eeprom_info(ctx, EEPROM_META_STEREO_MW_MAIN_DATA,
				(BYTE *)feature_return_para_32, *len);
		break;
	}
	return 0;
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, HHMAIN_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "hhmain read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "hhmain read_otp_info end\n");
}
...
static u16 hhmain_feedback_awbgain[] = {
	0x0B8E, 0x01,
	0x0B8F, 0x00,
	0x0B90, 0x02,
	0x0B91, 0x28,
...
static void feedback_awbgain(struct subdrv_ctx *ctx, kal_uint32 r_gain, kal_uint32 b_gain)
{
	...
	hhmain_feedback_awbgain[5] = r_gain_int;
	hhmain_feedback_awbgain[7] = (r_gain - r_gain_int * 512) / 2;
	hhmain_feedback_awbgain[9] = b_gain_int;
	hhmain_feedback_awbgain[11] = (b_gain - b_gain_int * 512) / 2;
	subdrv_i2c_wr_regs_u8(ctx, hhmain_feedback_awbgain,
		ARRAY_SIZE(hhmain_feedback_awbgain));
}
...
static int open(struct subdrv_ctx *ctx)
{
	...
	adaptor_i2c_wr_u8_u8(ctx->i2c_client, HHMAIN_AF_WRITE_ID >> 1, 0x02, 0x0);
	/* wait for af active */
	mdelay(3);
...
void hhmain_get_min_shutter_by_scenario(struct subdrv_ctx *ctx,
		enum SENSOR_SCENARIO_ID_ENUM scenario_id,
...
int hhmain_get_min_shutter_by_scenario_adapter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64 *feature_data = (u64 *) para;
	hhmain_get_min_shutter_by_scenario(ctx,
