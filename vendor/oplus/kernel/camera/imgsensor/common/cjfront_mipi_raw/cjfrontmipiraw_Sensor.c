 ...
 *============================================================================
 ****************************************************************************/
#include "cjfrontmipiraw_Sensor.h"

#define CJFRONT_EEPROM_READ_ID	0xA9
#define CJFRONT_EEPROM_WRITE_ID   0xA8
#define CJFRONT_MAX_OFFSET		0x4000
#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "cjfront_camera_sensor"
...
#ifndef SUBDRV_I2C_BUF_SIZE
#define SUBDRV_I2C_BUF_SIZE 256
#endif
#define AF_I2C_SLAVE_ADDR 0x18

static u16 module_flag = FALSE;
...
static int cjfront_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjfront_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjfront_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjfront_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjfront_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjfront_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjfront_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjfront_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjfront_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void cjfront_set_shutter_convert(struct subdrv_ctx *ctx, u32 shutter);
static int cjfront_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static int cjfront_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjfront_set_gain_convert(struct subdrv_ctx *ctx, u32 gain);
static int cjfront_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjfront_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjfront_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjfront_set_multi_shutter_frame_length_in_lut(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void cjfront_set_multi_shutter_frame_length_in_lut_convert(struct subdrv_ctx *ctx,
	u64 *shutters, u16 exp_cnt, u32 frame_length, u32 *frame_length_in_lut);
static void cjfront_write_frame_length_in_lut(struct subdrv_ctx *ctx, u32 fll, u32 *fll_in_lut);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int cjfront_get_otp_qcom_pdaf_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjfront_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjfront_i2c_burst_wr_regs_u16(struct subdrv_ctx *ctx, u16 * list, u32 len);
static int adapter_i2c_burst_wr_regs_u16(struct subdrv_ctx * ctx,
		u16 addr, u16 *list, u32 len);
static void cjfront_lens_pos_writeback(struct subdrv_ctx *ctx);
static int cjfront_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void cjfront_set_max_framerate_in_lut_by_scenario(struct subdrv_ctx *ctx,
...
static struct eeprom_map_info cjfront_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, cjfront_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, cjfront_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, cjfront_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, cjfront_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, cjfront_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, cjfront_get_otp_checksum_data},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, cjfront_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, cjfront_streaming_resume},
	{SENSOR_FEATURE_SET_ESHUTTER, cjfront_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, cjfront_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_GAIN, cjfront_set_gain},
	{SENSOR_FEATURE_SET_HDR_SHUTTER, cjfront_set_hdr_tri_shutter2},
	{SENSOR_FEATURE_SET_HDR_TRI_SHUTTER, cjfront_set_hdr_tri_shutter3},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, cjfront_set_multi_shutter_frame_length_ctrl},
	{SENSOR_FEATURE_GET_OTP_QCOM_PDAF_DATA, cjfront_get_otp_qcom_pdaf_data},
	{SENSOR_FEATURE_SET_AWB_GAIN, cjfront_set_awb_gain},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME_IN_LUT, cjfront_set_multi_shutter_frame_length_in_lut},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, cjfront_set_max_framerate_by_scenario},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	{ /* Mode02_05_OceanChangjiang_JN5_Fdsum_4096x3072_30fps_3056Msps */
		.frame_desc = frame_desc_prev,
		.num_entries = ARRAY_SIZE(frame_desc_prev),
		.mode_setting_table = cjfront_preview_setting,
		.mode_setting_len = ARRAY_SIZE(cjfront_preview_setting),
	...
	{  /* Mode02_05_OceanChangjiang_JN5_Fdsum_4096x3072_30fps_3056Msps */
		...
		.mode_setting_table = cjfront_capture_setting,
		.mode_setting_len = ARRAY_SIZE(cjfront_capture_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{  /* Mode03_09_OceanChangjiang_JN5_Fdsum_4096x2304_30fps_3056Msps */
		.frame_desc = frame_desc_vid,
		.num_entries = ARRAY_SIZE(frame_desc_vid),
		.mode_setting_table = cjfront_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(cjfront_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{ /* Mode04_09_OceanChangjiang_JN5_Fdsum_4096x2304_60fps_3056Msps */
		.frame_desc = frame_desc_hs_vid,
		.num_entries = ARRAY_SIZE(frame_desc_hs_vid),
		.mode_setting_table = cjfront_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(cjfront_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{ /* Mode04_09_OceanChangjiang_JN5_Fdsum_4096x2304_60fps_3056Msps */
		...
		.mode_setting_table = cjfront_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(cjfront_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{ /* Mode07_01_WD4_OceanChangjiang_JN5_Full_8192x6144_15.1fps_3056Msps */
		.frame_desc = frame_desc_cus1,
		.num_entries = ARRAY_SIZE(frame_desc_cus1),
		.mode_setting_table = cjfront_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(cjfront_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{  /* Mode05_21_0_OceanChangjiang_JN5_A2A2_AON_640x480_30fps_1992Msps */
		.frame_desc = frame_desc_cus2,
		.num_entries = ARRAY_SIZE(frame_desc_cus2),
		.mode_setting_table = cjfront_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(cjfront_custom2_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* Mode06_21_0_OceanChangjiang_JN5_A2A2_AON_680x512_30fps_3056Msps */
		.frame_desc = frame_desc_cus3,
		.num_entries = ARRAY_SIZE(frame_desc_cus3),
		.mode_setting_table = cjfront_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(cjfront_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{ /* Mode01_01_WD4_OceanChangjiang_JN5_Full_8192x6144_15.1fps_3056Msps */
		.frame_desc = frame_desc_cus4,
		.num_entries = ARRAY_SIZE(frame_desc_cus4),
		.mode_setting_table = cjfront_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(cjfront_custom4_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{ /* Mode02_05_OceanChangjiang_JN5_Fdsum_4096x3072_30fps_3056Msps no pd for secure cam */
		.frame_desc = frame_desc_cus5,
		.num_entries = ARRAY_SIZE(frame_desc_cus5),
		.mode_setting_table = cjfront_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(cjfront_custom5_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*Mode11_21_0_OceanChangjiang_JN5_A2A2_AON_640x480_10fps_592Msps*/
		.frame_desc = frame_desc_cus6,
		.num_entries = ARRAY_SIZE(frame_desc_cus6),
		.mode_setting_table = cjfront_custom6_setting,
		.mode_setting_len = ARRAY_SIZE(cjfront_custom6_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{ /* Mode02_05_OceanChangjiang_JN5_Fdsum_4096x3072_30fps_3056Msps */
		...
		.mode_setting_table = cjfront_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(cjfront_custom7_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{ /* Mode13_13_1_Changjiang_JN5_A2A2_FHD_30FPS_2048x1152_wAF */
		.frame_desc = frame_desc_cus8,
		.num_entries = ARRAY_SIZE(frame_desc_cus8),
		.mode_setting_table = cjfront_custom8_setting,
		.mode_setting_len = ARRAY_SIZE(cjfront_custom8_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{ /* Mode13_13_1_Changjiang_JN5_A2A2_FHD_30FPS_2048x1152_wAF.nset_2048x1536 */
		.frame_desc = frame_desc_cus9,
		.num_entries = ARRAY_SIZE(frame_desc_cus9),
		.mode_setting_table = cjfront_custom9_setting,
		.mode_setting_len = ARRAY_SIZE(cjfront_custom9_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	...
	.ana_gain_step = 2,
	.ana_gain_table = cjfront_ana_gain_table,
	.ana_gain_table_size = sizeof(cjfront_ana_gain_table),
	.tuning_iso_base = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
	...
	// .init_setting_table = cjfront_sensor_init_setting,
	// .init_setting_len =  ARRAY_SIZE(cjfront_sensor_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry cjfront_mipi_raw_entry = {
	.name = "cjfront_mipi_raw",
	.id = CJFRONT_SENSOR_ID,
	.pw_seq = pw_seq,
...
static void cjfront_set_dummy(struct subdrv_ctx *ctx)
{
	...
	if (ctx->s_ctx.mode[ctx->current_scenario_id].hdr_mode == HDR_RAW_LBMF)
		cjfront_write_frame_length_in_lut(ctx, ctx->frame_length, ctx->frame_length_in_lut);
	else
...
static void cjfront_set_max_framerate(struct subdrv_ctx *ctx, UINT16 framerate,
			kal_bool min_framelength_en)
{
	...
	cjfront_set_dummy(ctx);
}

static void cjfront_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u32 *shutter, u32 frame_length, bool auto_extend_en)
{
	...
	if (ctx->autoflicker_en) {
		...
		if (realtime_fps > 592 && realtime_fps <= 607) {
			cjfront_set_max_framerate(ctx, 592, 0);
		} else if (realtime_fps > 296 && realtime_fps <= 305) {
			cjfront_set_max_framerate(ctx, 296, 0);
		} else if (realtime_fps > 246 && realtime_fps <= 253) {
			cjfront_set_max_framerate(ctx, 246, 0);
		} else if (realtime_fps > 236 && realtime_fps <= 243) {
			cjfront_set_max_framerate(ctx, 236, 0);
		} else if (realtime_fps > 146 && realtime_fps <= 153) {
			cjfront_set_max_framerate(ctx, 146, 0);
		} else {
			/* Extend frame length */
...
static int cjfront_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	cjfront_lens_pos_writeback(ctx);
	cjfront_set_shutter_frame_length_convert(ctx, (u32 *)para, (u32) (*(para + 1)), (u16) (*(para + 2)));
	return 0;
}

static void cjfront_write_shutter(struct subdrv_ctx *ctx)
{
	kal_uint16 realtime_fps = 0;
...
static int cjfront_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	cjfront_set_shutter_convert(ctx, shutter);
	return 0;
}
...
static int cjfront_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (*(u32 *)para)
	cjfront_set_shutter_convert(ctx, *(u32 *)para);
...
static unsigned int read_cjfront_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != cjfront_eeprom_info[meta_id].meta)
		return -1;

	if (size != cjfront_eeprom_info[meta_id].size)
		return -1;

	addr = cjfront_eeprom_info[meta_id].start;
	readsize = cjfront_eeprom_info[meta_id].size;
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
    ...
    adaptor_i2c_rd_u8(ctx->i2c_client, CJFRONT_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
    return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, CJFRONT_EEPROM_WRITE_ID >> 1,
            addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, CJFRONT_EEPROM_WRITE_ID >> 1, reg, 0xA1);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, CJFRONT_EEPROM_WRITE_ID >> 1, reg, 0xA0);
    }
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if ((pStereodata->uSensorId == CJFRONT_SENSOR_ID) && (data_length - 2 == CALI_DATA_SLAVE_LENGTH)
            && (data_base == CJFRONT_STEREO_START_ADDR)) {
        ...
        } else if ((pStereodata->uSensorId == CJFRONT_SENSOR_ID) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            && (data_base == CJFRONT_AESYNC_START_ADDR)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                read_cmos_eeprom_8(ctx, CJFRONT_AESYNC_START_ADDR),
                read_cmos_eeprom_8(ctx, CJFRONT_AESYNC_START_ADDR+1),
                read_cmos_eeprom_8(ctx, CJFRONT_AESYNC_START_ADDR+2),
                read_cmos_eeprom_8(ctx, CJFRONT_AESYNC_START_ADDR+3),
                read_cmos_eeprom_8(ctx, CJFRONT_AESYNC_START_ADDR+4),
                read_cmos_eeprom_8(ctx, CJFRONT_AESYNC_START_ADDR+5),
                read_cmos_eeprom_8(ctx, CJFRONT_AESYNC_START_ADDR+6),
                read_cmos_eeprom_8(ctx, CJFRONT_AESYNC_START_ADDR+7));
        ...
        LOG_INF("cjfront write_Module_data pStereodata is null\n");
        return -1;
    }
    return ret;
}
...
static int cjfront_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	read_cjfront_eeprom_info(ctx, EEPROM_META_STEREO_MW_MAIN_DATA,
			(BYTE *)para, *len);
	return 0;
}
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, CJFRONT_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			*sensor_id = (subdrv_i2c_rd_u8(ctx, addr_h) << 8) |
			...
			if (*sensor_id == 0x38E5 || *sensor_id == 0x38E6) {
				*sensor_id = ctx->s_ctx.sensor_id;
				if (first_read) {
					read_eeprom_common_data(ctx, &oplus_eeprom_info, oplus_eeprom_addr_table);

					u8 af_code[AF_CODE_SIZE] = {0};
					read_cjfront_eeprom_info(ctx, EEPROM_META_AF_CODE, (BYTE *)af_code, AF_CODE_SIZE);
...
static int open(struct subdrv_ctx *ctx)
{
	...
	if (!_adaptor_ctx->aov_mclk_ulposc_flag) {
		sensor_write_id = ctx->i2c_write_id;
		ctx->i2c_write_id = AF_I2C_SLAVE_ADDR;
		subdrv_i2c_wr_u8_u8(ctx, 0x02, 0x01);
		subdrv_i2c_wr_u8_u8(ctx, 0x02, 0x00);
		mdelay(1);
		subdrv_i2c_wr_u8_u8(ctx, 0x02, 0x02);
		subdrv_i2c_wr_u8_u8(ctx, 0x06, 0x40);
		subdrv_i2c_wr_u8_u8(ctx, 0x07, 0x54);
		DRV_LOG_MUST(ctx, "AF INIT DONE");
		mdelay(1);
		ctx->i2c_write_id = sensor_write_id;
	}
...
/*
	if (module_flag == 0x010F || module_flag == 0x011F || (module_flag & 0xFF00) == 0x0200){
		...
		subdrv_i2c_wr_regs_u16(ctx, cjfront_sensor_init_pre_setting1, ARRAY_SIZE(cjfront_sensor_init_pre_setting1));
		mdelay(5);
		subdrv_i2c_wr_regs_u16(ctx, cjfront_sensor_init_pre_setting2, ARRAY_SIZE(cjfront_sensor_init_pre_setting2));
		mdelay(5);
		cjfront_i2c_burst_wr_regs_u16(ctx, cjfront_sensor_simple_init_setting, ARRAY_SIZE(cjfront_sensor_simple_init_setting));
		...
		subdrv_i2c_wr_regs_u16(ctx, cjfront_sensor_init_pre_setting2, ARRAY_SIZE(cjfront_sensor_init_pre_setting2));
		mdelay(5);
		cjfront_i2c_burst_wr_regs_u16(ctx, cjfront_sensor_init_setting, ARRAY_SIZE(cjfront_sensor_init_setting));
		DRV_LOGE(ctx, "setting end\n");
	};
*/
	...
	DRV_LOGE(ctx, "module_flag = 0x%x ", module_flag);

	subdrv_i2c_wr_regs_u16(ctx, cjfront_sensor_init_pre_setting, ARRAY_SIZE(cjfront_sensor_init_pre_setting));
	mdelay(5);
	if (module_flag == 0x7FF) {
		cjfront_i2c_burst_wr_regs_u16(ctx, cjfront_sensor_init_setting_short, ARRAY_SIZE(cjfront_sensor_init_setting_short));
		ctx->s_ctx.init_setting_len = ARRAY_SIZE(cjfront_sensor_init_pre_setting) + ARRAY_SIZE(cjfront_sensor_init_setting_short);
	} else {
		cjfront_i2c_burst_wr_regs_u16(ctx, cjfront_sensor_init_setting, ARRAY_SIZE(cjfront_sensor_init_setting));
		ctx->s_ctx.init_setting_len = ARRAY_SIZE(cjfront_sensor_init_pre_setting) + ARRAY_SIZE(cjfront_sensor_init_setting);
	}
	...
	if ((ctx->power_on_profile_en != NULL) && (*ctx->power_on_profile_en)) {
		...
		ctx->sensor_pw_on_profile.i2c_init_table_len =
			ARRAY_SIZE(cjfront_sensor_init_pre_setting) + ARRAY_SIZE(cjfront_sensor_init_setting);
	}
...
static int cjfront_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (ae_ctrl) {
		...
		case HDR_RAW_STAGGER:
			...
			break;
		case HDR_RAW_LBMF:
			cjfront_set_multi_shutter_frame_length_in_lut_convert(ctx,
				(u64 *)&ae_ctrl->exposure, exp_cnt, 0, frame_length_in_lut);
			set_multi_gain_in_lut(ctx, (u32 *)&ae_ctrl->gain, exp_cnt);
			break;
		case HDR_RAW_DCG_RAW:
			cjfront_set_shutter_convert(ctx, (u32)ae_ctrl->exposure.me_exposure);
			if (ctx->s_ctx.mode[scenario_id].dcg_info.dcg_gain_mode
				== IMGSENSOR_DCG_DIRECT_MODE)
				set_multi_gain(ctx, (u32 *)&ae_ctrl->gain, exp_cnt);
			else
				cjfront_set_gain_convert(ctx, ae_ctrl->gain.me_gain);
			break;
		default:
			cjfront_set_shutter_convert(ctx, (u32)ae_ctrl->exposure.le_exposure);
			cjfront_set_gain_convert(ctx, ae_ctrl->gain.le_gain);
			break;
		}
	}
...
int cjfront_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64* feature_data = (u64*)para;
	u32 gain = *feature_data;

	cjfront_set_gain_convert(ctx, gain);

	return 0;
}

static void cjfront_set_multi_shutter_frame_length(struct subdrv_ctx *ctx,
...
static int cjfront_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64* feature_data = (u64*)para;
	cjfront_lens_pos_writeback(ctx);
	cjfront_set_multi_shutter_frame_length(ctx, (u64 *)(*feature_data),
		(u16) (*(feature_data + 1)), (u16) (*(feature_data + 2)));
	return 0;
}

static void cjfront_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt)
{
	int i = 0;
	u64 values[3] = {0};
	u32 frame_length_in_lut[IMGSENSOR_STAGGER_EXPOSURE_CNT] = {0};
	...
	cjfront_set_multi_shutter_frame_length(ctx, values, exp_cnt, 0);
}
...
static int cjfront_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	cjfront_set_hdr_tri_shutter(ctx, feature_data, 2);
	return 0;
}
...
static int cjfront_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	cjfront_set_hdr_tri_shutter(ctx, feature_data, 3);
	return 0;
}
...
#define cjfront_AF_READ_ID  (0x18)
#define cjfront_AF_POSITON_ADD  (0x03)
...
static bool read_af_pos(struct subdrv_ctx *ctx, u16 *positon)
{
	...
	buf[0] = cjfront_AF_POSITON_ADD;

	msg[0].addr = cjfront_AF_READ_ID >> 1;
	msg[0].flags = i2c_client->flags;
	msg[0].buf = buf;
	msg[0].len = 1;

	msg[1].addr  = cjfront_AF_READ_ID >> 1;
...
static int cjfront_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (ctx->s_ctx.mode[scenario_id].hdr_mode == HDR_RAW_LBMF) {
		cjfront_set_max_framerate_in_lut_by_scenario(ctx, scenario_id, framerate);
		return ERROR_NONE;
	}
