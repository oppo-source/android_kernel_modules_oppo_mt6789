 ...
 *============================================================================
 ****************************************************************************/
#include "cjtelemipiraw_Sensor.h"
// #include "adaptor-subdrv-ctrl.h"
// #include "adaptor-subdrv.h"
// #include "adaptor-ctrls.h"
// #include "cjtele_ana_gain_table.h"
// #include <linux/of.h>
// #include "adaptor-i2c.h"
// #include <linux/regulator/consumer.h>

#define CJTELE_EEPROM_READ_ID	0xA1
#define CJTELE_EEPROM_WRITE_ID   0xA0
#define CJTELE_AF_WRITE_ID	0x18
#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "cjtele_camera_sensor"
...
static int cjtele_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjtele_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjtele_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjtele_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjtele_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjtele_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int cjtele_get_otp_qcom_pdaf_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjtele_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjtele_set_video_mode(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjtele_i2c_burst_wr_regs_u16(struct subdrv_ctx *ctx, u16 * list, u32 len);
static int adapter_i2c_burst_wr_regs_u16(struct subdrv_ctx * ctx,
		u16 addr, u16 *list, u32 len);
static int cjtele_get_otp_qcom_pdaf_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjtele_get_otp_qcom_pdaf_offset_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int cjtele_chk_streaming_st(void *arg);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt, u64 sof_ts);
static int cjtele_get_otp_initsetting_info(struct subdrv_ctx *ctx, u8 *para, u32 *len);
/* STRUCT */

struct mtk_sensor_ctle_param cjtele_static_ctle_param = {
	.cdr_delay = 0xC,
};

static struct eeprom_map_info cjtele_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, cjtele_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, cjtele_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, cjtele_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, cjtele_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, cjtele_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, cjtele_get_otp_checksum_data},
	{SENSOR_FEATURE_GET_OTP_QCOM_PDAF_DATA, cjtele_get_otp_qcom_pdaf_data},
	{SENSOR_FEATURE_GET_OTP_QCOM_PDAF_OFFSET_DATA, cjtele_get_otp_qcom_pdaf_offset_data},
	{SENSOR_FEATURE_SET_AWB_GAIN, cjtele_set_awb_gain},
	{SENSOR_FEATURE_SET_VIDEO_MODE, cjtele_set_video_mode},
	{SENSOR_FEATURE_GET_OTP_INITSETTING_INFO, cjtele_get_otp_initsetting_info},
};
...
static u32 cjtele_dcg_ratio_table_10bit[] = {8000};
// static u32 cjtele_dcg_ratio_table_12bit[] = {4000};
...
static struct subdrv_mode_struct mode_struct[] = {
	{/*00_4096x3072_30fps_2976Msps*/
		...
		.mode_setting_table = cjtele_preview_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_preview_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = cjtele_preview_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_preview_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{/*00_4096x3072_30fps_2976Msps*/
		...
		.mode_setting_table = cjtele_capture_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_capture_setting),
	...
	{/*07_4096x2560_30fps_2976Msps*/
		...
		.mode_setting_table = cjtele_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_normal_video_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = cjtele_normal_video_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_normal_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{/* 011_4096x2304_60fps_2976Msps */
		...
		.mode_setting_table = cjtele_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_hs_video_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = cjtele_hs_video_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_hs_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{/* 012_4096x2304_120fps_2976Msps */
		...
		.mode_setting_table = cjtele_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* 01_4096x3072_30fps_2976Msps AEB*/
		...
		.mode_setting_table = cjtele_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom1_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = cjtele_custom1_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_custom1_setting),
	...
	{/*06_4096*3072 30fps 2X2 AEB(AP merge)-izoom_QBC(AP RMSC)*/
		...
		.mode_setting_table = cjtele_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom2_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = cjtele_custom2_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_custom2_setting),
		.hdr_mode = HDR_RAW_LBMF,
		.raw_cnt = 2,
		.exp_cnt = 2,
		.pclk = 1920000000,
	...
	{  /*02_4096*3072 30FPS iDCG(AP Merge) */
		...
		.mode_setting_table = cjtele_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom3_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = cjtele_custom3_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_custom3_setting),
		...
		.dcg_info = {
			...
			.dcg_gain_table = cjtele_dcg_ratio_table_10bit,
			.dcg_gain_table_size = sizeof(cjtele_dcg_ratio_table_10bit),
		},
		.multi_exposure_shutter_range[IMGSENSOR_EXPOSURE_LE].min = 4,
		.multi_exposure_shutter_range[IMGSENSOR_EXPOSURE_ME].min = 4,
	...
	{ /* 03_8192*6144 2x2 Fullsize QBC (For 25M) */
		...
		.mode_setting_table = cjtele_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom4_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = cjtele_custom4_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_custom4_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{ /* 04_4096*3072 30fps Fullsize-crop izoom-QBC(AP RMSC)*/
		...
		.mode_setting_table = cjtele_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom5_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = cjtele_custom5_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_custom5_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{ /* 05_16384x12288_7.5fps_2976Msps */
		...
		.mode_setting_table = cjtele_custom6_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom6_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = cjtele_custom6_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_custom6_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{  /*08_4096x2560_30.01fps_2976Msps_iDCG*/
		...
		.mode_setting_table = cjtele_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom7_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = cjtele_custom7_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_custom7_setting),
		.hdr_mode = HDR_RAW_DCG_RAW,
		.raw_cnt = 2,
		.exp_cnt = 2,
		.pclk = 1920000000,
	...
	{ /* 09_4096*2560 2X2 izoom-bayer */
		...
		.mode_setting_table = cjtele_custom8_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom8_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = cjtele_custom8_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_custom8_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{/*010_4096*2560 Fullsize-crop izoom-QBC(AP RMSC)*/
		...
		.mode_setting_table = cjtele_custom9_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom9_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = cjtele_custom9_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_custom9_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{/*014_2048x1152_240.01fps_1760Msps*/
		...
		.mode_setting_table = cjtele_custom10_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom10_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* 20_4096x2304_30fps_2976Msps */
		...
		.mode_setting_table = cjtele_custom11_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom11_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* 4096x3072_60fps_2976Msps */
		...
		.mode_setting_table = cjtele_custom12_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom12_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{  /*15_4096x3072_30fps_2976Msps FCM*/
		...
		.mode_setting_table = cjtele_custom13_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom13_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = cjtele_custom13_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_custom13_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{  /*17_4096x2560_30.01fps_2976Msps 8Sum2Avg_iDCG1:4_HCGPD*/
		...
		.mode_setting_table = cjtele_custom14_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom14_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_RAW_DCG_RAW,
	...
	{  /*23_4912x3688_25fps_2976Msps 18M full crop*/
		...
		.mode_setting_table = cjtele_custom15_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom15_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = cjtele_custom15_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_custom15_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
	...
	{  /*19_4096x2560_30.01fps_2976Msps 8Sum2Avg_iDCG1:8_HCGPD*/
		...
		.mode_setting_table = cjtele_custom16_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom16_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = cjtele_custom16_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_custom16_setting),
		.hdr_mode = HDR_RAW_DCG_RAW,
		.raw_cnt = 2,
		.exp_cnt = 2,
		.pclk = 1920000000,
	...
	{/*21_3840x2160_120fps_2976Msps*/
		...
		.mode_setting_table = cjtele_custom17_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom17_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{ /* 22_4096x2304_60fps_2976Msps */
		...
		.mode_setting_table = cjtele_custom18_setting,
		.mode_setting_len = ARRAY_SIZE(cjtele_custom18_setting),
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = cjtele_custom18_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(cjtele_custom18_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 1920000000,
...
#ifdef CJTELE_ISF_DBG
struct subdrv_static_ctx cjtele_legacy_s_ctx = {
#else
static struct subdrv_static_ctx static_ctx = {
#endif
	.sensor_id = CJTELE_SENSOR_ID,
	...
	.ana_gain_step = 1,
	.ana_gain_table = cjtele_ana_gain_table,
	.ana_gain_table_size = sizeof(cjtele_ana_gain_table),
	.tuning_iso_base = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
	...
	// .init_setting_table = cjtele_init_setting,
	// .init_setting_len = ARRAY_SIZE(cjtele_init_setting),
	.mode = mode_struct,
	...
	.chk_s_off_sta = 1,
	.chk_s_off_end = 0,
	.chk_streaming_st = cjtele_chk_streaming_st,

	.checksum_value = 0xc88841a,
	.ctle_param = &cjtele_static_ctle_param,
};
...
#ifdef CJTELE_ISF_DBG
const struct subdrv_entry cjtele_mipi_raw_entry_legacy = {
#else
const struct subdrv_entry cjtele_mipi_raw_entry = {
#endif
	.name = "cjtele_mipi_raw",
	.id = CJTELE_SENSOR_ID, /* 0x1b73 */
	.pw_seq = pw_seq,
...
static int get_sensor_temperature(void *arg)
{
	...
	u8 temperature = (subdrv_ixc_rd_u16(ctx, ctx->s_ctx.reg_addr_temp_read) >> 8) & 0xFF;
...
static unsigned int read_cjtele_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != cjtele_eeprom_info[meta_id].meta)
		return -1;

	if (size != cjtele_eeprom_info[meta_id].size)
		return -1;

	addr = cjtele_eeprom_info[meta_id].start;
	readsize = cjtele_eeprom_info[meta_id].size;
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
    ...
    adaptor_i2c_rd_u8(ctx->i2c_client, CJTELE_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
    return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, CJTELE_EEPROM_WRITE_ID >> 1,
            addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, CJTELE_EEPROM_WRITE_ID >> 1, reg, 0x0E);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, CJTELE_EEPROM_WRITE_ID >> 1, reg, 0x00);
    }
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    UINT32 i = 0;
    kal_uint16 offset = 0;
    if (pStereodata->uSensorId != CJTELE_SENSOR_ID) {
        LOG_INF("Invalid Sensor id:0x%x write eeprom, Real Sensor id:0x%x\n", 
            pStereodata->uSensorId, CJTELE_SENSOR_ID);
    }
    if (pStereodata != NULL) {
        ...
        if (((data_length - 2 == CALI_DATA_SLAVE_TELE_LENGTH) && (data_base == CJTELE_STEREO_START_ADDR))
            || ((data_length - 2 == CALI_DATA_SLAVE_TELE_LENGTH) && (data_base == CJTELE_STEREO_105CM_START_ADDR))) {
        ...
        } else if ((data_length < AESYNC_DATA_LENGTH_TOTAL) && (data_base == CJTELE_AESYNC_START_ADDR)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                read_cmos_eeprom_8(ctx, CJTELE_AESYNC_START_ADDR),
                read_cmos_eeprom_8(ctx, CJTELE_AESYNC_START_ADDR+1),
                read_cmos_eeprom_8(ctx, CJTELE_AESYNC_START_ADDR+2),
                read_cmos_eeprom_8(ctx, CJTELE_AESYNC_START_ADDR+3),
                read_cmos_eeprom_8(ctx, CJTELE_AESYNC_START_ADDR+4),
                read_cmos_eeprom_8(ctx, CJTELE_AESYNC_START_ADDR+5),
                read_cmos_eeprom_8(ctx, CJTELE_AESYNC_START_ADDR+6),
                read_cmos_eeprom_8(ctx, CJTELE_AESYNC_START_ADDR+7));
        ...
        LOG_INF("cjtele write_Module_data pStereodata is null\n");
        return -1;
    }
    return ret;
}
...
static int cjtele_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	case EEPROM_STEREODATA_MT_MAIN:
		read_cjtele_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA,
				(BYTE *)feature_return_para_32, *len);
		break;
	case EEPROM_STEREODATA_MT_MAIN_105CM:
		read_cjtele_eeprom_info(ctx, EEPROM_META_STEREO_MT_MAIN_DATA_105CM,
				(BYTE *)feature_return_para_32, *len);
		break;
	default:
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, CJTELE_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
static void cjtele_write_init_setting(struct subdrv_ctx *ctx)
{
	u32 module_info = 0;
	...
	subdrv_i2c_wr_regs_u16(ctx, cjtele_init_setting_part1, ARRAY_SIZE(cjtele_init_setting_part1));
	mdelay(1);
	subdrv_i2c_wr_regs_u16(ctx, cjtele_init_setting_part2, ARRAY_SIZE(cjtele_init_setting_part2));

	if ((module_info | 0x000F) != 0x024F && (module_info | 0x000F) != 0x02FF) {
		mdelay(20);
		cjtele_i2c_burst_wr_regs_u16(ctx, cjtele_init_setting_part3_long,
										ARRAY_SIZE(cjtele_init_setting_part3_long));
		ctx->s_ctx.init_setting_len = ARRAY_SIZE(cjtele_init_setting_part1)
										+ ARRAY_SIZE(cjtele_init_setting_part2)
										+ ARRAY_SIZE(cjtele_init_setting_part3_long);
	} else {
		mdelay(1);
		cjtele_i2c_burst_wr_regs_u16(ctx, cjtele_init_setting_part3_short,
										ARRAY_SIZE(cjtele_init_setting_part3_short));
		ctx->s_ctx.init_setting_len = ARRAY_SIZE(cjtele_init_setting_part1)
										+ ARRAY_SIZE(cjtele_init_setting_part2)
										+ ARRAY_SIZE(cjtele_init_setting_part3_short);
	}
...
static int open(struct subdrv_ctx *ctx)
{
	...
	adaptor_i2c_wr_u8_u8(ctx->i2c_client, CJTELE_AF_WRITE_ID >> 1, 0x02, 0x0);
	// wait for af active
	mdelay(3);

	cjtele_write_init_setting(ctx);
...
static void set_group_hold(void *arg, u8 en)
{
	...
	DRV_LOG(ctx, "CJTELE gph en: %d  gph_enable: %d", en, gph_enable);
	if (en && !gph_enable) {
