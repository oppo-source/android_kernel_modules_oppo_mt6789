 ...
 *============================================================================
 ****************************************************************************/
#include "hhuwidemipiraw_Sensor.h"

#define HHUWIDE_EEPROM_READ_ID	0xA1
#define HHUWIDE_EEPROM_WRITE_ID   0xA0
#define HHUWIDE_MAX_OFFSET		0x4000
#define OPLUS_CAMERA_COMMON_DATA_LENGTH 40
#define PFX "hhuwide_camera_sensor"
...
static const char* ob_version = NULL;
static int group_hold_frame_count = 0;
static void set_group_hold(void *arg, u8 en);
static void get_ob_version(struct subdrv_ctx *ctx);
static u16 get_gain2reg(u32 gain);
static int hhuwide_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhuwide_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhuwide_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhuwide_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhuwide_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhuwide_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhuwide_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhuwide_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhuwide_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void hhuwide_set_shutter_convert(struct subdrv_ctx *ctx, u32 shutter);
static int hhuwide_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static int hhuwide_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhuwide_set_gain_convert(struct subdrv_ctx *ctx, u32 gain);
static int hhuwide_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhuwide_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhuwide_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhuwide_set_multi_shutter_frame_length_in_lut(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void hhuwide_set_multi_shutter_frame_length_in_lut_convert(struct subdrv_ctx *ctx,
	u64 *shutters, u16 exp_cnt, u32 frame_length, u32 *frame_length_in_lut);
static void hhuwide_write_frame_length_in_lut(struct subdrv_ctx *ctx, u32 fll, u32 *fll_in_lut);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int hhuwide_get_otp_qcom_pdaf_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhuwide_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhuwide_i2c_burst_wr_regs_u16(struct subdrv_ctx *ctx, u16 * list, u32 len);
static int adapter_i2c_burst_wr_regs_u16(struct subdrv_ctx * ctx,
		u16 addr, u16 *list, u32 len);
static void hhuwide_lens_pos_writeback(struct subdrv_ctx *ctx);
static int hhuwide_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void hhuwide_set_max_framerate_in_lut_by_scenario(struct subdrv_ctx *ctx,
...
static int hhuwide_set_frame_length_in_lut(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void hhuwide_set_frame_length_in_lut_convert(struct subdrv_ctx *ctx,
...
static struct eeprom_map_info hhuwide_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, hhuwide_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, hhuwide_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, hhuwide_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, hhuwide_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, hhuwide_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, hhuwide_get_otp_checksum_data},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, hhuwide_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, hhuwide_streaming_resume},
	{SENSOR_FEATURE_SET_ESHUTTER, hhuwide_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, hhuwide_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_GAIN, hhuwide_set_gain},
	{SENSOR_FEATURE_SET_HDR_SHUTTER, hhuwide_set_hdr_tri_shutter2},
	{SENSOR_FEATURE_SET_HDR_TRI_SHUTTER, hhuwide_set_hdr_tri_shutter3},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, hhuwide_set_multi_shutter_frame_length_ctrl},
	{SENSOR_FEATURE_GET_OTP_QCOM_PDAF_DATA, hhuwide_get_otp_qcom_pdaf_data},
	{SENSOR_FEATURE_SET_AWB_GAIN, hhuwide_set_awb_gain},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME_IN_LUT, hhuwide_set_multi_shutter_frame_length_in_lut},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, hhuwide_set_max_framerate_by_scenario},
	{SENSOR_FEATURE_SET_FRAMELENGTH_IN_LUT, hhuwide_set_frame_length_in_lut},
};
...
#define pd_i4Crop { \
		...
		{0, 0}, {0, 0}, {0, 0}, {208, 156}, {560, 420}, \
		/* <cust6> <cust7> <cust8> <cust9> <cust10>*/ \
		{560, 420}, {0, 384}, {0, 0}, {0, 0}, {2048, 1536}, \
		/* <cust11> <cust12>*/ \
		{2048, 1536}, {0, 0}, {0, 256}, {0, 256},\
}
...
static u32 hhuwide_dcg_ratio_table_10bit[] = {4000};
/*static u32 hhuwide_dcg_ratio_table_12bit[] = {4000};*/
...
/* not use, FCM 2 07_OceanChangjiang_UW_JN5_Fdsum_DSG_split*/
...
static struct subdrv_mode_struct mode_struct[] = {
	{ /*03_OceanDX4_05_JN5_Fdsum_12.5Mp_4096x3072_30fps_3056Msps*/
		...
		.seamless_switch_mode_setting_table = hhuwide_seamless_preview,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhuwide_seamless_preview),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 920000000,
	...
	{  /*05_OceanDX4_09_JN5_Fdsum_4K_4096x2304_30fps_3056Msps*/
		...
		.seamless_switch_mode_setting_table = hhuwide_seamless_vid,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhuwide_seamless_vid),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 920000000,
	...
	{ /* not use, 08_OceanDX4_09_JN5_Fdsum_4K_4096x2304_60fps_3056Msps */
	...
	{ /* 04_OceanDX4_05_JN5_Fdsum_12.5Mp_AEB_4096x3072_60fps_3056Msps */
		...
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhuwide_seamless_custom1,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhuwide_seamless_custom1),
	...
	{  /* 8192x6144 Fullsize QBC mode 7*/
		...
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhuwide_seamless_custom3,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhuwide_seamless_custom3),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 920000000,
	...
	{ /* 09_OceanChangjiang_UW_JN5_Fdsum RST 17.5ms */
	...
	{ /* 09_OceanChangjiang_UW_JN5_Fdsum RST 13ms*/
	...
	{  /* 07_OceanDX4_10_0_JN5_Fdsum_DSG_split_4096x2304_30fps_3056Msps AP merge*/
		...
		.seamless_switch_group = 2,
		.seamless_switch_mode_setting_table = hhuwide_seamless_custom7,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhuwide_seamless_custom7),
		...
		.dcg_info = {
			...
			.dcg_gain_table = hhuwide_dcg_ratio_table_10bit,
			.dcg_gain_table_size = sizeof(hhuwide_dcg_ratio_table_10bit),
		},
	...
	{  /* 07_OceanChangjiang_UW_JN5_Fdsum_DSG_split_4096x3072_30fps_3056Mbps */
		...
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhuwide_seamless_custom12,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhuwide_seamless_custom12),
		.hdr_mode = HDR_RAW_DCG_RAW,
		.raw_cnt = 2,
		.exp_cnt = 2,
		.pclk = 920000000,
	...
	{  /*09_OceanChangjiang_UW_JN5_Fdsum_4096x2560_30fps_3056Mbps*/
		...
		.seamless_switch_mode_setting_table = hhuwide_seamless_custom13,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhuwide_seamless_custom13),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 920000000,
	...
	{  /* 07_OceanChangjiang_UW_JN5_Fdsum_DSG_split_4096x2560_30fps_3056Mbps */
		...
		.seamless_switch_group = 3,
		.seamless_switch_mode_setting_table = hhuwide_seamless_custom14,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhuwide_seamless_custom14),
		.hdr_mode = HDR_RAW_DCG_RAW,
		.raw_cnt = 2,
		.exp_cnt = 2,
		.pclk = 920000000,
...
static struct subdrv_static_ctx static_ctx = {
	...
	.ana_gain_step = 2,
	.ana_gain_table = hhuwide_ana_gain_table,
	.ana_gain_table_size = sizeof(hhuwide_ana_gain_table),
	.tuning_iso_base = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
	...
	// .init_setting_table = hhuwide_sensor_init_setting,
	// .init_setting_len =  ARRAY_SIZE(hhuwide_sensor_init_setting),
	.mode = mode_struct,
...
static struct subdrv_pw_seq_entry pw_seq[] = {
	...
	{HW_ID_DVDD1, {1000000, 1000000}, 0},
	{HW_ID_AFVDD, {2800000, 2800000}, 0},
	{HW_ID_RST, {1}, 2000},
	{HW_ID_MCLK_DRIVING_CURRENT, {4}, 10000},
};

const struct subdrv_entry hhuwide_mipi_raw_entry = {
	.name = "hhuwide_mipi_raw",
	.id = HHUWIDE_SENSOR_ID,
	.pw_seq = pw_seq,
...
static void hhuwide_set_dummy(struct subdrv_ctx *ctx)
{
	...
	if (ctx->s_ctx.mode[ctx->current_scenario_id].hdr_mode == HDR_RAW_LBMF)
		hhuwide_write_frame_length_in_lut(ctx, ctx->frame_length, ctx->frame_length_in_lut);
	else
...
static void hhuwide_set_max_framerate(struct subdrv_ctx *ctx, UINT16 framerate,
			kal_bool min_framelength_en)
{
	...
	hhuwide_set_dummy(ctx);
}

static void hhuwide_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u32 *shutter, u32 frame_length, bool auto_extend_en)
{
	...
	if (ctx->autoflicker_en) {
		...
		if (realtime_fps > 592 && realtime_fps <= 607) {
			hhuwide_set_max_framerate(ctx, 592, 0);
		} else if (realtime_fps > 296 && realtime_fps <= 305) {
			hhuwide_set_max_framerate(ctx, 296, 0);
		} else if (realtime_fps > 246 && realtime_fps <= 253) {
			hhuwide_set_max_framerate(ctx, 246, 0);
		} else if (realtime_fps > 236 && realtime_fps <= 243) {
			hhuwide_set_max_framerate(ctx, 236, 0);
		} else if (realtime_fps > 146 && realtime_fps <= 153) {
			hhuwide_set_max_framerate(ctx, 146, 0);
		} else {
			/* Extend frame length */
...
static int hhuwide_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	hhuwide_lens_pos_writeback(ctx);
	hhuwide_set_shutter_frame_length_convert(ctx, (u32 *)para, (u32) (*(para + 1)), (u16) (*(para + 2)));
	return 0;
}

static void hhuwide_write_shutter(struct subdrv_ctx *ctx)
{
	kal_uint16 realtime_fps = 0;
...
static int hhuwide_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	hhuwide_set_shutter_convert(ctx, shutter);
	return 0;
}
...
static int hhuwide_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (*(u32 *)para)
	hhuwide_set_shutter_convert(ctx, *(u32 *)para);
...
static unsigned int read_hhuwide_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != hhuwide_eeprom_info[meta_id].meta)
		return -1;

	if (size != hhuwide_eeprom_info[meta_id].size)
		return -1;

	addr = hhuwide_eeprom_info[meta_id].start;
	readsize = hhuwide_eeprom_info[meta_id].size;
...
static int hhuwide_set_frame_length_in_lut(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	hhuwide_set_frame_length_in_lut_convert(ctx, (u32) (*feature_data),
		(u32 *) (*(feature_data + 1)));
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
    ...
    adaptor_i2c_rd_u8(ctx->i2c_client, HHUWIDE_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
    return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, HHUWIDE_EEPROM_WRITE_ID >> 1,
            addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, HHUWIDE_EEPROM_WRITE_ID >> 1, reg, 0xA1);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, HHUWIDE_EEPROM_WRITE_ID >> 1, reg, 0xA0);
    }
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if ((pStereodata->uSensorId == HHUWIDE_SENSOR_ID) && (data_length - 2 == CALI_DATA_SLAVE_LENGTH)
            && (data_base == HHUWIDE_STEREO_START_ADDR)) {
        ...
        } else if ((pStereodata->uSensorId == HHUWIDE_SENSOR_ID) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            && (data_base == HHUWIDE_AESYNC_START_ADDR)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                read_cmos_eeprom_8(ctx, HHUWIDE_AESYNC_START_ADDR),
                read_cmos_eeprom_8(ctx, HHUWIDE_AESYNC_START_ADDR+1),
                read_cmos_eeprom_8(ctx, HHUWIDE_AESYNC_START_ADDR+2),
                read_cmos_eeprom_8(ctx, HHUWIDE_AESYNC_START_ADDR+3),
                read_cmos_eeprom_8(ctx, HHUWIDE_AESYNC_START_ADDR+4),
                read_cmos_eeprom_8(ctx, HHUWIDE_AESYNC_START_ADDR+5),
                read_cmos_eeprom_8(ctx, HHUWIDE_AESYNC_START_ADDR+6),
                read_cmos_eeprom_8(ctx, HHUWIDE_AESYNC_START_ADDR+7));
        ...
        LOG_INF("hhuwide write_Module_data pStereodata is null\n");
        return -1;
    }
    return ret;
}
...
static int hhuwide_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	read_hhuwide_eeprom_info(ctx, EEPROM_META_STEREO_MW_MAIN_DATA,
			(BYTE *)para, *len);
	return 0;
}
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, HHUWIDE_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
			...
			if (*sensor_id == 0x38E5) {
				*sensor_id = ctx->s_ctx.sensor_id;
				if (first_read) {
					...
					read_hhuwide_eeprom_info(ctx, EEPROM_META_AF_CODE, (BYTE *)af_code, AF_CODE_SIZE);
...
static int open(struct subdrv_ctx *ctx)
{
	...
	u32 scenario_id = 0;
	u16 module_info = 0;
	u64 init_setting_time = 0;
	u16* init_setting_part1 = NULL;
	u16* init_setting_part2 = NULL;
	u32 init_setting_part1_size = 0;
	u32 init_setting_part2_size = 0;
	...
	DRV_LOG(ctx, "module_info (0x%x) write init setting +", module_info);
	if ((module_info | 0x00FF) == 0x07FF) {
		if (ob_version && strcmp("cj", ob_version) == 0) {
			init_setting_part1 = cj_hhuwide_sensor_init_pre_setting_short;
			init_setting_part2 = cj_hhuwide_sensor_init_setting_short;
			init_setting_part1_size = ARRAY_SIZE(cj_hhuwide_sensor_init_pre_setting_short);
			init_setting_part2_size = ARRAY_SIZE(cj_hhuwide_sensor_init_setting_short);
		} else {
			init_setting_part1 = hhuwide_sensor_init_pre_setting_short;
			init_setting_part2 = hhuwide_sensor_init_setting_short;
			init_setting_part1_size = ARRAY_SIZE(hhuwide_sensor_init_pre_setting_short);
			init_setting_part2_size = ARRAY_SIZE(hhuwide_sensor_init_setting_short);
		}
	} else {
		if (ob_version && strcmp("cj", ob_version) == 0) {
			init_setting_part1 = cj_hhuwide_sensor_init_pre_setting_long;
			init_setting_part2 = cj_hhuwide_sensor_init_setting_long;
			init_setting_part1_size = ARRAY_SIZE(cj_hhuwide_sensor_init_pre_setting_long);
			init_setting_part2_size = ARRAY_SIZE(cj_hhuwide_sensor_init_setting_long);
		} else {
			init_setting_part1 = hhuwide_sensor_init_pre_setting_long;
			init_setting_part2 = hhuwide_sensor_init_setting_long;
			init_setting_part1_size = ARRAY_SIZE(hhuwide_sensor_init_pre_setting_long);
			init_setting_part2_size = ARRAY_SIZE(hhuwide_sensor_init_setting_long);
		}
	}

	init_setting_time = ktime_get_boottime_ns();

	subdrv_i2c_wr_regs_u16(ctx, init_setting_part1, init_setting_part1_size);
	mdelay(5);
	hhuwide_i2c_burst_wr_regs_u16(ctx, init_setting_part2, init_setting_part2_size);
	DRV_LOG(ctx, "write init setting done");
	ctx->s_ctx.init_setting_len = init_setting_part1_size + init_setting_part2_size;

	init_setting_time = ktime_get_boottime_ns() - init_setting_time;

	if ((ctx->power_on_profile_en != NULL) && (*ctx->power_on_profile_en)) {
		ctx->sensor_pw_on_profile.i2c_init_period = init_setting_time;
		ctx->sensor_pw_on_profile.i2c_init_table_len = ctx->s_ctx.init_setting_len;
	}
	DRV_LOG_MUST(ctx, "write init setting X: size:%u, time(us):%lld\n", ctx->s_ctx.init_setting_len, init_setting_time / 1000);
	...
	if (ctx->s_ctx.mode[ctx->current_scenario_id].hdr_mode == HDR_RAW_LBMF) {
		...
		switch (ctx->s_ctx.mode[ctx->current_scenario_id].exp_cnt) {
		case 2:
			ctx->frame_length_in_lut[0] = ctx->frame_length / 2;
...
static int hhuwide_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (ae_ctrl) {
		...
		case HDR_RAW_STAGGER:
			...
			break;
		case HDR_RAW_LBMF:
			hhuwide_set_multi_shutter_frame_length_in_lut_convert(ctx,
				(u64 *)&ae_ctrl->exposure, exp_cnt, 0, frame_length_in_lut);
			set_multi_gain_in_lut(ctx, (u32 *)&ae_ctrl->gain, exp_cnt);
			break;
		case HDR_RAW_DCG_RAW:
			hhuwide_set_shutter_convert(ctx, (u32)ae_ctrl->exposure.me_exposure);
			if (ctx->s_ctx.mode[scenario_id].dcg_info.dcg_gain_mode
				== IMGSENSOR_DCG_DIRECT_MODE)
				set_multi_gain(ctx, (u32 *)&ae_ctrl->gain, exp_cnt);
			else
				hhuwide_set_gain_convert(ctx, ae_ctrl->gain.me_gain);
			break;
		default:
			hhuwide_set_shutter_convert(ctx, (u32)ae_ctrl->exposure.le_exposure);
			hhuwide_set_gain_convert(ctx, ae_ctrl->gain.le_gain);
			break;
		}
	}
	commit_i2c_buffer(ctx);
...
static int hhuwide_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (mode) {
		if (mode == 5) {
			...
			subdrv_i2c_wr_u16(ctx, 0x0620, 0x0001); /*DSG_LCG_black*/
		} else {
			subdrv_i2c_wr_u16(ctx, 0x0600, mode); /*100% Color bar*/
			subdrv_i2c_wr_u16(ctx, 0x0620, mode); /*DSG_LCG 100% Color bar*/
		}
	}
...
int hhuwide_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64* feature_data = (u64*)para;
	u32 gain = *feature_data;

	hhuwide_set_gain_convert(ctx, gain);

	return 0;
}

static void hhuwide_set_multi_shutter_frame_length(struct subdrv_ctx *ctx,
...
static int hhuwide_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64* feature_data = (u64*)para;
	hhuwide_lens_pos_writeback(ctx);
	hhuwide_set_multi_shutter_frame_length(ctx, (u64 *)(*feature_data),
		(u16) (*(feature_data + 1)), (u16) (*(feature_data + 2)));
	return 0;
}

static void hhuwide_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u64 *shutters, u16 exp_cnt)
{
	int i = 0;
	u64 values[3] = {0};
	u32 frame_length_in_lut[IMGSENSOR_STAGGER_EXPOSURE_CNT] = {0};
	...
	hhuwide_set_multi_shutter_frame_length(ctx, values, exp_cnt, 0);
}
...
static int hhuwide_set_hdr_tri_shutter2(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	hhuwide_set_hdr_tri_shutter(ctx, feature_data, 2);
	return 0;
}
...
static int hhuwide_set_hdr_tri_shutter3(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	hhuwide_set_hdr_tri_shutter(ctx, feature_data, 3);
	return 0;
}
...
#define hhuwide_AF_READ_ID  (0x18)
#define hhuwide_AF_POSITON_ADD  (0x03)
...
static bool read_af_pos(struct subdrv_ctx *ctx, u16 *positon)
{
	...
	buf[0] = hhuwide_AF_POSITON_ADD;

	msg[0].addr = hhuwide_AF_READ_ID >> 1;
	msg[0].flags = i2c_client->flags;
	msg[0].buf = buf;
	msg[0].len = 1;

	msg[1].addr  = hhuwide_AF_READ_ID >> 1;
...
static int hhuwide_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (ctx->s_ctx.mode[scenario_id].hdr_mode == HDR_RAW_LBMF) {
		hhuwide_set_max_framerate_in_lut_by_scenario(ctx, scenario_id, framerate);
		return ERROR_NONE;
	}
...
static void get_ob_version(struct subdrv_ctx *ctx) {
	...
	ret = of_property_read_string(dev->of_node, "ob-version", &ob_version);
	if (ret || ob_version == NULL) {
		DRV_LOGE(ctx, "ob_version of_property_read_string(ob-version) error %d", ret);
		return;
	}
	DRV_LOG_MUST(ctx, "get ob_version %s\n", ob_version);
	struct mode_setting_list_struct* mode_list = zj_mode;
	if (strcmp("cj", ob_version) == 0) {
		mode_list = cj_mode;
	}
	for (int mode_idx = 0; mode_idx < ctx->s_ctx.sensor_mode_num; ++mode_idx) {
		ctx->s_ctx.mode[mode_idx].mode_setting_table = mode_list[mode_idx].mode_setting_table;
		ctx->s_ctx.mode[mode_idx].mode_setting_len = mode_list[mode_idx].mode_setting_len;
	}
}
