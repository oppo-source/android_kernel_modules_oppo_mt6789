...
static void set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u64 *shutters,
                     kal_uint16 shutter_cnt, kal_uint16 frame_length)
{
	LOG_INF("Before shutters[0] =%llu, framelength =%d/%d\n",
	...
	LOG_INF("Exit shutters[0] =%llu, framelength =%d/%d\n",
        shutters[0], ctx->frame_length, frame_length);
...
static kal_uint32 seamless_switch(struct subdrv_ctx *ctx,
        enum SENSOR_SCENARIO_ID_ENUM scenario_id, struct mtk_hdr_ae *ae_ctrl)
{
    ...
    case SENSOR_SCENARIO_ID_CUSTOM6:
    {
        ...
        if (ae_ctrl) {
            LOG_INF("call SENSOR_SCENARIO_ID_CUSTOM6 %llu %u",
                    ae_ctrl->exposure.le_exposure, ae_ctrl->gain.le_gain);
            set_shutter(ctx, ae_ctrl->exposure.le_exposure, KAL_FALSE);;
            set_gain(ctx, ae_ctrl->gain.le_gain, KAL_FALSE);
        }
        FMC_GPH_END;
    }
        break;
    case SENSOR_SCENARIO_ID_CUSTOM7:
    {
        ...
        if (ae_ctrl) {
            LOG_INF("call SENSOR_SCENARIO_ID_CUSTOM7 %llu %llu %u %u",
	...
	case SENSOR_SCENARIO_ID_CUSTOM8:
	{
        ...
        if (ae_ctrl) {
            LOG_INF("call SENSOR_SCENARIO_ID_CUSTOM8 %llu %u",
                    ae_ctrl->exposure.le_exposure, ae_ctrl->gain.le_gain);
            set_shutter(ctx, ae_ctrl->exposure.le_exposure, KAL_FALSE);;
            set_gain(ctx, ae_ctrl->gain.le_gain, KAL_FALSE);
        }
	...
	case SENSOR_SCENARIO_ID_CUSTOM4:
	{
            ...
            if (ae_ctrl) {
                LOG_INF("call SENSOR_SCENARIO_ID_CUSTOM4 %llu %llu %u %u",
                    ae_ctrl->exposure.le_exposure,
                    ae_ctrl->exposure.me_exposure,
                    ae_ctrl->gain.le_gain,
                    ae_ctrl->gain.me_gain);
	...
	case SENSOR_SCENARIO_ID_NORMAL_PREVIEW:
	{
        ...
        if (ae_ctrl) {
            LOG_INF("call SENSOR_SCENARIO_ID_NORMAL_PREVIEW %llu %u",
                    ae_ctrl->exposure.le_exposure, ae_ctrl->gain.le_gain);
            set_shutter(ctx, ae_ctrl->exposure.le_exposure, KAL_FALSE);;
            set_gain(ctx, ae_ctrl->gain.le_gain, KAL_FALSE);
        }
	...
	case SENSOR_SCENARIO_ID_CUSTOM13:
	{
        ...
        if (ae_ctrl) {
            LOG_INF("call SENSOR_SCENARIO_ID_CUSTOM13 %llu %u",
                    ae_ctrl->exposure.le_exposure, ae_ctrl->gain.le_gain);
            set_shutter(ctx, ae_ctrl->exposure.le_exposure, KAL_FALSE);;
            set_gain(ctx, ae_ctrl->gain.le_gain, KAL_FALSE);
        }
	...
	case SENSOR_SCENARIO_ID_CUSTOM12:
	{
        ...
        if (ae_ctrl) {
            LOG_INF("call SENSOR_SCENARIO_ID_CUSTOM12 %llu %u",
                    ae_ctrl->exposure.le_exposure, ae_ctrl->gain.le_gain);
            set_shutter(ctx, ae_ctrl->exposure.le_exposure, KAL_FALSE);;
            set_gain(ctx, ae_ctrl->gain.le_gain, KAL_FALSE);
        }
...
//    int ret = ERROR_NONE;
    struct mtk_hdr_ae *pAeCtrls = NULL;
    uint32_t *pScenarios;
...
#define EEPROM_8k 0x8000
    ...
    case SENSOR_FEATURE_SET_SEAMLESS_EXTEND_FRAME_LENGTH:
        pr_info("extend_frame_len %llu\n", *feature_data);
        extend_frame_length(ctx, (MUINT32) *feature_data);
        pr_info("extend_frame_len done %llu\n", *feature_data);
        break;
    case SENSOR_FEATURE_SEAMLESS_SWITCH:
    {
        LOG_INF("SENSOR_FEATURE_SEAMLESS_SWITCH");
        if ((feature_data + 1) != NULL)
            pAeCtrls = (struct mtk_hdr_ae *)((uintptr_t)(*(feature_data + 1)));
        else
