 ...
 *============================================================================
 ****************************************************************************/
#include "hhfrontmipiraw_Sensor.h"


#define HHFRONT_EEPROM_READ_ID	0xA9
...
#define PFX "hhfront_camera_sensor"
...
static int hhfront_open(struct subdrv_ctx *ctx);
static int hhfront_get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
...
static int hhfront_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhfront_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int hhfront_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static int hhfront_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void set_sensor_cali(void *arg);
static void get_sensor_cali(void* arg);
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size);
static int hhfront_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);

/* STRUCT */

struct mtk_sensor_ctle_param hhfront_static_ctle_param = {
	.eq_bw = 0x3,
};

static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, hhfront_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, hhfront_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, hhfront_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, hhfront_get_otp_checksum_data},
	// {SENSOR_FEATURE_GET_MIN_SHUTTER_BY_SCENARIO, hhfront_get_min_shutter_by_scenario_adapter},
	{SENSOR_FEATURE_SET_AWB_GAIN, hhfront_set_awb_gain},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	{  /* B9-S9 3264x2448,30fps,2X2Binning,w/o PD,For MIPI(Around 864Mpps) */
		.frame_desc = frame_desc_prev,
		.num_entries = ARRAY_SIZE(frame_desc_prev),
		.mode_setting_table = hhfront_preview_setting,
		.mode_setting_len = ARRAY_SIZE(hhfront_preview_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhfront_preview_seamless_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhfront_preview_seamless_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 864000000,
	...
	{	/* B9-S7 3264x2448,30fps,2X2Binning,w/o PD,For MIPI(Around 868.8Mpps) */
		...
		.mode_setting_table = hhfront_capture_setting,
		.mode_setting_len = ARRAY_SIZE(hhfront_capture_setting),
	...
	{/*Reg B14-3, 3264x1836,30fps,2X2Binning,w/o PD,For(rst<21ms&cycle>1.5 s)_MIPI_445.2MPPS*/
		.frame_desc = frame_desc_vid,
		.num_entries = ARRAY_SIZE(frame_desc_vid),
		.mode_setting_table = hhfront_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(hhfront_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*Reg B15-3, 3264x1836,60fps,2X2Binning,w/o PD,For(rst<21ms&cycle>1.5 s)_MIPI_445.2MPPS*/
		.frame_desc = frame_desc_hs,
		.num_entries = ARRAY_SIZE(frame_desc_hs),
		.mode_setting_table = hhfront_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(hhfront_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*Reg B15-3, 3264x1836,60fps,2X2Binning,w/o PD,For(rst<21ms&cycle>1.5 s)_MIPI_445.2MPPS*/
		...
		.mode_setting_table = hhfront_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(hhfront_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
		...
		.ae_binning_ratio = 1000,
		.fine_integ_line = 356,
		.csi_param = {.dphy_trail = 66,},
	},
	{/*Reg V2-4 1280x960,10fps,4x4Binning_Crop,w/o PD,For MIPI<1.5G(Skew off)&Hblank=2.4us*/
		.frame_desc = frame_desc_cus1,
		.num_entries = ARRAY_SIZE(frame_desc_cus1),
		.mode_setting_table = hhfront_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(hhfront_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/* Reg F1-S9, 6560x4928,15fps,Full,w/o PD,For MIPI(Around 864Mpps)*/
		.frame_desc = frame_desc_cus2,
		.num_entries = ARRAY_SIZE(frame_desc_cus2),
		.mode_setting_table = hhfront_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(hhfront_custom2_setting),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = hhfront_custom2_seamless_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(hhfront_custom2_seamless_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 864000000,
	...
	{/*Reg_B8-3 3264x1856,15fps,2X2Binning,w/o PD,For MaxVB*/
		...
		.mode_setting_table = hhfront_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(hhfront_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{/*Reg V5 1632x1224,30fps,4x4Binning,w/o PD,For MaxVB&Binning-Ave*/
		.frame_desc = frame_desc_cus4,
		.num_entries = ARRAY_SIZE(frame_desc_cus4),
		.mode_setting_table = hhfront_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(hhfront_custom4_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{  /* Reg B16 3264x2448,60fps,2X2Binning,w/o PD,For VB Max */
		.frame_desc = frame_desc_cus5,
		.num_entries = ARRAY_SIZE(frame_desc_cus5),
		.mode_setting_table = hhfront_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(hhfront_custom5_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	...
	.ana_gain_type = 0,
	.ana_gain_step = 1,
	.ana_gain_table = hhfront_ana_gain_table,
	.ana_gain_table_size = sizeof(hhfront_ana_gain_table),
	.tuning_iso_base = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 16,
	...
	.init_setting_table = hhfront_init_setting,
	.init_setting_len = ARRAY_SIZE(hhfront_init_setting),
	.mode = mode_struct,
	...
	.init_in_open = TRUE,
	.ctle_param = &hhfront_static_ctle_param,
};

static struct subdrv_ops ops = {
	.init_ctx = init_ctx,
	.open = hhfront_open,
	.get_id = hhfront_get_imgsensor_id,
...
const struct subdrv_entry hhfront_mipi_raw_entry = {
	.name = "hhfront_mipi_raw",
	.id = HHFRONT_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int hhfront_open(struct subdrv_ctx *ctx)
{
	...
	DRV_LOG_MUST(ctx, "for hhfront start\n");
	...
	/* get sensor id */
	if (hhfront_get_imgsensor_id(ctx, &sensor_id) != ERROR_NONE)
...
static int hhfront_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	return hhfront_get_imgsensor_id(ctx, (u32 *)para);
}
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, HHFRONT_EEPROM_READ_ID >> 1,
		addr, data, size) < 0) {
		DRV_LOGE(ctx, "hhfront read_cmos_eeprom_p8 failed\n");
		return false;
	}
	DRV_LOGE(ctx, "hhfront read_cmos_eeprom_p8 success read size = %d\n", size);
	return true;
}

static void read_otp_info(struct subdrv_ctx *ctx)
{
	DRV_LOGE(ctx, "hhfront read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "hhfront read_otp_info end\n");
}
